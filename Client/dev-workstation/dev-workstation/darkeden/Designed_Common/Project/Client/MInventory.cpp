...
//20081111 아이템을 보관함에서 뽑아 올때 시프트 클릭시 위치 이동에 관해서 개인상점 등록 아이템의 경우 겹쳐지지 않도록 하기 위해서
#include "MGameDef.h"
#include "MStorage.h"
/*
#ifdef __GAME_CLIENT__
...
//20081111 아이템을 보관함에서 뽑아 올때 시프트 클릭시 위치 이동에 관해서 개인상점 등록 아이템의 경우 겹쳐지지 않도록 하기 위해서
			bool bPersonalShopItem = pInventoryItem->GetPersnal();
//add end
			//--------------------------------------------
			// 완전하게 쌓일 수 있는 조건이 되면...
			// 기존의 item의 좌표를 넘겨준다.
			...
			if (pInventoryItem->GetItemClass()==pItem->GetItemClass()
				...
				&& (((g_pTimeItemManager->IsExist(pInventoryItem->GetID()) == true)	&& (g_pTimeItemManager->IsExist(pItem->GetID()) == true))		
				|| ((g_pTimeItemManager->IsExist(pInventoryItem->GetID()) == false)	&& (g_pTimeItemManager->IsExist(pItem->GetID()) == false)))
				&& !bPersonalShopItem
				)
				// 시간제 아이템이 아닌 것은 시간제 아이템이 아닌것 끼리 시간제는 시간제끼리 겹쳐지도록 수정 (2008.08.27 shootkj)
