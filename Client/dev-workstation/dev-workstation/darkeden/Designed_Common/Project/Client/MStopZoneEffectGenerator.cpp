...
MStopZoneEffectGenerator::Generate( const EFFECTGENERATOR_INFO& egInfo )
{
	...
	else if(est>=EFFECTSPRITETYPE_unknow_2552 && est<=EFFECTSPRITETYPE_unknow_2566)
	{
		if (egInfo.pPreviousEffect!=NULL)
		{
			newFrameID = 5 + (egInfo.pPreviousEffect)->GetFrameID();
		...
		else
		{
			...
			{
				est = EFFECTSPRITETYPE_unknow_2557 + rand()%5;
			}
			else if (est>=EFFECTSPRITETYPE_unknow_2552
				&& est<=EFFECTSPRITETYPE_unknow_2556)
			{
				est = EFFECTSPRITETYPE_unknow_2552 + rand()%5;
			}
			else if (est>=EFFECTSPRITETYPE_unknow_2562
				&& est<=EFFECTSPRITETYPE_unknow_2566)
			{
				est = EFFECTSPRITETYPE_unknow_2562 + rand()%5;
			}
		}
	}
	...
	else if(est == EFFECTSPRITETYPE_HEAVENLY_SPARK_CENTER)
	{
		est = EFFECTSPRITETYPE_HEAVENLY_SPARK_CENTER - rand()%3;
	}
	...
	bool			repeatFrame	= (*g_pEffectSpriteTypeTable)[est].RepeatFrame;
			...
			//char szBuf[256];
			//sprintf(szBuf, "File: %d,%d,%d\n",
			//bltType, frameID, repeatFrame);
			//
...
#if __CONTENTS(__SECOND_RARE_SKILL)
			|| est == EFFECTSPRITETYPE_XRL_EXPLOSION
#endif //__SECOND_RARE_SKILL
