...
class MFakeCreature : public MCreatureWear {
	...
	public :
		enum FAKE_CREATURE_TYPE
		{
			...
			FAKE_CREATURE_RANDOM_MOVE,		// 해당 지역에서 랜덤 이동
			FAKE_CREATURE_XRL_ROCKET,			// XRL 로켓 런쳐
		};
	...
	public :
		...
		void				SetbatbreakerFastMoveAction(TYPE_SECTORPOSITION sX, TYPE_SECTORPOSITION sY,
											 TYPE_ACTIONINFO skillID, 
											 BYTE Dir); 
											 //MActionResult* pResult);
		...
		// chyaya
		void AddIdleAction(BYTE action);
		void RemoveIdleAction(BYTE action);
		void ClearIdleAction();

		// 2007.10.02 by chyaya
		void KeepRandomMove();
		void SetFakeRange(WORD w, WORD h)	{ m_FakeRangeW = w; m_FakeRangeH = h; }
