...
MStopZoneRectEffectGenerator::Generate( const EFFECTGENERATOR_INFO& egInfo )
{
	...
	// 임시 땜빵 코드.. 케케~
	//-----------------------------------------------------------
	BOOL bDarkness = FALSE, bGrayDarkness = FALSE, bSharpHail = FALSE, bChakramHail = FALSE, bBLIZZARD = FALSE,bgunfield = FALSE;
	...
	else if(frameID == EFFECTSPRITETYPE_unknow_2310  || frameID == EFFECTSPRITETYPE_unknow_2860)
	{
		bChakramHail = TRUE;
		power = 2;
	}
	else if(frameID == EFFECTSPRITETYPE_unknow_2304 || frameID == EFFECTSPRITETYPE_unknow_2854)
	{
		bBLIZZARD = TRUE;
		power = 2;
	}
	else if (frameID>=EFFECTSPRITETYPE_unknow_2552 && frameID<=EFFECTSPRITETYPE_unknow_2566)
	{
		if (egInfo.pPreviousEffect!=NULL)
		{
			TYPE_FRAMEID oldFrameID = (egInfo.pPreviousEffect)->GetFrameID();
		...
		{
			if (frameID>=EFFECTSPRITETYPE_unknow_2552
				&& frameID<=EFFECTSPRITETYPE_unknow_2556)
			{
				frameID = EFFECTSPRITETYPE_unknow_2552 + rand()%5;
			}
		}

		if( frameID >= EFFECTSPRITETYPE_unknow_2552 &&
			frameID <= EFFECTSPRITETYPE_unknow_2566 )
			bgunfield = TRUE;
		power = 2;
	}
	//---------------------------------------------
	...
	for (y=sY1; y<=sY2; y++)
	{
		for (x=sX1; x<=sX2; x++)
		{
			...
			/*		else if ( bChakramHail )
			{
			frameID = EFFECTSPRITETYPE_unknow_2310 + rand() %5;
			}*/
			pEffect = new MEffect(bltType);
			...
			if(egInfo.nActionInfo == SKILL_LAND_MINE_EXPLOSION || bSharpHail || bChakramHail || bBLIZZARD )
			{
				TempDelay = rand()%16;
