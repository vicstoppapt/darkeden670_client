...
MStopZoneXEffectGenerator::Generate( const EFFECTGENERATOR_INFO& egInfo )
{
	...
	pEffect->SetCount( egInfo.count+5 , egInfo.linkCount );			// 지속되는 Frame
	pEffect->SetDelayFrame(5);
	...
	pEffect->SetZ(egInfo.z0);			
	pEffect->SetStepPixel(egInfo.step);		// 실제로 움직이지는 않지만, 다음 Effect를 위해서 대입해준다.	
	pEffect->SetCount( egInfo.count+8 , egInfo.linkCount );		// 지속되는 Frame

	pEffect->SetDelayFrame(8);
	...
	pEffect->SetZ(egInfo.z0);			
	pEffect->SetStepPixel(egInfo.step);		// 실제로 움직이지는 않지만, 다음 Effect를 위해서 대입해준다.	
	pEffect->SetCount( egInfo.count+15 , egInfo.linkCount );			// 지속되는 Frame

	pEffect->SetDelayFrame(15);
	...
	pEffect->SetZ(egInfo.z0);			
	pEffect->SetStepPixel(egInfo.step);		// 실제로 움직이지는 않지만, 다음 Effect를 위해서 대입해준다.	
	pEffect->SetCount( egInfo.count+11, egInfo.linkCount );			// 지속되는 Frame

	pEffect->SetDelayFrame(11);
