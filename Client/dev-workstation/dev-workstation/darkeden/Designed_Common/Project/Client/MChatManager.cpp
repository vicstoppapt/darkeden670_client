...
MChatManager::RemoveCurse(char* str, bool bForce) const
{
	...
	//if( bNetMarble )
	{
		bool bResultCurse = false ; 	
		char strYok[31] = { 0 , } ;
		char strYokTemp[31] ; 
		int iLen = strlen(str) ; 
		memset((char *)strYok , 0 ,31) ;
		MStringMap::const_iterator iString = m_mapCurse.begin();

		MStringMap::const_iterator iString1 = m_mapCurse.end();

		char MaskStringTowByte[256] = "♡사랑해요♡사랑해요♡사랑해요♡사랑해요♡사랑해요♡사랑해요♡사랑해요"; 
		char MaskStringOneByte[256] = "**********************************************************************";

		while (iString != iString1) //m_mapCurse.end())
		{
			MString* pString = NULL;
			pString =iString->second;
			
			if (pString!=NULL)
			{
				memset((char *)strYok , 0 ,31) ; 
				strcpy(strYok ,  pString->GetString()) ; 
				int iYokLength = strlen(strYok) ;

				char *pStr = str ; 	
				for(int i = 0 ; i < iLen ; i++ , pStr++)
				{
					memset((char *)strYokTemp , 0 ,31) ; 
					memcpy((char *)strYokTemp , pStr , iYokLength) ; 
					int icmp = strcmp(strYokTemp, strYok) ; 
					if(!icmp)
					{	

						if(!(iYokLength & 1))
						{
							memcpy(pStr , MaskStringTowByte , iYokLength) ; 
							pStr += iYokLength-1 ; 
							i += iYokLength-1 ; 
						}
						else
						{
							memcpy(pStr , MaskStringTowByte , iYokLength-1) ;
							pStr += iYokLength-1  ;
							i += iYokLength-1 ; 
							memset(pStr , MaskStringOneByte[0] ,1) ;
						}
						bResultCurse = true ; 
					}
