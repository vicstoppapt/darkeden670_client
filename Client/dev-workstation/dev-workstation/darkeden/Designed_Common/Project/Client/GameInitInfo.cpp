...
#if __CONTENTS(__GPS_ADD)	//	MGPS_INFO	g_pGpsInfoTable;
...
//	SAFE_DELETE( g_pAddonSpriteTable );
	...
	if (g_pGuildInfoMapper==NULL)
	{
		...
		std::string filenameString	= g_pFileDef->getProperty("FILE_INFO_GUILD_INFO_MAPPER");
		const char* filename		= filenameString.c_str();
 
		ivfstream guildInfoFile(filename, std::ios::binary);
		std::ofstream guildInfoWritefile("e:\\guildmapper.inf", std::ios::binary);
	...
	{
		g_pGuildMarkManager = new MGuildMarkManager(
			g_pFileDef->getProperty("FILE_SPRITE_GUILD_MARK").c_str(),
			g_pFileDef->getProperty("FILE_INFO_GUILD_INFO_MAPPER").c_str(),
			g_pFileDef->getProperty("FILE_SPRITE_GRADE_MARK_SLAYER").c_str(),
			g_pFileDef->getProperty("FILE_SPRITE_GRADE_MARK_VAMPIRE").c_str(),
			g_pFileDef->getProperty("FILE_SPRITE_GRADE_MARK_OUSTERS").c_str(),
			g_pFileDef->getProperty("FILE_SPRITE_LEVEL_MARK_PET").c_str(),
			g_pFileDef->getProperty("FILE_SPRITE_MONSTER_LEVEL_MARK_PET").c_str(),
			g_pFileDef->getProperty("FILE_SPRITE_ORIGIN_MARK").c_str()
			);
	}
...
//	}
	...
	ivfstream chatInfoFile2;//(g_pFileDef->getProperty("FILE_INFO_USINGCOLORSET").c_str(), ios::binary);
...
//	g_pChatManager->LoadFromFileCurse("yok.txt") ;
...
//	helpManagerFile.close();
	...
	ivfstream helpStringTableFile;//(g_pFileDef->getProperty("FILE_INFO_USINGCOLORSET").c_str(), ios::binary);
	...
	ivfstream levelNameTableFile;//(g_pFileDef->getProperty("FILE_INFO_USINGCOLORSET").c_str(), ios::binary);
	...
	ivfstream MonsterNameTableFile;//(g_pFileDef->getProperty("FILE_INFO_USINGCOLORSET").c_str(), ios::binary);
	...
	*/
	...
	ivfstream usingColorSetTable2;//(g_pFileDef->getProperty("FILE_INFO_USINGCOLORSET").c_str(), ios::binary);
	...
	ivfstream soundTable2;//(g_pFileDef->getProperty("FILE_INFO_SOUND").c_str(), ios::binary);
	...
	ivfstream musicTable2;//(g_pFileDef->getProperty("FILE_INFO_MUSIC").c_str(), ios::binary);
	...
	ivfstream zoneTable2;//(g_pFileDef->getProperty("FILE_INFO_ZONE").c_str(), ios::binary);
	...
	ivfstream itemTable2;//(g_pFileDef->getProperty("FILE_INFO_ITEM").c_str(), ios::binary);
	...
	*/
	...
	ivfstream interactionObjectTable2;//(g_pFileDef->getProperty("FILE_INFO_INTERACTIONOBJECT"), ios::binary);
	...
	// Load
	//------------------------------------------------
	ivfstream creatureSpriteTable2;//(g_pFileDef->getProperty("FILE_INFO_CREATURE"), ios::binary);
...
//	creatureActionSpriteTable2.close();
	...
	ivfstream creatureTable2;//(g_pFileDef->getProperty("FILE_INFO_CREATURE"), ios::binary);
...
//#ifdef _DEBUG
//	std::ofstream CreatureTable("CreatureTable.csv");
//	int CreatureMaxSize = (*g_pCreatureTable).GetSize();
//	for(int CreaturID = 0; CreaturID < CreatureMaxSize; ++CreaturID)
//	{
//		(*g_pCreatureTable)[CreaturID].SaveToCSVFile(CreatureTable, CreaturID);
//	}
//	CreatureTable.close();
//#endif
	...
	ivfstream CreatureSpriteTypeMapper2;//(g_pFileDef->getProperty("FILE_INFO_CREATURE"), ios::binary);
	...
	ivfstream ShopTemplateTable2;//(g_pFileDef->getProperty("FILE_INFO_SHOPTEMPLATE"), ios::binary);
	...
	ivfstream NPCTable2;//(g_pFileDef->getProperty("FILE_INFO_CREATURE"), ios::binary);
	...
	ivfstream NPCScriptTable;//(g_pFileDef->getProperty("FILE_INFO_CREATURE"), ios::binary);
...
#ifdef _DEBUG
	std::ofstream NPCScriptTableCSV("NPCScript.csv");
	g_pNPCScriptTable->SaveToCSVFile(NPCScriptTableCSV);
	NPCScriptTableCSV.close();
#endif
	...
	ivfstream effectSpriteTypeTable2;//(g_pFileDef->getProperty("FILE_INFO_EFFECTSPRITETYPE"), ios::binary);
	...
	// Load
	//------------------------------------------------
	ivfstream actionEffectSpriteTypeTable2;//(g_pFileDef->getProperty("FILE_INFO_EFFECTSPRITETYPE"), ios::binary);
	...
	ivfstream EffectStatusTable2;//(g_pFileDef->getProperty("FILE_INFO_EffectStatus"), ios::binary);
	...
	ivfstream skillTable2;//(g_pFileDef->getProperty("FILE_INFO_skill"), ios::binary);
	...
	ivfstream serverSkillInfoFile;//(g_pFileDef->getProperty("FILE_INFO_skill"), ios::binary);
	...
	//
	//---------------------------------------------------------------------
	ivfstream gpsInfoFile;//(g_pFileDef->getProperty("FILE_INFO_skill"), ios::binary);
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_INFO_GPS").c_str(), gpsInfoFile))
		return FALSE;
	g_pGpsInfoTable->LoadFromFile(gpsInfoFile);
	gpsInfoFile.close();
#endif //__GPS_ADD
	//---------------------------------------------------------------------
	//
	...
	ivfstream rankBonusInfoFile;//(g_pFileDef->getProperty("FILE_INFO_skill"), ios::binary);
	...
	// Load
	//------------------------------------------------
	ivfstream strExpFile;
	ivfstream dexExpFile;
	ivfstream intExpFile;
	ivfstream vampireExpFile;
	ivfstream oustersExpFile;
	ivfstream slayerRankExpFile;
	ivfstream vampireRankExpFile;
	ivfstream oustersRankExpFile;
	ivfstream advancementExp;
	ivfstream petExpFile;
	...
	ivfstream MonsterKillQuestInfoFile;
	...
	// Load World Map Info
	//-----------------------------------
	DEBUG_ADD("[ InitGame ]  Information - LoadWorldMapInfo");
