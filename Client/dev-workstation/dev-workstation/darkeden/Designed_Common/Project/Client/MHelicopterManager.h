...
class MHelicopter {
	public :
		...
		bool			SetChaseCreature(TYPE_OBJECTID id, int x0, int y0, int nExtendLevel = 0);	

		TYPE_OBJECTID	GetCreatureID() const	{ return m_CreatureID; }
		TYPE_OBJECTID	GetEffectID() const		{ return m_EffectID; }

		int				GetExtendLevel() const	{ return m_nExtendLevel; }
	...
	protected :
		...
		int						m_nExtendLevel;	// Extend Portal Level
};
