...
class MOustersGear : public MPlayerGear	{
	public :
		...
		//----------------------------------------------------
		//nanomech 2006.03.08 Item수정 작업 순서 교정
		enum GEAR_OUSTERS
		{
			...
			//by nanomech 2006.02.08
			GEAR_OUSTERS_ETHEREALCHAIN,		// 전면전 보상아이템 목걸이 타입 코어잽 .. 목걸이2에만 장착된다.
			
			GEAR_OUSTERS_CUE1,				// 큐1 - 반지에 덛붙이는 아이템
			GEAR_OUSTERS_CUE2,				// 큐2
		...
		inline static bool		IsGearSlotEtherealChain(MItem* pItem);
		//nanomech 2006.03.08 Item수정 작업 end
		inline static bool		IsGearSlotCue(MItem* pItem);
	...
	protected :
		...
		static const int s_Slot_Offset_Cue;			///< 큐를 덧붙여 씌우게될 아이템과 큐의 인덱스 차이 by chyaya

		BYTE		m_Gilles_CoreZap;				///< 왜 static const로 안했을까 -_-?
