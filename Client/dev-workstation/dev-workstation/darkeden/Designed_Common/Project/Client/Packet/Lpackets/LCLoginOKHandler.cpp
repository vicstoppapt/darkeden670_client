...
#endif //__XTRAP
	if( g_pUserInformation->UserID.GetLength() >= 13 )
	{
...
//			SetMode( MODE_WAIT_WORLD_LIST );
		...
		g_pUserInformation->IsUnderFifthteen = pPacket->isUnderFifthteen();	// 15세 미만인가?
		g_pUserInformation->IsUnderFifthteen = false;// by svi, 豚冀痰빵朞嶝角뤠직沂

//		if(g_pUserInformation->IsUnderFifthteen)		// 15세 미만이면 피가 안떨어진다
//			g_pUserOption->BloodDrop = false ;
		...
		*/
		...
		bGoreLevel = true; //by viva
		g_pUserInformation->bNetmarbleGoreLevel = bGoreLevel;	//bNetmarbelGoreLevel값은 현재 사용되는 곳이 없다 (20071218)

		if (!bGoreLevel)										//Login한 유저가 청소년이다.
			g_pUserOption->UseTeenVersion = TRUE;				//강제로, UseTeenVersion으로 설정한다.
		else if( g_pUserOption->UseTeenVersion)					//Login한 유저가 청소년이 아니지만, 로그인창에서 UseTeenVersion을 설정했다면!!
			bGoreLevel = FALSE;									//bGoreLevel을 FALSE로 만들어, 게임초기화시 반영한다. (SetGoreLevel)
...
//#endif
	...
	if( g_pUserInformation != NULL && !g_pUserInformation->IsNetmarble )
	{
		...
		{
			g_LeftPremiumDays = 15;	// 휴면 계정 이벤트가 15일로 바뀌었다. (2008.04.29)
