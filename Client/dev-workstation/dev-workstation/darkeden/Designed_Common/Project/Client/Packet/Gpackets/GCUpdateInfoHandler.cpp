...
//	UI_AffectUserOption();
	...
	{	
		...
		*/
		...
		backupbInvincible = g_pPlayer->GetInvincible();//20070409 by dieaice iddqd버그 수정
		delete g_pPlayer;
	}	
	g_pPlayer = new MPlayer;

	//20070409 by dieaice iddqd버그 수정
		...
		for(int i = 0 ; i < pEInfo->getListNum(); i++)
		{
			...
			if( status == EFFECTSTATUS_TRANSFORM_TO_BAT )
			{
...
#if __CONTENTS(__SECOND_TRANSFORTER)
				{
					...
					else if(VampireInfo->getWingItemType()==2)
						g_PreviousCreatureType = CREATURETYPE_NEDE;
					else if(VampireInfo->getWingItemType()==3)
...
//	UI_CloseSubInventory(); // subInventory는 닫자..정보가 없기 땜시..
...
//	g_pPlayer->SetBatColorSet(0xFFFF);
	...
	if(pcType == PC_VAMPIRE)
	{
		...
		if( bIsAdvancementClass && g_pPlayer->GetCreatureType() == CREATURETYPE_VAMPIRE_GHOST )
			...
			g_pPlayer->SetWingEffect1();
		else if(g_pPlayer->GetCreatureType() == CREATURETYPE_NEDE)
			g_pPlayer->CreateAttachEffect(EFFECTSPRITETYPE_unknow_2328, 0xFFFF, 0,FALSE, MEffect::EFFECT_ATTACH, false);
		//else if(g_pPlayer->GetCreatureType() == CREATURETYPE_KLTL)
