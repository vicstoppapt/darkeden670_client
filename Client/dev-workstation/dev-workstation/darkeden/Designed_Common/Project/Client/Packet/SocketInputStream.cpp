...
//////////////////////////////////////////////////////////////////////
void SocketInputStream::peek ( char * buf , uint len, uint pos ) 
	throw ( ProtocolException , Error )
{
	__BEGIN_TRY
	...
	if ( m_Head < m_Tail ) {	// normal order
		...
		memcpy( buf , &m_Buffer[m_Head + pos] , len );
		...
		int rightLen = m_BufferLen - (m_Head + pos);
		if ( (len) <= rightLen ) {
			memcpy( &buf[0]        , &m_Buffer[m_Head + pos] , len );
		} else 
		{
			if(rightLen == 0)
			{
				memcpy( &buf[0] , &m_Buffer[0]      , len );
			}
			else if (rightLen > 0) 
			{
				memcpy( &buf[0]        , &m_Buffer[m_Head + pos] , rightLen );
				memcpy( &buf[rightLen] , &m_Buffer[0]      , len - rightLen );
			}
			else
			{
				rightLen = abs(rightLen);
				memcpy( &buf[0]        , &m_Buffer[rightLen] , len );			
			}
		}
	}
...
#if __CONTENTS(__CRYPT)	
bool SocketInputStream::Replace(PACKETDATA* pData, uint len)
	throw ( ProtocolException , Error )
{
	__BEGIN_TRY
...
#if __CONTENTS(__CRYPT)	
bool SocketInputStream::DecryptHeader(PACKETDATA* pData)
{ 
	m_pCryptManager->DecryptHeader(pData); 
	if(m_Key == pData->m_Key.dwKey)
		return false; // 만약 키값이 같다면 해킹으로 간주
	m_Key = pData->m_Key.dwKey;
	return true;
}
#endif
