	 ...
	 throw ( ProtocolException , Error )
{
	...
	switch (pPacket->getCode())
	{
		...
		case GC_TRADE_VERIFY_CODE_ADD_ITEM_FAIL :
			// 2008.09.25	shootkj
			// 아이템 추가를 못하는 경우 클라이언트에서는 아이템을 지워주지 않아야 한다.
//			if (g_pTempInformation->GetMode()==TempInformation::MODE_TRADE_VERIFY_ADD_ITEM)	
			{
				g_pPlayer->ClearItemCheckBuffer();
				...
				MItem* pItem	= (MItem*)g_pTempInformation->pValue_Cannot_ItemExchange;	//추가된 코드 2008.09.25	shootkj
				
				//-----------------------------------------------------------
				// 선택되어 있던 아이템 --> 취소
				//-----------------------------------------------------------
				if(pItem != NULL)															//추가된 코드 2008.09.25	shootkj
					pItem->UnSetTrade();													//추가된 코드 2008.09.25	shootkj

				//-----------------------------------------------------------
				// 뭔가 바뀐다면... OK취소
				//-----------------------------------------------------------

				if(g_pTradeManager != NULL)													//추가된 코드 2008.09.25	shootkj
				{
					g_pTradeManager->RefuseOtherTrade();									//추가된 코드 2008.09.25	shootkj
					g_pTradeManager->RefuseMyTrade();										//추가된 코드 2008.09.25	shootkj
				}

				g_pTempInformation->pValue_Cannot_ItemExchange	= NULL;						//추가된 코드 2008.09.25	shootkj
