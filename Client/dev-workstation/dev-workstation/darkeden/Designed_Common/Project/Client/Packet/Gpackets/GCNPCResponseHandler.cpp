...
//			UI_ShowGuildQuit();
		...
		//	value
			iSecondParameter = pPacket->getParameter(); //  : 기부금  : 측의금 
		...
		case NPC_RESPONSE_SHOW_COMMON_MESSAGE_DIALOG:           // 각종 메시지 다이얼로그를 띄운다.
			...
			break;
		case NPC_RESPONSE_SHOW_DONATION_WEDDING_COMPLETE_DIALOG:	// 축의금 성공창을 띄운다.
			 UI_PopupMessage( UI_STRING_MESSAGE_WEDDING_CONTRIBUTION_THANKS);
			 break ;	
		case NPC_RESPONSE_SHOW_ALEADY_DONATED_DIALOG:	// 축의금 을 이미 내셨습니다. 
			 UI_PopupMessage( UI_STRING_MESSAGE_WEDDING_CONTRIBUTION_FAIL);
			 break ; 
		
		case NPC_RESPONSE_NOT_CAHNGE_COLOR:	// 색상이 중복되어 바꿀수 없다.
			 UI_PopupMessage( UI_STRING_MESSAGE_NOT_FLAME_COLOR);
			 g_pPlayer->SetPrevDyeColorSet(ITEMTYPE_NULL) ;
			 break ; 

		case NPC_RESPONSE_NOT_ENOUGH_MONEY_CAHNGE_COLOR:	// 돈이 충분하지 않아 색상을 바꿀수 없다.
			 UI_PopupMessage( UI_STRING_MESSAGE_NOT_MONEY_FKANE_COLOR);
			 g_pPlayer->SetPrevDyeColorSet(ITEMTYPE_NULL) ;
			 break ; 

		case NPC_RESPONSE_CHANGE_COLOR_OK:	// 염색이 완료 되었습니다.
			{	
		...
		case NPC_NOT_ENOUGH_EXCHANGE_OF_MONEY:	// 돈이 충분하지 않아 수표로 바꿀수 없다
			 UI_PopupMessage( UI_STRING_MESSAGE_NOT_ENOUGH_EXCHANGE_OF_MONEY);
			 break ; 
		
		case NPC_ENOUGH_PLAYER_MONEY:	// 플레이어의 소지금이 최대 소지금액을 넘었다
			 UI_PopupMessage( UI_STRING_MESSAGE_NOT_ENOUGH_PLAYER_MONEY);
			 break ; 

		case NPC_NOT_ENOUGH_CHECK_MONEY:	// 수표가 없다!
			 UI_PopupMessage( UI_STRING_MESSAGE_NOT_ENOUGH_CHECK_MONEY);
			 break ; 

		case NPC_EXCHANGE_OF_MONEY_OK:	// 정상적으로 거래가 완료 되었습니다
		...
		// 공성전 신청 관련 에러 메시지 
		case NPC_RESPONSE_NOT_ENOUGH_LEVEL:         // 레벨이 충분 하지 않다 
			UI_PopupMessage(UI_STRING_MESSAGE_SIEGE_WAR_CHALLENGE_ERROR_1);
			break;
		case NPC_RESPONSE_NOT_ENOUGH_GUILD_MEMBER:  // 길드원이 부족하다 
			UI_PopupMessage(UI_STRING_MESSAGE_SIEGE_WAR_CHALLENGE_ERROR_2);
			break;
		case NPC_RESPONSE_NOT_ENOUGH_GUILD_LEVEL:   // 길드원의 평균 레벨이 부족하다 
			UI_PopupMessage(UI_STRING_MESSAGE_SIEGE_WAR_CHALLENGE_ERROR_3);
			break;

		case NPC_RESPONSE_NOT_ENTER_SURVIVAL_ZONE:	// 서바이벌 존에 입장 불가능하다. (입장 불가능 시간, 제한 인원)
			UI_PopupMessage(UI_STRING_MESSAGE_NOT_ENTER_SURVIVAL_ZONE);
			break;
			
		case NPC_RESPONSE_GIVE_GAMBLE_ITEM_OK:		// 겜블해서 아이템 받았을때
			sprintf(sz_temp, (*g_pGameStringTable)[UI_STRING_MESSAGE_GIVE_GAMBLE_ITEM].GetString(), pPacket->getParameter() );
			g_pUIDialog->PopupFreeMessageDlg( sz_temp );
			break;
		case NPC_RESPONSE_GAMBLE_INIT_OK:			// 겜블 누적점수 초기화 할 때
			UI_PopupMessage(UI_STRING_MESSAGE_GAMBLE_ACCUMULATION_INIT);
			break;

		case NPC_NOT_ENOUGH_EVENT_ITEM:				// 교환에 필요한 이벤트 아이템이 부족할 때
			UI_PopupMessage(UI_STRING_MESSAGE_NOT_ENOUGH_EVENT_ITEM);
			break;
			
		case NPC_NOT_ENOUGH_ITEM:					// 교환에 필요한 아이템이 부족할 때
			UI_PopupMessage(UI_STRING_MESSAGE_NOT_ENOUGH_ITEM);
			break;

		// 2007 출석 체크 이벤트
		case NPC_RESPONSE_DENY_ATTEND:					// 신규 가입 회원만 이벤트에 참여 가능 합니다.
			UI_PopupMessage(UI_STRING_MESSAGE_DENY_ATTEND);
			break;

		case NPC_RESPONSE_TODAY_NOT_ATTEND:				// 이미 출석 하셨습니다. 내일 다시 참여해 주세요.
			UI_PopupMessage(UI_STRING_MESSAGE_TODAY_NOT_ATTEND);
			break;

		case NPC_RESPONSE_CHECK_COUNT_ATTEND:			// [캐릭터]님은 [출석 체크 회수]번 출석하셨습니다.
		...
		case NPC_RESPONSE_BLITZ_COUPON:					// 총 문양 누적 개수: %d\n남은 잔여 문양 개수: %d
