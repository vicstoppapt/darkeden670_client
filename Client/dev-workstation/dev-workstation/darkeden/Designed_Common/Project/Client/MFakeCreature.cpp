...
extern bool			HasEffectStatusWingSylph( MCreature* pCreature );
extern void			RemoveEffectStatusWingSylph( MCreature* pCreature );
#endif //__FAST_TRANSFORTER
#if __CONTENTS(__SECOND_TRANSFORTER)
extern bool			HasEffectStatusUnicorn( MCreature* pCreature );
extern void			RemoveEffectStatusUnicorn( MCreature* pCreature );

extern bool			HasEffectStatusOusterNewWings1( MCreature* pCreature );
extern void			RemoveEffectStatusOusterNewWings1( MCreature* pCreature );

extern bool			HasEffectStatusOusterNewWings2( MCreature* pCreature );
extern void			RemoveEffectStatusOusterNewWings2( MCreature* pCreature );
...
{ 
	...
	switch (fct)
	{
		...
		case FAKE_CREATURE_XRL_ROCKET:
#endif //__SECOND_RARE_SKILL
			break;
		case FAKE_CREATURE_RANDOM_MOVE:
			m_nextMoveTime	= timeGetTime() + 120 + rand() % 50;
			m_PatrolCount	= 8 + rand() % 3;
			break;
	}
}
		...
		if (m_ActionCount>=m_ActionCountMax)
		{
			...
			if (m_bAlive)
			{
				...
				{		
					...
					{
						BYTE standAction = (m_MoveDevice==MOVE_DEVICE_RIDE)? ACTION_SLAYER_MOTOR_STAND : ACTION_STAND;

						if(standAction == ACTION_STAND && !m_IdleActionSet.empty())
						{
							if(rand()%5 == 0)
							{
								int randomValue = rand()%m_IdleActionSet.size();

								std::set<BYTE>::iterator pos = m_IdleActionSet.begin();
								std::advance(pos, randomValue);

								standAction = *pos;
							}	
						}
#if __CONTENTS(__NEW_PET_FIRE_ELEMENTAL)
						TYPE_SOUNDID soundID = (*g_pCreatureTable)[m_CreatureType].GetActionSound( standAction );
...
void
MFakeCreature::SetbatbreakerFastMoveAction(TYPE_SECTORPOSITION sX, TYPE_SECTORPOSITION sY,
											 TYPE_ACTIONINFO skillID, 
											 BYTE Dir)
{
	FastMovePosition(sX, sY);
	...
	m_nBasicActionInfo = m_nSpecialActionInfo = skillID;
	m_DirectionMove = m_Direction = m_CurrentDirection = Dir;
}
...
//			}
			...
			case FAKE_CREATURE_XRL_ROCKET:
			{
				...
				{
					if(pCreature != NULL)
						ExecuteActionInfoFromMainNode(SKILL_CLIENT_XRL_BLOW,pCreature->GetX(), pCreature->GetY(), 0,pCreature->GetDirection(),	0,	
								pCreature->GetX(), pCreature->GetY(), 0, 10, NULL, false);			
					return true;
				}
			}
		...
		case FAKE_CREATURE_DRAGON_TORNADO:
			{
				...
				firstSector.x = g_pPlayer->GetX() + SECTOR_SKIP_PLAYER_LEFT-1; 
				firstSector.y = g_pPlayer->GetY() + SECTOR_SKIP_PLAYER_UP-1;
				endSector.x	  = firstSector.x+SECTOR_WIDTH+2;
				endSector.y   = firstSector.y+SECTOR_HEIGHT+2;
				// 화면 밖으로 가믄 없애자..일단..
...
MFakeCreature::KeepWanderCreature()
{
	...
	firstSector.x = g_pPlayer->GetX() + SECTOR_SKIP_PLAYER_LEFT-1;// +4 테스트
	firstSector.y = g_pPlayer->GetY() + SECTOR_SKIP_PLAYER_UP-1;
	endSector.x	  = firstSector.x+SECTOR_WIDTH+2;
	endSector.y   = firstSector.y+SECTOR_HEIGHT+2;
	// 화면 안에 있는 유령 이믄..
...
#if __CONTENTS(__SECOND_RARE_SKILL)		
		&& GetCreatureType() != CREATURETYPE_XRL_ROCKET_LUNCHER
#endif //__SECOND_RARE_SKILL
		)
		return;
...
void
MFakeCreature::KeepRandomMove()
{
	if(m_nextMoveTime > timeGetTime()) return;

	const TYPE_SECTORPOSITION rangeStartPosX = m_FakeX - m_FakeRangeW / 2;
	const TYPE_SECTORPOSITION rangeStartPosY = m_FakeY - m_FakeRangeH / 2;


	// 랜덤으로 위치를 선택한다.
	TYPE_SECTORPOSITION nextPosX = rangeStartPosX + rand() % m_FakeRangeW;
	TYPE_SECTORPOSITION nextPosY = rangeStartPosY + rand() % m_FakeRangeH;

	int nextDir = MTopView::GetDirectionToPosition(GetX(), GetY(), nextPosX, nextPosY);

	m_bTraceFlag = true;
	SetCurrentDirection(nextDir);

	int distance = max(abs(m_X - nextPosX), abs(m_Y - nextPosY));

//	SetFakeCreatureFastMoveAction(nextPosX, nextPosY, ACTIONINFO_NULL, OBJECTID_NULL);	

	const int averageRange = (m_FakeRangeW + m_FakeRangeH) / 2;

	if(distance > averageRange / 2 && m_PatrolCount > 3)
	{
		SetFakeCreatureFastMoveAction(nextPosX, nextPosY, ACTIONINFO_NULL, OBJECTID_NULL);
		m_nextMoveTime	= timeGetTime() + distance * 20 + rand() % ((distance + 1) * 10);
	}
	else
	{
		SetNextDestination(nextPosX, nextPosY);
		m_nextMoveTime	= timeGetTime() + distance * 100 + rand() % ((distance + 1) * 100);
	}
	
	m_DestX = nextPosX;
	m_DestY = nextPosY;
	m_DirectionMove = nextDir;
...
// 	{
// 		if(!IsDead()) SetDead();
// 		m_nextMoveTime	= timeGetTime() + 200;
// 	}
...
void MFakeCreature::AddIdleAction(BYTE action)
{
	m_IdleActionSet.insert(action);
}


void MFakeCreature::RemoveIdleAction(BYTE action)
{
	m_IdleActionSet.erase(action);
}


void MFakeCreature::ClearIdleAction()
{
	m_IdleActionSet.clear();
}