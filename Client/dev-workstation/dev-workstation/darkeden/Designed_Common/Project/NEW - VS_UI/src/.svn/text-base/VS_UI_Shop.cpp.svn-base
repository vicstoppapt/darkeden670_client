...
C_VS_UI_SHOP::C_VS_UI_SHOP()
{
	...
	Set(10 + ptOffset.x, 30 + ptOffset.y, 332, 497);
					...
					else
					{
						...
						if (p_item->IsIdentified())
						{
							...
							if(p_item->IsTuningItem())
								CIndexSprite::SetUsingColorSet(p_item->GetTuningItemColorSet() ,0 );
							else
#endif //__TUNING_ITEM
...
{	
	int buttonID = p_button->GetID();
	if (buttonID == MYSTERIOUS_TAB_ID && (!m_pShop->GetShelf(MYSTERIOUS_TAB_ID)->IsEnable()
		|| !g_pSystemAvailableManager->IsAvailableGambleSystem() ) ||
		buttonID == NORMAL_TAB_ID && !m_pShop->GetShelf(NORMAL_TAB_ID)->IsEnable() ||
		buttonID == SPECIAL_TAB_ID && !m_pShop->GetShelf(SPECIAL_TAB_ID)->IsEnable())
		return;
