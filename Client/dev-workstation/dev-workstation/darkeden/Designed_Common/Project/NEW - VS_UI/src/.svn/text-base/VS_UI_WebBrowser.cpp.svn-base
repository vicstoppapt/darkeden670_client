...
C_VS_UI_WEBBROWSER::C_VS_UI_WEBBROWSER()
{
	...
	Set(g_pUserInformation->iResolution_x/2 - m_pC_WebBrowser_spk->GetWidth()/2 , g_pUserInformation->iResolution_y/2 - m_pC_WebBrowser_spk->GetHeight()/2  , m_pC_WebBrowser_spk->GetWidth(), m_pC_WebBrowser_spk->GetHeight());

//#ifdef CONVERT_1024_768
	//Set(0, 0, RESOLUTION_X , RESOLUTION_Y);
//
//	Set(RESOLUTION_X/2 - m_pC_WebBrowser_spk->GetWidth()/2 , RESOLUTION_Y/2 - m_pC_WebBrowser_spk->GetHeight()/2  , m_pC_WebBrowser_spk->GetWidth(), m_pC_WebBrowser_spk->GetHeight());
	...
	m_pC_button_group  = new ButtonGroup(this);
		...
		{			
			case RACE_SLAYER:
				 ...
				 break ; 	
			case RACE_VAMPIRE:
				 m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(779 - m_pC_WebBrowser_spk->GetWidth(WEBBROWSER_CLOSE_BUTTON)-3 , 5+2 , m_pC_WebBrowser_spk->GetWidth(WEBBROWSER_CLOSE_BUTTON), m_pC_WebBrowser_spk->GetHeight(WEBBROWSER_CLOSE_BUTTON), WEBBROWSER_CLOSE, this, WEBBROWSER_CLOSE_BUTTON));	 
				 break ; 
			case RACE_OUSTERS:
				 m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(779 - m_pC_WebBrowser_spk->GetWidth(WEBBROWSER_CLOSE_BUTTON) + 10 , 5 + 10 , m_pC_WebBrowser_spk->GetWidth(WEBBROWSER_CLOSE_BUTTON), m_pC_WebBrowser_spk->GetHeight(WEBBROWSER_CLOSE_BUTTON), WEBBROWSER_CLOSE, this, WEBBROWSER_CLOSE_BUTTON));
				 break ; 
		}
...
C_VS_UI_WEBBROWSER::~C_VS_UI_WEBBROWSER()
{
	...
	if(m_pC_WebBrowser_spk)
	{
		DeleteNew(m_pC_WebBrowser_spk);
		m_pC_WebBrowser_spk = NULL;
	}
...
//	SendMessage(g_hWnd , WM_ACTIVATE,  0 ,  0) ;
	SendMessage(g_hWnd , WM_ACTIVATE,  1 ,  0) ;
...
bool C_VS_UI_WEBBROWSER::Start(HWND hwndApp,  char* pURL, HINSTANCE hInst) //   void* pWebOjbect )
{
	...
	if ((m_HIWebDll = LoadLibrary("DarkedenWeb.dll")))
	{
		...
		int iStartRangeX = 0 , iStartRangeY  = 0 ;
		switch(g_eRaceInterface)
		{			
			case RACE_SLAYER:
				 iStartRangeX -= 3 ;
		...
		if ((m_hWnd_Explorer = CreateWindowEx(0, ClassName, "Darkeden WebPage", WS_CHILD | WS_VISIBLE ,

						iStartRangeX + x + 25, iStartRangeY + y + 25, 754, 553,
...
void	C_VS_UI_WEBBROWSER::Show()
{
	...
	{
		gpC_global_resource->m_pC_assemble_box_spk->BltLocked(x, y, WEBBROWSER_MAIN);
