...
//	int n = 0;
	...
	for (i=0; i<MAX_ITEM_CLASS; i++)
	{
		...
		if( 1
		/*
			i >= ITEM_CLASS_CARRYING_RECEIVER	&&
			i <= ITEM_CLASS_MITTEN
		*/

		/*	i == ITEM_CLASS_CORE_ZAP			||
			i == ITEM_CLASS_ETHEREAL_CHAIN		||
			i == ITEM_CLASS_CUE_OF_ADAM			||
			i == ITEM_CLASS_RING				||
			i == ITEM_CLASS_BRACELET			||
			i == ITEM_CLASS_VAMPIRE_BRACELET	||
			i == ITEM_CLASS_OUSTERS_RING		||
			i == ITEM_CLASS_BELT				||
			i == ITEM_CLASS_OUSTERS_ARMSBAND	||*/
			//i == ITEM_CLASS_EVENT_ITEM ||
			//i == ITEM_CLASS_EVENT_STAR //|| 
			//i == ITEM_CLASS_CONTRACT_OF_BLOOD
			...
			for (int t=0; t<classSize; t++)
			{
				...
				while(1)
				{
					...
					if((*g_pItemOptionTable)[option].Name.GetLength() != 0)
					{
						...
						//if(rand()&0x1) p_item->AddItemOption( option );
						DWORD dw = p_item->GetItemOptionColorSet() ; 
						break;
					}
				}
				...
				p_item->SetItemColorSet(645);
				
				//p_item->SetItemColorSet(647);

				p_item->SetItemOptionExtFlag(rand() & 1) ; 
				//p_item->SetItemOptionExtFlag(true);
