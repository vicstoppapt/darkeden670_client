...
public:
	...
	char		m_composing_string[1024];
	char		m_Before_composing_string[1024];

	CI();
	~CI();
	
	virtual	bool	IsKorean()	{ return true; }
...
class CI_JAPAN : public CI
{
	bool		bHiragana ; 
public :

	bool	IsJapanese() { 	return true; }
	 void	IMEObjectConvert(wchar_t*	StrIMEWide , int&	StrIMEWideSize);
	void IME_MessageProcessor(UINT message, WPARAM wParam, LPARAM lParam);
	void IME_NextComposition();
	void IME_Composition();
