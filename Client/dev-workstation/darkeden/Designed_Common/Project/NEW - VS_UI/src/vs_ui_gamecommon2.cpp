...
bool C_VS_UI_MIXING_FORGE::Check(int grid_start_x, int grid_start_y)
{
	...
	if( p_item != NULL)
	{
		if(p_item->GetItemClass()==ITEM_CLASS_CORE_ZAP)
		{
			...
			{
				g_pUIDialog->PopupFreeMessageDlg("ÏÞÖÆµÄ½äÖ¸Ì×²»¿ÉÒÔÉý¼¶");
				return false;
			}
			else
			if(m_p_select_item[0] == NULL && m_p_select_item[1] != NULL &&
			...
			{
				g_pUIDialog->PopupFreeMessageDlg("Ö»ÓÐÏàÍ¬µÈ¼¶µÄ½äÖ¸Ì×¿ÉÒÔºÏ³É£¡");
				return false;
			}
			else
			if(m_p_select_item[0] != NULL && m_p_select_item[1] == NULL &&
				(m_p_select_item[0]->GetGrade() != p_item->GetGrade()))
			{
				...
				{
					g_pUIDialog->PopupFreeMessageDlg("Ö»ÓÐÍ¬ÑùÑÕÉ«µÄ½äÖ¸Ì×¿ÉÒÔºÏ³É£¡");
					return false;
				}
		}
		else if(p_item->GetItemClass()==ITEM_CLASS_CUE_OF_ADAM)
		{
			if(p_item->IsUniqueItem()||p_item->IsQuestItem())
			{
				g_pUIDialog->PopupFreeMessageDlg("ÏÞÖÆµÄÊÖïíÌ×²»¿ÉÒÔÉý¼¶");
				return false;
			}
			else
			if(m_p_select_item[0] == NULL && m_p_select_item[1] != NULL &&
				(m_p_select_item[1]->GetGrade() != p_item->GetGrade()))
			{
				g_pUIDialog->PopupFreeMessageDlg("Ö»ÓÐÏàÍ¬µÈ¼¶µÄÊÖïíÌ×¿ÉÒÔºÏ³É£¡");
				return false;
			}
			else
			if(m_p_select_item[0] != NULL && m_p_select_item[1] == NULL &&
				(m_p_select_item[0]->GetGrade() != p_item->GetGrade()))
			{
				...
				if(m_p_select_item[0] != NULL && m_p_select_item[1] == NULL &&
					(m_p_select_item[0]->GetItemType() != p_item->GetItemType()))
				{
					g_pUIDialog->PopupFreeMessageDlg("Ö»ÓÐÍ¬ÑùÑÕÉ«µÄÊÖïíÌ×¿ÉÒÔºÏ³É£¡");
					return false;
				}
		}
		else
...
bool C_VS_UI_MIXING_FORGE::IsCorrectOption(MItem *p_item1, MItem *p_item2)
{
	...
	if(p_item1->GetItemClass()==ITEM_CLASS_CORE_ZAP && p_item2->GetItemClass()==ITEM_CLASS_CORE_ZAP)		   //Ì××Ó²»ÅÐ¶ÏÊôÐÔ
		return true;
	if(p_item1->GetItemClass()==ITEM_CLASS_CUE_OF_ADAM && p_item2->GetItemClass()==ITEM_CLASS_CUE_OF_ADAM)		   //Ì××Ó²»ÅÐ¶ÏÊôÐÔ
		return true;
	return false;
}
...
//	case 28:
	...
	case 25:
		...
		break;
	case 26:
	    m_PetInfo.ITEM_TYPE = 26;		//ÓÄÚ¤ÀÇ
		m_pC_spk.Open(SPK_PET_INFO_YML);
		break;
	case 27:
	    m_PetInfo.ITEM_TYPE = 27;			   //XK2Ì¹¿Ë
		m_pC_spk.Open(SPK_PET_INFO_XKTK);
		break;
	case 28:
	    m_PetInfo.ITEM_TYPE = 28;				//¶ñÄ§Áú
		m_pC_spk.Open(SPK_PET_INFO_EML);
		break;
	case 29:
	    m_PetInfo.ITEM_TYPE = 29;				//»¨ÏÉ×Ó
...
void C_VS_UI_HELPDESC::HelpDescPasing()
{
	...
	char *istag = NULL;													//ÅÂÅ©°¡ ÀÖ´ÂÁö °Ë»çÇÏ´Â º¯¼ö
	char *isfont = NULL;												//ÅÂÅ©°¡ ÀÖ´ÂÁö °Ë»çÇÏ´Â º¯¼ö
...
//				jpg.Release();
		...
		else 
		{
			...
			if(linecount < count)			//  ¹®ÀÚ¿­ÀÌ count º¸´Ù ÀÛÀº°æ¿ì
			{
				...
				count -= linecount;			// count ´Â ´ÙÀ½¹®ÀÚ¿­±îÁöÀÇ °ªÀ» °¡Á®¿Í¾ß ÇÔÀ¸·Î ÇöÁ¦±îÁöÀÇ ¹®ÀÚ¿­±æÀÌ¸¦ •ûÁØ´Ù
				pCur = NULL;		
				count = limitlinecnt;
			}
			else
			...
			parsing_data.push_back(tempstr.c_str());
			NullSizey--; // ÀÌ¹ÌÁö°¡ ÀÖÀ»°æ¿ì ÇÑÁÙÇÑÁÙ push_back ¿¡ ³ÖÀ»‹š ¸¶´Ù ÇÑÁÙ¾¿ •ûÁØ´Ù.
			tempstr = "";
		}
	}
	while(NullSizey > 0)						// ÀÌ°÷Àº ÀÌ¹ÌÁöÀÇ ysize º¸´Ù ÅØ½ºÆ®°¡ ÀÛÀ»‹š
			...
			while(itr != TempInfo->vMissionList.end())
			{
				...
				if( NULL != TempMissionElement)
				{
					...
					if(NULL != pChildElement2)
					{
						...
						if(0 == stricmp(pChildElement2->GetName().c_str(),"Time")) // ½Ã°£ Á¦ÇÑÀÌ ÀÖ´Â ¹Ì¼ÇÀÌ¸é
						{
