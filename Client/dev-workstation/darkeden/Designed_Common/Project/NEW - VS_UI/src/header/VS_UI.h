...
public:
	...
	virtual void SetSize(int nWidth, int nHigh) {}
	virtual void SetWidth(int nWidth) {}
	virtual void SetHigh(int nHigh) {}

	virtual void SetFocus(bool bFoucus) {}

	virtual void Show(int nShow) {}

	virtual bool PtInRect( POINT pt) { return false; }
	virtual void SetChatMode(int nChatMode) {}
};
...
public:
	...
	void	SetWin32Edite(IWin32Edite* pEdite) { m_pC_Win32Edite = pEdite; }
	IWin32Edite*	GetWin32Edite()	{ return m_pC_Win32Edite; }
	...
	{ if(m_pC_game) m_pC_game->StartSkillCastingProgress(skillID, time, inventoryGrid); }
	bool	IsSkillCastingProgress()
	{ return m_pC_game && m_pC_game->IsSkillCastingProgress(); }
	void	EndSkillCastingProgress()
	{ if(m_pC_game) m_pC_game->EndSkillCastingProgress();		}

	// gamemenu option
	void	RunOption(bool IsTitle = false);
	void	CloseOption();
	bool	IsRunningOption(){ return (m_pC_option != NULL); }	// by larosel
	void	Background_Music_ONOFF();
	void	Effect_Music_ONOFF();
...
//server name by larosel
	void	SetServerList(LPSTR *name, int *id, int *status, int size, int default_id, bool *nonpk = NULL, 
							WORD *pwSlayerNum	= NULL, WORD *pwVampire	= NULL, WORD *pwOusters	= NULL)	{
		if(m_pC_title)m_pC_title->SetServerList(name, id, status, size, default_id, nonpk, pwSlayerNum, pwVampire, pwOusters);
#else
//server name by larosel
	void	SetServerList(LPSTR *name, int *id, int *status, int size, int default_id, bool *nonpk = NULL)	{
		if(m_pC_title)m_pC_title->SetServerList(name, id, status, size, default_id, nonpk);
...
//	bool	IsRunningSkillView(){ return m_pC_game->IsRunningSkillView(); }
	...
	int	GetQuikSlot(C_VS_UI_OUSTERS_QUICKITEM::HOTKEY hotkey) const;
	int	GetQuikSlot(C_VS_UI_SLAYER_QUICKITEM::HOTKEY hotkey) const;
	void	FinishLevelUp();
	void	LevelUp();
...
//	void	RunInfo(bool skill_mode);
	...
	void	RunOtherInfo(bool bIsMonster = false);
	...
	const char* GetNewInputString() const						{ return m_pC_game->GetNewInputString(); }
#endif //__ITEM_SHOW
...
//	void	GetSlotIndicesItemCanReplace(const MItem* pItem, std::vector<int>& slotIndices);
	...
	const MItem * GetGearOtherCoreZapItem(int slot) const; // Hays Add
...
//	int	GetSendPossibleSlot() const;
	...
	int		GetSelectedMainMenu()			{ if(m_pC_game) return m_pC_game->GetSelectedMainMenu(); return 0;}
	void	SetSelectMainnMenu(int menu)	{ if(m_pC_game) m_pC_game->SetMainMenu(menu); }
...
//	void	CloseTitleOption();
	...
	void	RequestDie(DWORD timer = 30000, bool enforce = false);
	...
	void	RunConfirmError(char *message); // 불가능
#endif //__TUNING_ITEM
	...
	void	RequestResurrect(bool f1, bool f2, bool eternity = false, bool IsSiegeAttacker = false, bool IsSkillFromOther = false);
	void	RequestResurrectSetButton(int resurrectMode, bool enable = true);
...
//	void	SetChangeablePetNickName(bool Is);
	...
	void	RunWebBrowser(HWND hWnd, char* szURL, HINSTANCE hInst) ; //  void* pWebOjbect = NULL);
	...
	void	RunAskCheckToEnterServerDialog(int AskType , int value = 0);
	...
	//20070604 by diesirace timecount interface
	void	RunTimeCount(DWORD time, LPCTSTR desc);
	void	CloseTimeCount();
	bool	IsRuningTimeCount();
	
	//20070904 by diesirace usergrade add srart
	...
	//usergrade add end
#if __CONTENTS(__GEAR_SWAP_CHANGE)	// C_VS_UI 장착창 ID를 얻을 함수 선언
	DWORD	GetGearID();
	...
	void	RunIndounNpcDlg(S_INDOUN_DLG_INFO Indoun_Npc_Dlg_Info, ENUMTYPE_INDUONSELECT etIndounSelect);
	void	CloseIndounNpcDlg();

	void	StartProgress(WORD wZoneID);
	void	SetProgressZoneID(WORD wZoneID);
	WORD	GetProgressZoneID();
#endif // __TIPOJYU_CASTLE;

#if __CONTENTS(__MOVE_ZONE)
	void	MoveZoneUi_On()		{ m_bMoveZoneUi = true; }
	void	MoveZoneUi_Off()	{ m_bMoveZoneUi = false; }
	BOOL	IsMoveZoneUi()		{ return m_bMoveZoneUi; }
#endif //__MOVE_ZONE
...
//	void HotKey_WindowToggle();
	...
	void HotKey_Gear();	
#if __CONTENTS(__GEAR_SWAP_CHANGE)	// C_VS_UI 장착창을 교체키 함수 선언
	...
	void HotKey_Sysrq();		// print screen sysrq
	...
	BOOL	m_bActProgress;
	BOOL	DidProgressFlg() { return m_bActProgress; }
	void	UnProgressFlg() { m_bActProgress = false; }
