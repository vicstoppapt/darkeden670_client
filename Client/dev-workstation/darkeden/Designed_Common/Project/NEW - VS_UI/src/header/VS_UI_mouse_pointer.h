...
struct MOVE_ZONE_INFO
{
public:
	int		m_ZoneID;
	POINT	m_ZonePoint;
	int		m_nMoveEffectCount;
	POINT	m_EffectPoint;
...
public:
	...
	void clear()
	{
		...
		m_ZonePoint.x		= 0;
		m_ZonePoint.y		= 0;
		m_nMoveEffectCount	= -1;
		m_EffectPoint.x		= 0;
		m_EffectPoint.y		= 0;
	};

	void set( int _ZoneID, LONG _ZonePoint_x, LONG _ZonePoint_y, int _nMoveEffectCount, LONG _EffectPoint_x, LONG _EffectPoint_y )
	{
		m_ZoneID			= _ZoneID;
		m_ZonePoint.x		= _ZonePoint_x;
		m_ZonePoint.y		= _ZonePoint_y;
		m_nMoveEffectCount	= _nMoveEffectCount;
		m_EffectPoint.x		= _EffectPoint_x;
		m_EffectPoint.y		= _EffectPoint_y;
	};
...
#if __CONTENTS(__MOVE_ZONE)
	bool						m_bMoveZoneEffect;
	MOVE_ZONE_INFO				m_MoveZoneInfo;
#endif //__MOVE_ZONE
...
#if __CONTENTS(__MOVE_ZONE)
	void	Process();
	void	SetMoveZoneEffect()		{ m_bMoveZoneEffect = true; }
	void	UnSetMoveZoneEffect()	{ m_bMoveZoneEffect = false; }
	bool	IsMoveZoneEffect()		{ return m_bMoveZoneEffect; }

	void	SetMoveZoneInfo( int _ZoneID, LONG _ZonePoint_x, LONG _ZonePoint_y, int _nMoveEffectCount, LONG _EffectPoint_x, LONG _EffectPoint_y )
	{
		m_MoveZoneInfo.set( _ZoneID, _ZonePoint_x, _ZonePoint_y, _nMoveEffectCount, _EffectPoint_x, _EffectPoint_y );
	};

	void	UnSetMoveZoneInfo()
	{
		m_MoveZoneInfo.clear();
	};
	
	int		GetMoveZoneEffectCount()	{ return m_MoveZoneInfo.m_nMoveEffectCount; }
#endif //__MOVE_ZONE
};
