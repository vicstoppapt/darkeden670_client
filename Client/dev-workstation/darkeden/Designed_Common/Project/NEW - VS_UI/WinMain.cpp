...
bool						gbl_ui_input_state; // UI가 입력을 받았는가?
...
//						gC_vs_ui.AddChatToHistory(buf, "다크에덴");
//					}
					hp++;
...
// 						temp_party->Name = "쭉쭉빵빵";
// 						g_pParty->AddMember(temp_party);
// 						
// 						//temp_party->bMale = false;
// 						//temp_party->hairStyle = W_FACE1;
// 						//temp_party->Name = "쭉쭉빵빵1234";
// 						//g_pParty->AddMember(temp_party);
// 
...
//						gC_vs_ui.RunTeamInfo(true, &info);	
					...
					//	gC_vs_ui.RunTeamRegist(false, 100000, 2,"2002-01-01", "쑥갓팀이다",100);
...
//						petInfo.NAME = "헤헤";//pPetItem->GetPetName();
//						petInfo.NICK_NAME = "바보 펫^^;";
...
//							warlist.reinforceGuildName = "소니  길드";
//							for(int j = 0; j<5; j++)
//							{
//								warlist.challengerGuildID[j] = rand()%1000;
//								warlist.challengerGuildName[j] = "소니  길드(공격)";
//							}
//							warlist.year = 2003;
//							warlist.month = 12;
//							warlist.day = i;
...
//					gC_vs_ui.RunTe
//					gC_vs_ui.SetWhisperID("쑥갓");
		//			gC_vs_ui.AddChatToHistory("보통 채트지요..홍홍..", "운영자0", CLD_ZONECHAT);
//					gC_vs_ui.OpenInventoryToRepair();
//					gC_vs_ui.RequestDie();
//					gC_vs_ui.SetAccelMode(ACCEL_CHARINFO);
//					gC_vs_ui.RequestParty("쑥갓");
...
//					gC_vs_ui.UnselectSkill();
					...
					//gC_vs_ui.LevelUp();
//					gC_vs_ui.AddChatToHistory("보통 채트지요..홍홍..", "다크에덴");
//					gC_vs_ui.AddChatToHistory("마지막 4번 돋움입니다.. 벌써 마지막이냐구요? 윈도 기본폰트가 이거 바께 엄네요ㅡ.ㅡ 우워~ 돋아나라-_-a", "치아푸푸", CLD_ZONECHAT);
//					gC_vs_ui.RunTutorialExit();
					break;

				case DIK_F2:
					{
						//gC_vs_ui.AddInputString("테스트2");
						gC_vs_ui.AddChatToHistory( "테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임테스트임", "아이디임", CLD_INFO, RGB(255, 255, 255) );

						//gC_vs_ui.Run_Campaign_Help_Unfortunate_Neighbors(5000); 
						//gC_vs_ui.RunModifyTax() ;
						//	gC_vs_ui.RunDelegation_Of_Power("영채바보") ; 

						//gC_vs_ui.RunTaxRate(-20)  ; 
						//gC_vs_ui.RunPartyCancel("쑥갓");
					...
					//	gC_vs_ui.RunSlayerPortal(2); 
						...
						//char  strMsg[256] = "assadfsafㅁ안ㅁ아항남하" ; 

						//SendMessage(g_hWnd , WM_COPYDATA ,  (LPARAM)(LPCTSTR)strMsg , 0 ) ;

					//gC_vs_ui.RunExchangeCancel("바보");
...
//					}
					//gC_vs_ui.RunNamingChange(NULL, "우아~");
					//g_hInst = hInst ; 
					//gC_vs_ui.RunStorage();
					}
...
//						}
//				 	}					
////					gC_vs_ui.AddChatToHistory("보통 채트지요..홍홍..", "운영자0", CLD_NORMAL, RGB(0, 0, 255));
					//gC_vs_ui.HotKey_F2();
//					gC_vs_ui.RunPartyAsk("쑥갓", (C_VS_UI_REQUEST_PARTY::REQUEST_PARTY)0);
//					gC_vs_ui.RunHealSkillTree();
//					gC_vs_ui.AddChatToHistory("길드채팅이지요..호홋..", "다크에덴", CLD_GUILD);

//					gC_vs_ui.AddChatToHistory("마지막 4번 돋움입니다.. 벌써 마지막이냐구요? 윈도 기본폰트가 이거 바께 엄네요ㅡ.ㅡ 우워~ 돋아나라-_-a", "치아푸푸", CLD_NORMAL);
					//gC_vs_ui.RunBookcase();
//					gC_vs_ui.RunSlayerPortal();
//					gC_vs_ui.RunPartyCancel("쑥갓");
...
//					gC_vs_ui.RunBladeSkillTree();
					//gC_vs_ui.AddChatToHistory("\"귓속말을 전달할 수 없습니다\"~~ -> info", "다크에덴", CLD_INFO);
//					gC_vs_ui.AddChatToHistory("파티채팅이지요..호홋..", "다크에덴", CLD_PARTY);
//					gC_vs_ui.RunExchangeAsk("시기");
					//gC_vs_ui.RunBriefing();
//					gC_vs_ui.RunElevator();
//					gC_vs_ui.RunExchangeCancel("시기");
...
//				gC_vs_ui.AddChatToHistory("외치기지요..호홋..", "다크에덴", CLD_ZONECHAT);
//					gC_vs_ui.RunSwordSkillTree();
					//gC_vs_ui.HotKey_F4();
//					gC_vs_ui.RunVampireSkillTree();
				//	gC_vs_ui.RunComputer();
//					gC_vs_ui.AddChatToHistory("귓속말 길드지요..호홋..", "다크에덴", CLD_WHISPER);
					//					gC_vs_ui.ServerDisconnectMessage();
					//					gC_vs_ui.StartTitle();
					//					gC_vs_ui.RequestParty("쑥쑥");
					//					gC_vs_ui.OpenInventoryToSell();
					//gC_vs_ui.RequestParty("쑥쑥");
					{
				...
				case DIK_F5:
					{
						...
						//gC_vs_ui.OpenInventoryToRepair();
						//					gC_vs_ui.AddChatToHistory("귓말 일반이지요..호홋..", "다크에덴", CLD_WHISPER);
						//	PARTY_INFO *temp_party = new PARTY_INFO;
						//	temp_party->bMale = false;
						//	temp_party->hairStyle = W_FACE1;
						//	temp_party->Name = "쭉쭉빵빵";
						...
						//					gC_vs_ui.RequestParty("쑥쑥", C_VS_UI_REQUEST_PARTY::REQUEST);
						//gC_vs_ui.OpenInventoryToSell();
//						{
//							DWORD id = 101;
//							gC_vs_ui.RunImageNotice();
//							gC_vs_ui.AddNotice( "손희승", id );
//							int step = 1;
////							gC_vs_ui.RunLotteryCard(step);
//						}
					...
					gC_vs_ui.OpenInventoryToSilvering();
					{
						...
						else
						{
							*/
							/*
							static num = 431;
							m_pC_dialog = new C_VS_UI_NPC_DIALOG(kk, num, "쑥갓");
						
							DIALOG_MENU d_menu[] = {
								{"팔기니까니까니까니까라니까니까니까니까니까라니?", 0},
								{"팔기니까니까라니까니까니까니까니까라니?", 1},
								{"팔기까니까니까라니까니까니까니까니까라니?", 2},
								{"끝내기", DIALOG_EXECID_EXIT},
								};
							m_pC_dialog->SetMenu(d_menu, 4, false);
							
							static char * pp_dmsg[] = { // Message는 반드시 static or global로 해야 한다.
								"아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑",
								"아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑",
								"아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑",
								"아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑",
								"아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑아무거나 골라 봅숑",
...
//					gC_vs_ui.ChangeToVampireInterface();
				...
				case DIK_F10: // 마우스 정지현상 일어남.
					gC_vs_ui.RunOtherInfo() ;  
					//gC_vs_ui.OpenInventoryToRepair();
					
					//
					// F10만 이상하게 virtual key로 받을 수 없다.. 음...
					// 그래서 이것만 dinput으로 받는다.
					//
					//gC_vs_ui.HotKey_F10();
//					gC_vs_ui.RunStorage();
					break;
...
void MouseEventReceiver(CDirectInput::E_MOUSE_EVENT event, int x, int y, int z)
{
	...
	// Window mode debug 할 때는 DI쓰면 pointer가 Window 밖으로 나가므로 이때는
	// WindowProc()으로 넘어오는 좌표를 사용한다.
#ifndef _FULLSCREEN
	POINT point;
	GetCursorPos(&point);
...
////////////////////////////////////////////////////////////////////
// 랜즈 효과(표면 flip 이전에 출력한다)...........
// 실행: Lenz( 200,200, 50 ); // 위치, 반지름...(화면:640*480)...
...
// 간단히 1번만 실행하도록 하였음...(알고리즘 자체는 복합 파동이 가능한 거구염-좀만 수정하면...)
// 가로,세로(640,480)...
// 물방울의 시작,밀도, 높이 각...
void Water( int xx, int yy, char Density, char Radius )    
{
    ...
    static int Height[2][307200], hkind = 0;    // 물방울의 높이 값을 저장한 메	모리...
	static WORD WaveScr[307200];
    static WORD Color2; // 표면의 픽셀은 WORD 형...
    ...
    if( bStarted )    {
		...
		for( cnt = 1; cnt<306560; cnt++) {
            ...
            addr1 = OPos/640; // y 위치 찾음...
            addr2 = OPos - addr1*640; // x 위치 찾음...
        ...
        // 상호 상쇠, 충돌 작용까정 처리(Smoothing 2개...)
		// Smooth를 2개의 메모리로 반드시 써야함...(그러지 않으면, 전체 이미지 영역으로		번짐...)
			switch( hkind )    {
        case 0:
...
void ProgramLoop()
{
//	if (gC_vs_ui.UpdateState()) //!쉿 마우스포인터는~
	if(gbl_game_back)
	{
	...
	gC_vs_ui.Show();
	gC_vs_ui.DrawMousePointer(); // Client의 요청으로 밖으로 빼었다.

//	gC_vs_ui.SetZoneName("에슬라니아 NW");
...
//			
//			int arms = dw_left;	// 0: 도 1: 검 2: 메이스 3: 십자가 4: AR 5: TR 6: SMG 7:SG
//		}
//		break;
...
//			break;
		...
		//
		// Game Menu가 실행되었다. 다른 모든 창을 닫아야 한다.
		//
		...
		case UI_INFO:
			...
			else
			{
				...
				g_char_slot_ingame.sz_guild_name = "";//드림마스터라니까";
				...
				//g_char_slot_ingame.STATUS.push_back(efs);
				//S_SLOT::UI_EFFECTSTATUS_STRUCT efs;
				efs.actionInfo = SKILL_CLIENT_MAGICAL_PET_CHANGER;
				efs.delay100msec = 100000;
...
/*
		...
		case UI_PLEASE_PCS_CONNECT_ME: // 대기방 click! (이미 연결-duplex-되지 않았음)
			//
		...
		//	break;
*/
		case UI_CHAT_RETURN: // 채팅창 입력 // by larosel

//			if (dw_right)
//			{
//				gC_vs_ui.AddChatToHistory((char *)void_ptr, "다크에덴", CLD_NORMAL, true);	// 길드채팅
//			}
//			else
			{
...
//			gC_vs_ui.RunNewUserRegistration();
		...
		case UI_GAMEMENU_CONTINUE:
			...
			break;
/*
		case UI_END_PCS: // PCS 창에서 PCS number를 send하고 connecting 중에 END를 눌렀다.
			break;
		...
		case UI_SEND_PCS_NUMBER:
			...
			//
			// 연결하기를 원하는 pcs number의 정보를 설정해줘야 한다.
			//
			gC_vs_ui.OnLinePCS(NULL, dw_left); // 연결성공

			//gC_vs_ui.ConnectionFailed(); // 연결실패.
			break;

		case UI_PDS_CLOSED:
			//
			// PDS가 닫혔다. PCS 연결은 끊을필요가 없겠지..
			//
...
//			break;
		...
		case UI_GO_BILING_PAGE :
			...
			{								
				...
				_spawnl(_P_NOWAIT, str, "Explorer.exe", "http://www.darkeden.co.kr", NULL);
			}
			break;
...
//
//				{
//					if (g_pOustersGear->ReplaceItem(pMouseItem,		// 추가할 item
//						dw_left,	// 추가할 위치 
//						pOldItem))		// 원래있던 item
//					{
...
//				}
				...
				{
				case RACE_SLAYER:
					if (g_pSlayerGear->ReplaceItem(pMouseItem,		// 추가할 item
						dw_left,	// 추가할 위치 
						pOldItem))		// 원래있던 item
					{
						if (pOldItem != NULL) // replace 되었는가?
				...
				case RACE_VAMPIRE:
					if (g_pVampireGear->ReplaceItem(pMouseItem,		// 추가할 item
						dw_left,	// 추가할 위치 
						pOldItem))		// 원래있던 item
					{
						if (pOldItem != NULL) // replace 되었는가?
		...
		case UI_ITEM_DROP_TO_QUICKSLOT:
			{
				...
				if (g_pQuickSlot->ReplaceItem(pMouseItem,		// 추가할 item
					dw_left, pOldItem))		// 원래있던 item
				{
					if (pOldItem != NULL) // replace 되었는가?
					{				
						gC_vs_ui.PickUpItem( pOldItem );
		...
		case UI_ITEM_PICKUP_FROM_GEAR:
			...
			{
			case RACE_SLAYER:
				g_pSlayerGear->RemoveItem((MSlayerGear::GEAR_SLAYER)dw_left);		// 원래있던 item
				break;
		...
		case UI_CONNECT_SERVER:
			...
			slot.man_info.helmet = M_HELMET2;
			slot.man_info.coat = M_COAT2;
			slot.man_info.trouser = M_TROUSER2;
			slot.man_info.face = M_FACE2;
			slot.man_info.hair = M_HAIR1;
			slot.man_info.left = M_SHIELD2;//M_TR;//M_SWORD;
			slot.man_info.right = M_MACE;//W_SWORD;//W_DRAGON_SHIELD;
			slot.skin_color = 0;
			slot.hair_color = 0;
			slot.left_color = 405;
...
*/
		...
		case UI_CLOSE_INVENTORY_SUB:
			gC_vs_ui.CloseSubInventory(dw_left);
			break;
	}
}
...
// for Memory Leak Detecting
	...
	iovfs_base::start_vfs("data/dklegend", O_RDONLY);
...
//		g_pClientConfig->Use3DHAL			= TRUE;
	...
	g_pUserOption->DrawMinimap			= FALSE;	// minimap을 그릴까?
	g_pUserOption->DrawZoneName			= TRUE;		// Zone이름 출력
	g_pUserOption->DrawGameTime			= TRUE;		// 게임 시간 출력
	g_pUserOption->DrawInterface		= FALSE;	// interface
	g_pUserOption->DrawFPS				= FALSE;	// FPS
	g_pUserOption->BlendingShadow		= FALSE;	//TRUE;			// 그림자 반투명
	g_pUserOption->FilteringCurse		= TRUE;		// 나쁜 말 제거
	g_pUserOption->PlayMusic			= TRUE;		// 음악 출력
	g_pUserOption->PlaySound			= TRUE;		// 효과음 출력
	g_pUserOption->VolumeMusic			= 10;		// 음악 소리 크기
	g_pUserOption->VolumeSound			= 10;		// 효과음 소리 크기
...
//	}
	...
	//
	// `윈도우는 항상 하나의 윈도우 클래스에 기반하여 만들어지며, 윈도우 클래스는
	//  그 윈도우에 대한 메시지를 처리할 윈도우 프로시저를 지정한다.
	//
	// `하나의 윈도우 클래스에서 하나 이상의 윈도우가 만들어질 수 있다.
	//
	g_hInst = hInst ; 
	wcl.hInstance = hInst;
	...
	//  수평윈도우 크기나 수직윈도우 크기가 변할 때마다 다시 그려져야 한다는 것을
	//  가리킨다. 따라스 윈도우의 크기를 변경하면 WM_PAINT 메시지를 보낸다.
	//
	wcl.style =	CS_HREDRAW;// | CS_VREDRAW;
