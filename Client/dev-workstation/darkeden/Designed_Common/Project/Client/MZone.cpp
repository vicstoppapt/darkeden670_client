...
// 800x600 해상도라도 시야 섹터는 1024x768과 동일하다.
// 서버에서는 해상도의 구분이 없기 때문
...
#define	CheckCreatureInGunFiled( sector, x, y )						\
{															\
	if (g_pPlayer->GetX()==x && g_pPlayer->GetY()==y)		\
{														\
	g_pPlayer->CheckInGunFiled();						\
}														\
	\
	int numCreature = sector.GetCreatureSize();				\
	\
...
MZone::AddCorpseFromCreature(TYPE_OBJECTID id)
{
	...
	// 채팅을 없애준다.
	//----------------------------------------
#if __CONTENTS(__TIPOJYU_CASTLE)			//채팅 설정
		...
		if (bAdd)
		{
			...
			if ( pCreature->IsPlayer() && pCreature->IsSlayer())
			{
				...
				if ( info.bAddon )
				{
					...
					if ( strSTDName.find("뒈掉칡辜났") != -1 )
						((MCreatureWear*)pCreature)->SetHellBikeEffect( 0 );
				}
			}
	...
	if (theIterator == m_mapItem.end())
	{
		...
		{		
			...
			if(pItem->GetItemClass() == ITEM_CLASS_WAR_ITEM)
			{
				...
				{
					ExecuteActionInfoFromMainNode(
						(SKILL_CLIENT_SLAYER_HERO + ((warItemIndex-1)%3)),		// 값 자체가 RESULT_ACTIONINFO이다.
...
//					// 2004, 10, 19, sobeit add end
					...
					{					
						...
						else if (	est1>=EFFECTSPRITETYPE_unknow_2552
							&& est1<=EFFECTSPRITETYPE_unknow_2566
							&& est2>=EFFECTSPRITETYPE_unknow_2552
							&& est2<=EFFECTSPRITETYPE_unknow_2566
							&& (est1>=EFFECTSPRITETYPE_unknow_2557
							&& est1<=EFFECTSPRITETYPE_unknow_2561
							&& est2>=EFFECTSPRITETYPE_unknow_2557
							&& est2<=EFFECTSPRITETYPE_unknow_2561
							|| est1>=EFFECTSPRITETYPE_unknow_2552
							&& est1<=EFFECTSPRITETYPE_unknow_2556
							&& est2>=EFFECTSPRITETYPE_unknow_2552
							&& est2<=EFFECTSPRITETYPE_unknow_2556
							|| est1>=EFFECTSPRITETYPE_unknow_2562
							&& est1<=EFFECTSPRITETYPE_unknow_2566
							&& est2>=EFFECTSPRITETYPE_unknow_2562
							&& est2<=EFFECTSPRITETYPE_unknow_2566) 
							)
						{
							if (pNewEffect->GetEndFrame() > pEffect->GetEndFrame()) 
							...
							if (est1>=EFFECTSPRITETYPE_unknow_2557 &&
								est1<=EFFECTSPRITETYPE_unknow_2561)
							{
								sector.SetGunFiled();
							}
							...
							CheckCreatureInGunFiled( sector, x, y );
...
MZone::CanAddEffect(MEffect* pNewEffect)
{
	...
	bool bGunFiled	=	frameID >= (*g_pEffectSpriteTypeTable)[EFFECTSPRITETYPE_unknow_2552].FrameID		&&
		frameID <= (*g_pEffectSpriteTypeTable)[EFFECTSPRITETYPE_unknow_2566].FrameID;
	...
	// 어떤 이팩트가 걸려있는 경우 그 위에 걸리지 않는 이팩트가 있다.
	if( bDarkNess || bAcidSwamp || bProminence || bBloodWall || bHellFire || bFuryOfGnome || bGunFiled)
	{
		bool bCantUseMercyGround				= bDarkNess || bAcidSwamp || bProminence || bGunFiled;
		...
		bool bCantUseRegenTower					= bDarkNess || bAcidSwamp || bProminence || bGunFiled;
		...
		bool bCantUseGLORYGROUND				= bDarkNess || bAcidSwamp || bProminence || bBloodWall || bHellFire || bFuryOfGnome;
		TYPE_FRAMEID frameIDGLORYGROUNDBegin	= (*g_pEffectSpriteTypeTable)[EFFECTSPRITETYPE_unknow_2235].FrameID;
		TYPE_FRAMEID frameIDGLORYGROUNDEnd		= (*g_pEffectSpriteTypeTable)[EFFECTSPRITETYPE_unknow_2236].FrameID;

		bool bCantUseSummonClay					= bDarkNess || bAcidSwamp || bBloodWall || bGunFiled;
		...
		for(;iGroundEffect != m_mapGroundEffect.end(); ++iGroundEffect)
		{
			...
			else if(bCantUseGLORYGROUND							&&
					SearchedFrameID >= frameIDGLORYGROUNDBegin		&&
					SearchedFrameID <= frameIDGLORYGROUNDEnd)
			{
				if(x <= effectX + 2 && x >= effectX - 2 &&
				   y <= effectY + 2 && y >= effectY - 2)
				{
...
MZone::RemoveTileEffect(TYPE_SECTORPOSITION sX, TYPE_SECTORPOSITION sY, int effectStatus, int serverID)
{
	...
	switch (effectStatus)
	{
		...
		case EFFECTSTATUS_DRAGON_HURRICANE:
		case EFFECTSTATUS_DRAGON_HURRICANE_CHILD:
		case EFFECTSTATUS_unknow231_22:
		case EFFECTSTATUS_unknow231_23:
			{
...
MZone::RemoveTileEffectByActionInfo(TYPE_SECTORPOSITION sX, TYPE_SECTORPOSITION sY, TYPE_ACTIONINFO actionInfoIdx, int serverID)
{
	...
	for(int idx = 0; idx < size; ++idx) {
		...
		// Effect Generator마다 지우는 방법이 다르다.
		// 지우고 싶은 Effect의 Effect Generator가 없다면
		// 그때 그때 추가한다 -ㅅ-
...
//----------------------------------------------------------------------
// tile에 있는 Mercy Ground effect를 지운다.
//----------------------------------------------------------------------
bool		
MZone::RemoveMercyGroundTileEffect(TYPE_SECTORPOSITION sLeft, TYPE_SECTORPOSITION sTop, TYPE_SECTORPOSITION sRight, TYPE_SECTORPOSITION sBottom)
{
	...
	// 일단 중간타일을 검색해서 가장자리 타일들을 지운다.
	// 중간타일은 검색에 필요하기 때문에 일단 지우지 않는다.
	for(int iy = sTop; iy <= sBottom; ++iy)
		for(int ix = sLeft; ix <= sRight; ++ix)
		{
			...
			for(int i = 0; i < 9; ++i)
			{
				if(i == 4) continue;	// 중간 타일은 나중에 지운다.
...
//								frameID <= EFFECTSPRITETYPE_MAP_BLACK_SMALL_SMOKE_3
		...
		bool isRemoveGunFiledEffect = (frameID>=EFFECTSPRITETYPE_unknow_2552 && frameID<=EFFECTSPRITETYPE_unknow_2566);
...
//											pEffect->GetFrameID() >= EFFECTSPRITETYPE_MAP_BLACK_SMALL_SMOKE_3;
				...
				bool isExistGunFiledEffect = (pEffect->GetFrameID()>=EFFECTSPRITETYPE_unknow_2552 && pEffect->GetFrameID()<=EFFECTSPRITETYPE_unknow_2566);
				//--------------------------------------------------
				// 같은 effect type인 경우
				//--------------------------------------------------
				if (pEffect->GetFrameID() == frameID
				...
				if (pEffect->GetFrameID() == frameID
					|| isRemoveGunFiledEffect
					&& isExistGunFiledEffect					
					)
				{
					RemoveSectorEffect( sX, sY, pEffect->GetID() );
					...
					// 다크니스인 경우 그 타일에 있던 캐릭터를 보여준다.
					if (isRemoveGunFiledEffect)
					{
						CheckCreatureInGunFiled( sector, sX, sY );
					}
...
//----------------------------------------------------------------------
// (sX,sY) 에서 근접한 숫자 순서대로 Tile과 ImageObject의 SpriteSet을 검색해서 넘겨준다.
//----------------------------------------------------------------------
void						
