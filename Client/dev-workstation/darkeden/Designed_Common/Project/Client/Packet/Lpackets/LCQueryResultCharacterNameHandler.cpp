	 ...
	 throw ( ProtocolException , Error )
{
		...
		&& g_pUserInformation->WhisperID==pPacket->getCharacterName().c_str())
	{
		if (pPacket->isExist())
		{
			g_pUIDialog->PopupFreeMessageDlg( (*g_pGameStringTable)[STRING_ERROR_ALREADY_REGISTER_ID].GetString(), -1, -1, UI_DIALOG_TITLE_OK );
		}
		else
		{
			g_pUIDialog->PopupFreeMessageDlg( (*g_pGameStringTable)[STRING_MESSAGE_CAN_REGISTER_NAME].GetString(),-1, -1, UI_DIALOG_TITLE_OK );
		}
