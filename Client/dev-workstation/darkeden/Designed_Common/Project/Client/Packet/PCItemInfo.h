...
public:
	...
	///////////////////////////맣芚溝固///////////////////////////////////
	void addGTwoOptionType(OptionType_t OptionType) throw() { m_GTwoOptionType.push_back( OptionType ); }
	void setGTwoOptionType(const std::list<OptionType_t>& OptionType) throw() { m_GTwoOptionType = OptionType; }
	int getGTwoOptionTypeSize() const throw() { return m_GTwoOptionType.size(); }
	const std::list<OptionType_t>& getGTwoOptionType() const throw() { return m_GTwoOptionType; }
	OptionType_t popGTwoOptionType() throw() 
	{ 
		if (m_GTwoOptionType.empty()) return 0;
		OptionType_t optionType = m_GTwoOptionType.front(); 
		m_GTwoOptionType.pop_front(); 
		return optionType; 
	}
...
public:
	uint getSize() const throw()
	{
		return szObjectID +
			szBYTE +
			szItemType + 
			szBYTE + m_OptionType.size() + m_GTwoOptionType.size() +
			szDurability +
			szSilver + 
			szGrade +
			szEnchantLevel +
...
protected:
	...
	std::list<OptionType_t> m_GTwoOptionType;       // 맣芚
