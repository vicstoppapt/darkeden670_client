...
{
#if __CONTENTS(__TIPOJYU_CASTLE)	//던전 정보 초기화
	...
	throw ()
{
	...
	SAFE_DELETE(m_pInstanceDoungeonInfo);	//던전 정보 메모리 삭제
#endif // __TIPOJYU_CASTLE
...
#if __CONTENTS(__TIPOJYU_CASTLE) // GCUpdateInfoFactory 
	SAFE_DELETE(m_pInstanceDoungeonInfo);
#endif // __TIPOJYU_CASTLE
...
#if __CONTENTS(__TIPOJYU_CASTLE) // GCUpdateInfoFactory 
//	m_pInstanceDoungeonInfo	= new InstanceDoungeonInfo();
//	m_pInstanceDoungeonInfo->read(iStream);
#endif // __TIPOJYU_CASTLE
	__END_CATCH
}
...
#if __CONTENTS(__TIPOJYU_CASTLE) // GCUpdateInfoFactory	패킷 파일 쓰기		서버에서 아직 준비 안됨
