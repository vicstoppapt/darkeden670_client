	...
	throw ( Error )
{
	...
	__BEGIN_DEBUG
		...
		pPacketIDSet->addPacketID( Packet::PACKET_LC_SHOW_WEB_PAGE );
#if __CONTENTS(__LC_SELECT_WORLD_ERROR)
		pPacketIDSet->addPacketID( Packet::PACKET_LC_SELECT_WORLD_ERROR );
		...
		pPacketIDSet->addPacketID( Packet::PACKET_LC_SERVER_LIST );
		pPacketIDSet->addPacketID( Packet::PACKET_LC_QUERY_RESULT_CHARACTER_NAME );
//20090105 PACKET_LC_WORLD_LIST가 CPS_AFTER_SENDING_CL_GET_PC_LIST일때도 들어올수 있다. 로그아웃 하고 월드를 변경하러 가는 경우에 그렇수 있으며
//Tilte UI변경 이후에는 채널 변경을 위해 가더라도 PACKET_LC_WORLD_LIST받게 되었다.
		...
		pPacketIDSet->addPacketID( Packet::PACKET_GC_UPDATE_WEEKITEM_LIST );
#endif //__PREMIUM_GIVE_ITEM_UI
		pPacketIDSet->addPacketID( Packet::PACKET_GC_ADD_EFFECT_TO_SCREEN);
		pPacketIDSet->addPacketID( Packet::PACKET_GC_SYSTEM_MESSAGE);
#if __CONTENTS(__FRIEND_ADDITION)
		pPacketIDSet->addPacketID( Packet::PACKET_GC_FRIEND_LIST );
		pPacketIDSet->addPacketID( Packet::PACKET_GC_FRIEND_WAIT_LIST );
		pPacketIDSet->addPacketID( Packet::PACKET_GC_FRIEND_UPDATE_CONNECT );
		pPacketIDSet->addPacketID( Packet::PACKET_GC_FRIEND_UPDATE_ZONE );
