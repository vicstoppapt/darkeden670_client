...
class CInfomationBord 
{
public:
	CInfomationBord();
	~CInfomationBord();

	void Parse(const char* szString);
	RECT SetTextPos(int x, int  y);
	void ShowBord(PrintInfo * p_print_info = NULL);

protected:
	std::string  m_strShowScore;
	std::string  m_strV[3];
	std::string  m_strS[3];
	std::string  m_strO[3];
};
...
class CMessageArray {
	public :
		...
		void		BeginUpdateTimer(int nDelay);
		void		Update();
		//--------------------------------------------------
		// Add / Get
		//--------------------------------------------------
		...
		void		AddFormatVL(const char* format);
