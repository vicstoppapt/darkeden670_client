...
BYTE GetCreatureActionCountMax( const MCreature* pCreature, int action )
{
	...
	bool bWerWolf = pCreature->GetCreatureType() == CREATURETYPE_WER_WOLF; //±äÀÇ »ñÈ¡¶¯×÷±í
	...
	//bool bInstallMK1 = pCreature->GetCreatureType() == CREATURETYPE_INSTALL_MK1;
	bool bInstallMK1 = pCreature->HasEffectStatus(EFFECTSTATUS_INSTALL_MK1);
	// 2005, 1, 7, sobeit add end
	//if( !pCreature->IsAdvancementClass() || g_pTopView == NULL || bBat || bWolf || bWerWolf || bInstallTurret)
	if( !pCreature->IsAdvancementClass() || g_pTopView == NULL || bBat || bWolf || bWerWolf || bInstallTurret|| bInstallMK1 || bGhost
#if __CONTENTS(__FAST_TRANSFORTER)
		|| bFlitterMous
		|| bNede
		|| bKltl
...
bool			IsCreatureActionAttack( MCreature *pCreature )
{
	...
	if( pCreature->IsSlayer() )
	{
		//20090122 ½ÂÁ÷ÀÏ °æ¿ì ¾×¼Ç °ªÁß¿¡ ¸ÅÁ÷ ¾îÅÃÀÌ ÀÎ½ÄÀÌ ¾ÈµÇ ´ÙÀ½°ú °°ÀÌ ÇØÁÖ¾î¾ß ÇÔ
		//±Ã±Ø ÀûÀ¸·Ð action°ªÀÌ Á¤»óÀûÀ¸·ÎÄÁ¹öÁ¯ µÇ¾ß ÇÏ°Å³ª ÄÁ¹öÁ¯ µÇÁö ¾Ê´Â°Ô ¸Â´Ù¸é ÀÌ°Ô ¸ÂÀ»Áöµµ...
...
{
	pCreature->HasEffectStatus( EFFECTSTATUS_OusterWings_New_1 );
}
bool HasEffectStatusOusterNewWings2( MCreature* pCreature )
{
	if( pCreature->HasEffectStatus( EFFECTSTATUS_OusterWings_New_2 ))
		return true;
	return false;
}
...
{
	pCreature->HasEffectStatus( EFFECTSTATUS_OusterWings_New_2 );
}
#endif //__SECOND_TRANSFORTER
...
MCreature::MCreature()
{
	...
	m_ColorPersonalShop	= s_PersnalShopColor[0];	// µðÆúÆ® °ªÀ¸·Î ³×Ãò·² Ä®¶ó¸¦ ³Ö´Â´Ù.
...
//	m_HalluName = 0;
	...
	m_bOusterGhost = false;
	m_OusterGhostCount =0;

	m_GunFiledCount = -1;
	m_GunFiledCountInc = 0;
}
...
bool IsCreatureTypeVampirePC(int type)
{
	return type == CREATURETYPE_VAMPIRE_MALE1 ||
		...
		type == CREATURETYPE_WER_WOLF;//ÅÐ¶ÏÊÇ²»ÊÇÎüÑª¹íÈËÎï
}
...
//	else
	...
	if( m_CreatureType != CREATURETYPE_WER_WOLF )
		m_bMale = (*g_pCreatureTable)[m_CreatureType].bMale;//±äÀÇÐÔ±ð
	...
	if (HasEffectStatus(EFFECTSTATUS_TRANSFORM_TO_WERWOLF)
		&& m_CreatureType != CREATURETYPE_WER_WOLF)//²»±äÀÇÊ± ÒÆ³ýÐ§¹û
	{
	...
	if (HasEffectStatus(EFFECTSTATUS_INSTALL_MK1)
		&& m_CreatureType != CREATURETYPE_INSTALL_MK1)
	{
...
//			break;
	...
	//		break;
		...
		case EFFECTSTATUS_HYDRO_CONVERGENCE:
			...
			// È­¸éÀ» ÇÏ¾á»öÀ¸·Î ºí¶óÀÎµåÇÏ´Â ÀÌÆÑÆ® ÇØÁ¦
		...
		case EFFECTSTATUS_FREEZE:
			...
			break;
		case EFFECTSTATUS_unknow231_21:
		case EFFECTSTATUS_994:
			if (m_bAlive)
			{
				SetStop();			
			}			
...
//					false);			// ±â¼ú Ã·ºÎÅÍ ½ÃÀÛÇÑ´Ù.
			...
			//Add By Hays 2013  3  10  MK1²âÊÔ
	/*	case EFFECTSTATUS_INSTALL_MK1: // ÃÑ½½ 130 ÀÎ½ºÅç ÅÍ·¿
             SetMoveDevice(MOVE_DEVICE_WALK);
			delayFrame = 0xFFFF;
			break;*/
		...
		case EFFECTSTATUS_HYDRO_CONVERGENCE_BOMB:
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_HYDRO_CONVERGENCE_BOMB, GetX(), GetY(), 0, GetDirection(), GetID(),
								GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
		case EFFECTSTATUS_BLIND:	// È­¸éÀ» ÇÏ¾á»öÀ¸·Î ºí¶óÀÎµåÇÏ´Â ÀÌÆÑÆ® ¼³Á¤
			if(g_pTopView != NULL && GetID() == g_pPlayer->GetID())
			{
		...
		case EFFECTSTATUS_unknow231_21:
			...
			AffectMoveBufferAll();
			ActionMoveNextPosition();
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_ATTRACT_SWORD_CRASH_PC_HOLD,GetX(), GetY(), 0,GetDirection(),	GetID(),	
				GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
		case EFFECTSTATUS_994:
			AffectMoveBufferAll();
			ActionMoveNextPosition();
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_ATTRACT_SWORD_CRASH_2_PC_HOLD,GetX(), GetY(), 0,GetDirection(),	GetID(),	
				GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
		case EFFECTSTATUS_997:
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_RAGE_BURST_HIT,GetX(), GetY(), 0,GetDirection(),	GetID(),	
			GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
		case EFFECTSTATUS_1002:
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_RAGE_BURST_2_HIT,GetX(), GetY(), 0,GetDirection(),	GetID(),	
				GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
		case EFFECTSTATUS_998:
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_FROZEN_GROUND_HIT,GetX(), GetY(), 0,GetDirection(),	GetID(),	
				GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
		case EFFECTSTATUS_1003:
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_FROZEN_GROUND_2_HIT,GetX(), GetY(), 0,GetDirection(),	GetID(),	
				GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
		case EFFECTSTATUS_995:
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_EXTENSIVE_DELUSION_CURSE,GetX(), GetY(), 0,GetDirection(),	GetID(),	
				GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
		case EFFECTSTATUS_1001:
			ExecuteActionInfoFromMainNode(SKILL_CLIENT_EXTENSIVE_DELUSION_CURSE_2,GetX(), GetY(), 0,GetDirection(),	GetID(),	
				GetX(), GetY(), 0, delayFrame, NULL, false);
			break;
	}
...
//		SetAction(ACTION_STAND);
	...
	else if( IsOusters() )
	{
		...
		if(m_MoveDevice==MOVE_DEVICE_WING_SYLPH)
		{
			...
			if(GetWingItemType()==2)
				m_MoveAction = ACTION_OUSTERS_UNICORN_MOVE;
			if(GetWingItemType()==3)
				m_MoveAction = ACTION_OUSTERS_UNICORN_MOVE;
#endif //__SECOND_TRANSFORTER
		}
		else
...
{
	//-----------------------------------------------------------
	// ¹ÚÁã³ª ´Á´ë , °í½ºÆ® ¸é °ø°Ý¼Óµµ µû·Î ¾ø´ç..
	//-----------------------------------------------------------
	if (m_CreatureType==CREATURETYPE_BAT ||
		m_CreatureType==CREATURETYPE_VAMPIRE_GHOST ||
		m_CreatureType==CREATURETYPE_WOLF ||
		m_CreatureType==CREATURETYPE_WER_WOLF//±äÀÇºóµÄ¹¥»÷ËÙ¶È
#if __CONTENTS(__FAST_TRANSFORTER)
		|| m_CreatureType==CREATURETYPE_FLITTERMOUSE
		|| m_CreatureType==CREATURETYPE_NEDE
		|| m_CreatureType==CREATURETYPE_KLTL
...
{ 
	...
	if( IsOusters() && HasEffectStatusOusterNewWings1( this ))
	{
		if (action==ACTION_STAND)
		{
			action = ACTION_OUSTERS_UNICORN_STAND;
	...
	if( IsOusters() && HasEffectStatusOusterNewWings2( this ))
	{
		if (action==ACTION_STAND)
		{
			action = ACTION_OUSTERS_UNICORN_STAND;
...
//			delayFrame = 32;
	// 2004, 08, 05 sobeit add end - ¼º¹®, ±æµå Å¸¿ö ÆÄÆíÆ¢±â
		...
		switch( type )
		{
		// Vampire Renewal ±âÁ¸ ±¸Á¶ µû¶ó°¡±â ¤Ð_¤Ð by chyaya
		...
		case EFFECTSPRITETYPE_RENEWAL_TALON_OF_CROW_MALE_ACTION_1_NORMAL :
			...
			// ¿©±â¼­ break ÇÏ¸é ¾ÈµÊ
			// ¹Ø¿¡ WeaponSpeed¿¡ µû¸¥ ±¸¹®À» Ã³¸®ÇØÁà¾ß Á¦´ë·Î ³ª¿Â´Ù.
...
//		case EFFECTSPRITETYPE_THROW_CHACRAM_25_MASTERY:
		...
		case EFFECTSPRITETYPE_SUMMON_WATER_ELEMENTAL_HEAL:
			orbitType = 2;
			break;
		//case EFFECTSPRITETYPE_unknow_2224:
		//	delayFrame = 25;
		//	break;
		//case EFFECTSPRITETYPE_unknow_2298:
		case EFFECTSPRITETYPE_unknow_2299:
		//case EFFECTSPRITETYPE_unknow_2300:
		//case EFFECTSPRITETYPE_unknow_2301:
		//case EFFECTSPRITETYPE_unknow_2302:
		//case EFFECTSPRITETYPE_unknow_2303:
				IsMulti = true;
				delayFrame = 21;
				//switch( GetWeaponSpeed() )
				//{
				...
				//}
				break;
		//case EFFECTSPRITETYPE_unknow_2302:	 //µ÷Õû¶¯»­ËÙ¶È
		//	delayFrame = 10;
	//		break;
		}
...
MCreature::UpdateAttachEffect()
{
	...
	int bShowColor =  (HasEffectStatus(EFFECTSTATUS_CURSE_PARALYSIS) || HasEffectStatus(EFFECTSTATUS_FREEZE) || HasEffectStatus(EFFECTSTATUS_unknow660)
...
//		}
		...
		if(HasEffectStatusOusterNewWings1(this))//Ä§ÁéÐÂ×øÆï
		{
			RemoveEffectStatusOusterNewWings1( this );
			SetAction( ACTION_STAND );
		}
		if(HasEffectStatusOusterNewWings2(this))//Ä§ÁéÐÂ×øÆï
		{
			RemoveEffectStatusOusterNewWings2( this );
			SetAction( ACTION_STAND );
		}
#endif //__SECOND_TRANSFORTER
		if(HasEffectStatus(EFFECTSTATUS_GLACIER))
...
MCreature::UpdateCutHeight()
{
	if (m_bCutHeight)
	{
		...
		if( GetCreatureType() == 702 || GetCreatureType() == 703 || GetCreatureType() == 704  || GetCreatureType() == 1562)
		{
...
/////////////////ouster ghost/////////////
bool MCreature::IsOusterGhost()
{
	return m_bOusterGhost;
}

int MCreature::GetOusterGhostCount()
{
	return m_OusterGhostCount;
}

void				
MCreature::UpdateOusterGhost()
{
	if(IsOusterGhost() && !IsFastMove())
	{
		if(--m_OusterGhostCount < 0)
		{
			StopOusterGhost();
		}
	}
...
//	}
	...
	// ¸ó½ºÅÍµéÀº °ø°Ý ¸ð¼Ç°ú ¸¶¹ý ½ÃÀü ¸ð½ÀÀÌ ÇÏ³ª»ÓÀÌ´Ù.
	...
	if(HasEffectStatus(EFFECTSTATUS_INSTALL_TURRET) && action != ACTION_STAND && action != ACTION_ATTACK)
		return;
	if(HasEffectStatus(EFFECTSTATUS_INSTALL_TURRET) && action != ACTION_STAND)
		return;
...
//		int i = 0;
	...
	if (action < GetActionMax())
	{
		...
		if( IsOusters() && HasEffectStatusOusterNewWings1( this ) && m_MoveDevice==MOVE_DEVICE_WING_SYLPH)
		{
			if (action==ACTION_STAND)
			{
				action = ACTION_OUSTERS_UNICORN_STAND;
		...
		if( IsOusters() && HasEffectStatusOusterNewWings2( this ) && m_MoveDevice==MOVE_DEVICE_WING_SYLPH)
		{
			if (action==ACTION_STAND)
			{
				action = ACTION_OUSTERS_UNICORN_STAND;
...
//	str = (char*)temp.c_str();
	...
	if(strstr(str, "¹èÃ¶¼ü´Ï´Ù") != NULL || strcmp(str, "A1") ==0)
	{
		m_HeadSkin = 215;
	}
	else if(strstr(str, "ÇØ°ñ¹Ù°¡Áö") != NULL|| strcmp(str, "A2") ==0)
	{
		m_HeadSkin = 149;
	}
	else if(strstr(str, "´Á´ëÀÎ°£") != NULL|| strcmp(str, "A3") ==0)
	{
		m_HeadSkin = 397;
	}
	else if(strstr(str, "´ß´ë°¡¸®") != NULL|| strcmp(str, "A4") ==0)
	{
		m_HeadSkin = 221;
	}
	else if(strstr(str, "°¡¸é¶óÀÌ´õ") != NULL|| strcmp(str, "A5") ==0)
	{
		m_HeadSkin = 316;
	}
...
//	if (m_NextChatFadeTime >= g_CurrentTime || ( CurPernalShop() == 1 && m_NextChatFadeTime == 0 ) )
	...
	else
	{
		...
		{
#if __CONTENTS(__TIPOJYU_CASTLE)		//±¸½Ã¿Â ¸»Ç³¼± ¶ç¿ì±â
...
//						moveTimes_div_2 = moveTimes_div_2/2;
				...
				{				
					...
					else if (m_MoveDevice==MOVE_DEVICE_RIDE)
					{
						...
						||(m_MoveDevice==MOVE_DEVICE_WING_SYLPH && HasEffectStatusOusterNewWings1( this ))//Ä§ÁéÐÂ×øÆï
			...
			if (m_bFastMove)
			{
					...
					&& m_nNextUsedActionInfo!=m_nBasicActionInfo)
				{
					...
					if (pCreature!=NULL 
						...
						//&& m_nSpecialActionInfo != SKILL_ADV2_SUDDEN_BLAST 
						...
						&& !HasEffectStatus(EFFECTSTATUS_BIKE_CRASH))
					{
						...
						else if(m_nSpecialActionInfo == SKILL_ADV2_SUDDEN_BLAST)
						{
							ActionInfo = SKILL_ADV2_SUDDEN_BLAST;
						}
						else if(m_nSpecialActionInfo == SKILL_ACCELERATE_CHARGE_1)
						{
							ActionInfo = SKILL_ACCELERATE_CHARGE_1;
						}
						else if(m_nSpecialActionInfo == SKILL_ACCELERATE_CHARGE_2)
						{
							ActionInfo = SKILL_ACCELERATE_CHARGE_2;
						}
						else if(m_nSpecialActionInfo == SKILL_SET_AFIRE_2)
						{
							ActionInfo = RESULT_SKILL_SET_AFIRE_2;
						}
...
{	
	...
	if (m_nUsedActionInfo!=ACTIONINFO_NULL)
	{
		...
		BOOL bStartEffect = m_ActionCount==StartFrame || 
							StartFrame >= GetActionCountMax() && bEndAction || m_nUsedActionInfo == 518;
...
MCreature::AffectUsedActionInfo(TYPE_ACTIONINFO nUsedActionInfo)
{
	...
	if ((*g_pActionInfoTable)[nUsedActionInfo].GetSize()!=0)
	{
		...
		if (bCastingAction)
		{
			...
			if( nUsedActionInfo == CASTING_FIRE || nUsedActionInfo == CASTING_WATER ||          //by rallser Ê¹ÓÃ¼¼ÄÜ ÈËÎïÉíÌåÌØÐ§
...
//----------------------------------------------------------------------
void MCreature::SetDirection(BYTE d)		
{ 
	m_Direction = d; 
}//by viva
void
...
MCreature::CheckDropBlood()
{
	if (!HasEffectStatus(EFFECTSTATUS_COMA))
	{
		...
		{
			HP_t currentHP = m_Status[MODIFY_CURRENT_HP];
			HP_t maxHP = m_Status[MODIFY_MAX_HP];
...
MCreature::UpdateStatus()
{
	...
	// ¾Æ·¡ »óÅÂ¿¡¼­ Ç®·Á³µÀ»¶§ ÇÇ°¡ ÇÑ²¨¹ø¿¡ Â÷´Â°ÍÀ» ¹æÁöÇÏ±â À§ÇØ¼­ ¸®Á¨Å¸ÀÓÀ» µ¿±âÈ­ ÇÑ´Ù.
	//--------------------------------------------------------
	if (!m_bAlive 
		...
		{			
			...
			{		
					...
					)
				{
					...
					else if(IsOusters() && HasEffectStatusOusterNewWings1( this ))
						m_Action = ACTION_OUSTERS_UNICORN_STAND;
					else if(IsOusters() && HasEffectStatusOusterNewWings2( this ))
						m_Action = ACTION_OUSTERS_UNICORN_STAND;
#endif //__SECOND_TRANSFORTER
...
//		DEBUG_ADD("return");
	...
	if ((*g_pActionInfoTable)[nActionInfo].IsTargetZone()											||
		...
		|| nActionInfo == SKILL_BLOODY_WINGS	// ÀÌ°Å ¹¹ÀÌ·¡ °³¶Ë °°³× -_-;
#if __CONTENTS(__SECOND_RARE_SKILL)
		|| nActionInfo == SKILL_BRIONAC || nActionInfo == SKILL_BLADE_STORM
		|| nActionInfo == SKILL_LETHAL_CLAW || nActionInfo == SKILL_FROZEN_GLACIER || nActionInfo == SKILL_ADV2_CHAOS_SPLITTER || nActionInfo == SKILL_ADV2_CHAOS_SPLITTER_2
#endif //__SECOND_RARE_SKILL
#if __CONTENTS(__TIPOJYU_CASTLE)	//¹®Áö±â ½ºÅ³
	...
	//	DEBUG_ADD("1");
		...
		if(GetCreatureType() == 764 || GetCreatureType() == 765 || GetCreatureType() == 891 || GetCreatureType() == 2014 || GetCreatureType() == 2015)
		{
			SetTraceID( GetID() );
...
MCreature::StopBloodDrain()
{
	...
	{
		if( m_ActionCount>9 && m_ActionCount<70 && GetCreatureType() == CREATURETYPE_WER_WOLF )//ÎüÑª
		{
			m_ActionCount = 70;
			UnSetStopBloodDrain();
		} else
...
void	
MCreature::UpdateGunFiled()
{
	if (g_pPlayer->HasEffectStatus( EFFECTSTATUS_BLINDNESS ) ||
		!IsSlayer() || IsSlayer() && g_pZone->GetID() == 3001
		&& !g_pPlayer->HasEffectStatus( EFFECTSTATUS_GHOST )
		) 
	{
		if (m_GunFiledCountInc==0)
		{
			if (m_ActionCount>=GetActionCountMax()-1)
			{
				CheckInGunFiled();
		...
		{
			m_GunFiledCount += m_GunFiledCountInc;

			// darkness¿¡ µé¾î°¡´Â °æ¿ì
			if (m_GunFiledCount < 0)
			{
				m_GunFiledCount = 0;
				m_GunFiledCountInc = 0;
			}
			// darkness¿¡¼­ ºüÁ®³ª¿À´Â °æ¿ì
			else if (m_GunFiledCount >= MAX_GUNFILED_COUNT)
			{
				m_GunFiledCount = -1;
				m_GunFiledCountInc = 0;
			}
	...
	{
		{
				...
				&& m_Y >= 0 && m_Y < g_pZone->GetHeight())
			{
				...
				if (m_bAlive && (sector.HasGunFiled()))
				{
					m_GunFiledCount = MAX_GUNFILED_COUNT;
					m_GunFiledCountInc = 0;
				}
				else
				{		
...
void
MCreature::PlaceInGunFiled(bool IsBlindness)
{
	if (
		(IsBlindness || g_pPlayer->HasEffectStatus(EFFECTSTATUS_BLINDNESS) || (!IsSlayer() || IsSlayer() && g_pZone->GetID() == 3001))
		...
		)
	{
		...
		{
			m_GunFiledCount = MAX_GUNFILED_COUNT;		
			m_GunFiledCountInc = -1;
		}	
		else if (m_GunFiledCount > 0)
		{
			m_GunFiledCountInc = -1;
		}
		else if (m_GunFiledCount==0)
...
void
MCreature::PlaceNotInGunFiled()
{
	if (!IsSlayer() || IsSlayer() && g_pZone->GetID() == 3001
		&& !g_pPlayer->HasEffectStatus( EFFECTSTATUS_GHOST )
		)
	{
		if (m_GunFiledCount >= 0 && m_GunFiledCount < MAX_GUNFILED_COUNT)
		{
			m_GunFiledCountInc = 1;
		}	
	}
...
BOOL	
MCreature::ShowInGunFiled(int sX, int sY) const
{
	if (
		m_GunFiledCount >= 0
		&& !g_pPlayer->HasEffectStatus( EFFECTSTATUS_GHOST )
	...
	{
		if ((max(abs(m_X-sX), abs(m_Y-sY))) <= m_GunFiledCount)
		{
			return TRUE;
		}
...
{ 
		...
		)
	{
		...
		if( HasEffectStatus( EFFECTSTATUS_RAGE_OF_BLOOD ) )
			amount += 150;	 //by rallser
	}
...
MCreature::CheckRegen()
{
	...
	switch (m_CreatureType)
	{
		...
		case CREATURETYPE_WER_WOLF ://ÎüÑª¹íÉúÃü»Ö¸´
...
bool MCreature::UpdateBurningSol()
{
	if(m_bBurningSol)
	{
		...
		else if(HasEffectStatus(EFFECTSTATUS_CURSE_PARALYSIS) || HasEffectStatus(EFFECTSTATUS_unknow660))
		{
			StopBurningSol();
			SetStop();
...
bool MCreature::UpDateInstallMK1()
{
	if(HasEffectStatus(EFFECTSTATUS_INSTALL_MK1))
	{
return false;
...
MCreature::GetMasterEffectType(DWORD Status)
{
	...
	if(MarketType> MAX_EFFECTSPRITETYPE) return 0;
...
//					else
				/*	if(MarketType == 4) //sjheon 2005.05.12
					...
					else if(MarketType != 5 && MarketType != 4 && MarketType != 0 && MarketType != 6 && MarketType != 7 && MarketType != 8 && MarketType != 9 && MarketType != 10 && MarketType != 11 && MarketType != 12)
					    ...
					    type =  EFFECTSPRITETYPE_unknow_2166;//ÈËÀàÐÂ³á°ò
						//EFFECTSPRITETYPE_unknow_2166;
					else if(MarketType == 7) 
					    type =  EFFECTSPRITETYPE_TIPOJYU_LIFE_CASTLE_ADDMISION_NPC1;//Èý×åÍ¨ÓÃ
						//EFFECTSPRITETYPE_unknow_2166;
					else if(MarketType == 8) 
					    type =  EFFECTSPRITETYPE_BIKE_CRASH;//Ä¦ÍÐ
						//EFFECTSPRITETYPE_unknow_2166;
					else if(MarketType == 9) 
					    type =  EFFECTSPRITETYPE_unknow_2170;//ÈËÀàÕ½Éñ
						//EFFECTSPRITETYPE_unknow_2166;
				    else if(MarketType == 11) 
					    type =  EFFECTSPRITETYPE_unknow_2173;//GM±êÊ¾
						//EFFECTSPRITETYPE_unknow_2166;
					else if(MarketType == 10) 
					    type =  EFFECTSPRITETYPE_GEN_MASTER;//ÂÌ»ð2589
				    else if(MarketType == 12) 
					    type =  EFFECTSPRITETYPE_unknow_2591;*/
					if (MarketType != 0)//ÐÂ¹â»·
					{
					   switch(MarketType)
					   {
					   case 1:
							type = EFFECTSPRITETYPE_MARKET_MASTER_SLAYER_150_ADVANCE_FEAR;
							break;
					   case 2:
						   	type = EFFECTSPRITETYPE_MARKET_MASTER_SLAYER_150_ADVANCE_FEAR+1;
							break;
					   case 3:
						   type = EFFECTSPRITETYPE_MARKET_MASTER_SLAYER_150_ADVANCE_FEAR+2;
						    break;
					   case 4:
						   type = EFFECTSPRITETYPE_ADVANCEMENT_AURA_CRYSTAL_SLAYER;
						   break;
					   case 5:
						   ...
						   break;
					   case 6:
						   type = EFFECTSPRITETYPE_unknow_2166;
						   break;
					   case 7:
						   type = EFFECTSPRITETYPE_TIPOJYU_LIFE_CASTLE_ADDMISION_NPC1;
						   break;
					   case 8:
						   type = EFFECTSPRITETYPE_BIKE_CRASH;
						   break;
					   case 9:
						   type = EFFECTSPRITETYPE_unknow_2170;
						   break;
					   case 10:
						   type = EFFECTSPRITETYPE_GEN_MASTER;
						   break;
					   case 11:
						   type = EFFECTSPRITETYPE_unknow_2173;
						   break;
					   case 12:
						   type = EFFECTSPRITETYPE_unknow_2591;
						   break;
					   default:
						   type = EFFECTSPRITETYPE_BLOOD_GUN_1_1 + MarketType;
						   break;
					   }
...
//					else
	/*				if(MarketType == 4) // sjheon 2005.05.12
					   ...
					   // type =  EFFECTSPRITETYPE_unknow_2342;
					       type =  EFFECTSPRITETYPE_unknow_2167;
					else if(MarketType == 7) 
					    type =  EFFECTSPRITETYPE_TIPOJYU_LIFE_CASTLE_ADDMISION_NPC1;//Èý×åÍ¨ÓÃ ÈËÀà¹â»·
					else if(MarketType == 8) 
					    type =  EFFECTSPRITETYPE_unknow_2273;//¹í×å×¨ÓÃ
					else if(MarketType == 9) 
					    type =  EFFECTSPRITETYPE_unknow_2169;//¹í×åÕ½Éñ
					else if(MarketType == 10) 
					    type =  EFFECTSPRITETYPE_GEN_MASTER;//ÂÌ»ð
				    else if(MarketType == 11) 
					    type =  EFFECTSPRITETYPE_unknow_2173;//GM±êÊ¾
					 else if(MarketType == 12) 
					    type =  EFFECTSPRITETYPE_unknow_2591;*/
				...
				if (MarketType != 0)
					{
					   ...
					   {
					   case 1:
							type = EFFECTSPRITETYPE_MARKET_MASTER_VAMPIRE_150_FRONT_FEAR;
							break;
					   case 2:
						   	type = EFFECTSPRITETYPE_MARKET_MASTER_VAMPIRE_150_FRONT_FEAR+1;
							break;
					   case 3:
						   type = EFFECTSPRITETYPE_MARKET_MASTER_VAMPIRE_150_FRONT_FEAR+2;
						    break;
					   case 4:
						   type = EFFECTSPRITETYPE_ADVANCEMENT_AURA_CRYSTAL_VAMPIRE;
						   break;
					   ...
					   case 7:
						   ...
						   break;
					   case 8:
						   type = EFFECTSPRITETYPE_unknow_2273;
						   break;
					   case 9:
						   type = EFFECTSPRITETYPE_unknow_2169;
						   break;
					   case 10:
						   type = EFFECTSPRITETYPE_GEN_MASTER;
						   break;
...
//					else
					...
					//else if(MarketType != 5  && MarketType != 4)
					   ...
					   // type =  EFFECTSPRITETYPE_unknow_2168;
                        type =  EFFECTSPRITETYPE_unknow_2168;	
					else if(MarketType == 7) 
					    type =  EFFECTSPRITETYPE_TIPOJYU_LIFE_CASTLE_ADDMISION_NPC1;//Èý×åÍ¨ÓÃ
						//EFFECTSPRITETYPE_unknow_2166;
					else if(MarketType == 8) 
					    type =  EFFECTSPRITETYPE_MOONLIGHT_NPC_EFFECT;//Ä§×å×¨ÓÃ
						//EFFECTSPRITETYPE_unknow_2166;
					else if(MarketType == 9) 
					    type =  EFFECTSPRITETYPE_unknow_2171;//¹í×åÕ½Éñ
						//EFFECTSPRITETYPE_unknow_2166;
				    else if(MarketType == 11) 
					    type =  EFFECTSPRITETYPE_unknow_2173;//GM±êÊ¾
						//EFFECTSPRITETYPE_unknow_2166;	
					...
					if (MarketType != 0)
					{
					   ...
					   {
					   case 1:
							type = EFFECTSPRITETYPE_MARKET_MASTER_OUSTERS_150_FEAR;
							break;
					   case 2:
						   	type = EFFECTSPRITETYPE_MARKET_MASTER_OUSTERS_150_FEAR+1;
							break;
					   case 3:
						   type = EFFECTSPRITETYPE_MARKET_MASTER_OUSTERS_150_FEAR+2;
						    break;
					   case 4:
						   type = EFFECTSPRITETYPE_ADVANCEMENT_AURA_CRYSTAL_OUSTER;
						   break;
					   ...
					   case 7:
						   ...
						   break;
					   case 8:
						   type = EFFECTSPRITETYPE_MOONLIGHT_NPC_EFFECT;
						   break;
					   case 9:
						   type = EFFECTSPRITETYPE_unknow_2171;
						   break;
					   case 10:
						   type = EFFECTSPRITETYPE_GEN_MASTER;
						   break;
...
//		else if(HasEffectStatus(EFFECTSTATUS_unknow1))	status = EFFECTSTATUS_unknow1;
	}
	else if(IsVampire())
	{
...
bool	MCreature::IsShowNameCreature() const
{
	...
	bool bNo		= m_CreatureType == 659 ||								// ´ëÁöÀÇ »Ô
					  m_CreatureType == 673 ||								// ¸®Á¨ Á¸ Å¸¿ö
					  (m_CreatureType >= 726 && m_CreatureType <= 729)	||	// ¼º¹®
					  (m_CreatureType > 948 && m_CreatureType <1021)	||	// ¸Þ³×±×·Î½º Á¦´Ü	
					  m_CreatureType	== 1041||	// ¸Þ³×±×·Î½º Á¦´Ü	
					  m_CreatureType	== 1704||	// µØÓüÖ®ÃÅ²»ÏÔÊ¾Ãû×Ö
					   m_CreatureType	== 1909||
					    m_CreatureType	== 1817||
					  m_CreatureType	== 1714;							// µØÓüÖ®ÃÅ²»ÏÔÊ¾Ãû×Ö
	return	bOk && !bNo;
}
...
#endif //__FAST_TRANSFORTER
#if __CONTENTS(__FAST_TRANSFORTER)
void MCreature::SetWingEffect1() // 1Â÷ ÀÌµ¿¼ö´Ü
{
...
#if __CONTENTS(__SECOND_TRANSFORTER)
void MCreature::SetWingEffect2() // 2Â÷ ÀÌµ¿¼ö´Ü
{
	Race raceType = GetRace();
	int spriteType = EFFECTSPRITETYPE_NULL;
	switch(raceType)
...
void MCreature::SetWingEffect3(WORD type) // 2Â÷ ÀÌµ¿¼ö´Ü
{
	//TYPE_EFFECTSPRITETYPE spriteType = EFFECTSPRITETYPE_unknow_2327;


	//EFFECTSPRITETYPETABLE_INFO::COLORGROUP_MAP &colorGroupMap =
	//	(*g_pEffectSpriteTypeTable)[spriteType].ColorGroupMap;

	//EFFECTSPRITETYPETABLE_INFO::COLORGROUP_MAP::iterator pos = colorGroupMap.find(0);

	//if(pos != colorGroupMap.end())
	//	spriteType = pos->second;

	CreateAttachEffect(	EFFECTSPRITETYPE_unknow_2327, 0xFFFF, 0,FALSE, MEffect::EFFECT_ATTACH, false);
}
#endif //__SECOND_TRANSFORTER
