...
EFFECTSTATUS_NODE::SaveToFile(std::ofstream& file)
{
	...
	file.write((const char*)&new201, 1);
...
//----------------------------------------------------------------------
void			
EFFECTSTATUS_NODE::LoadFromFile(ivfstream& file)
{
	file.read((char*)&bUseEffectSprite, 1);
	file.read((char*)&bAttachGround, 1);
	file.read((char*)&new201, 1);

	file.read((char*)&EffectSpriteType, SIZE_EFFECTSPRITETYPE);//	2
	...
	file.read((char*)&ActionInfo, SIZE_ACTIONINFO);//2
	file.read((char*)&OriginalActionInfo, SIZE_ACTIONINFO);//2
	file.read((char*)&SoundID, sizeof(int) );	//2
}
