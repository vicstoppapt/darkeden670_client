...
MAroundZoneEffectGenerator::Generate( const EFFECTGENERATOR_INFO& egInfo )
{
	...
	*/
	...
	else if(est == EFFECTSPRITETYPE_BLADE_STORM)
		num = 6;
#endif //__SECOND_RARE_SKILL

#if __CONTENTS(__TIPOJYU_CASTLE)
	else if(est == EFFECTSPRITETYPE_TIPOJYU_LIFE_GATEKEEPER_ATTACK_A)
		num = 6;
	else if(est == EFFECTSPRITETYPE_TIPOJYU_HADES_GATEKEEPER_ATTACK_A)
		num = 6;
#endif // __TIPOJYU_CASTLE
	else if(est == EFFECTSPRITETYPE_GREAT_RUFFIAN_2_AXE_THROW)
		num = 3;
	//else if(est == EFFECTSPRITETYPE_unknow_2656)
	//	num = 3;
	else
		num = rand()%2 + 2;
	...
	for (int TempCount=0; TempCount<num; TempCount++)
	{
		...
		if (egInfo.effectSpriteType == EFFECTSPRITETYPE_POWER_OF_LAND_FIRE_2)
		{
			...
			{
			case 0:
				pixelPoint.x = pixelPoint.x - rand()%(TILE_X<<1) - TILE_X_HALF;
				pixelPoint.y = pixelPoint.y - rand()%(TILE_Y<<1) - TILE_Y_HALF;
				break;
		...
		else if (est==EFFECTSPRITETYPE_GUN_DUST_1)
		{
			...
			//pixelPoint.y += (rand()%TILE_Y_HALF) - (TILE_Y_HALF>>1);
...
#if __CONTENTS(__SECOND_RARE_SKILL)
			|| est == EFFECTSPRITETYPE_BLADE_STORM
#endif //__SECOND_RARE_SKILL
#if __CONTENTS(__TIPOJYU_CASTLE)
			|| est == EFFECTSPRITETYPE_TIPOJYU_LIFE_GATEKEEPER_ATTACK_A
			|| est == EFFECTSPRITETYPE_TIPOJYU_HADES_GATEKEEPER_ATTACK_A
#endif // __TIPOJYU_CASTLE
			)
		{
			int DirX = 0, DirY = 0;
...
//			pixelPoint.x += ((((TempCount)*(TILE_X_HALF))*DirX)+1);
//			pixelPoint.y += ((((TempCount)*(TILE_Y_HALF))*DirY)+1);
			pixelPoint.x += ((((TempCount+1)*(TILE_X_HALF))*DirX));
			pixelPoint.y += ((((TempCount+1)*(TILE_Y_HALF))*DirY));
			dwWaitCount = (TempCount);
		}
		else if(est == EFFECTSPRITETYPE_GREAT_RUFFIAN_2_AXE_THROW )//|| est == EFFECTSPRITETYPE_unknow_2656)
		{
			pixelPoint.x = egInfo.x0; 
			pixelPoint.y = egInfo.y0;
		}
		...
		// 방향 설정
		if(est == EFFECTSPRITETYPE_GREAT_RUFFIAN_2_AXE_THROW )//|| est == EFFECTSPRITETYPE_unknow_2656) 
		{
