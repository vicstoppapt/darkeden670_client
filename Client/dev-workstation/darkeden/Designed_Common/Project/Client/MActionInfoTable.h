...
// 기술의 속성에 대한 flag
//----------------------------------------------------------------------
#define	FLAG_ACTIONINFO_TARGET_NONE			0x00	// 사용할 수 없음
...
#define	FLAG_ACTIONINFO_SELECT_CORPSE				0x20	// Zone의 시체에 사용
...
class MActionInfo : public CTypeTable<ACTION_INFO_NODE> {
	public :
		...
		void		SetStartWithCasting()	{ m_StartFrame[0] = m_StartFrame[1] = m_StartFrame[2] = m_StartFrame[2] = m_StartFrame[3] = m_StartFrame[4] = m_StartFrame[5] = m_StartFrame[6] = m_StartFrame[7] = m_StartFrame[8] = 0; }
		void		SetStartAfterCasting()	{ m_StartFrame[0] = m_StartFrame[1] = m_StartFrame[2] = m_StartFrame[2] = m_StartFrame[3] = m_StartFrame[4] = m_StartFrame[5] = m_StartFrame[6] = m_StartFrame[7] = m_StartFrame[8] = 0xFFFF; }
		...
		// 캐스팅 타임을 설정한다. (1/10초 단위)
		WORD		GetCastingTime() const					{ return m_CastingTime;		}
		void		SetCastingTime(WORD ct)					{ m_CastingTime = ct;		}
		...
		void		SetComboSkillActionStep(TYPE_ACTIONINFO ComboStep1 , TYPE_ACTIONINFO ComboStep2 ,  TYPE_ACTIONINFO ComboStep3 ,  TYPE_ACTIONINFO ComboStep4) ; 
		TYPE_ACTIONINFO		GetComboSkillActionStep(byte byStep) ;
		...
		void		SetStartFrameAll(int f)	{ m_StartFrame[0] = m_StartFrame[1] = m_StartFrame[2] = m_StartFrame[2] = m_StartFrame[3] = m_StartFrame[4] = m_StartFrame[5] = m_StartFrame[6] = m_StartFrame[7] = m_StartFrame[8]= f; }
		void		SetStartFrame(int speed, int f)	{ m_StartFrame[speed] = f; }
		int			GetStartFrame(int speed) const	{ return m_StartFrame[speed]; }

		
		void		SetComboSkillStartFrame(int speed, int Step1 , int Step2 , int Step3 , int Step4) ;
		int			GetComboSkillStartFrame(int speed ,  int  Setp) const	{ return m_ComboSkillStartFrame[speed][Setp-1]; }
		...
		//-------------------------------------------------------
		void					SetSelectCreatureAll()				{ m_SelectCreatureAttr = FLAG_ACTIONINFO_SELECT_ALL; }
		void					SetSelectCreature(BYTE fSelect)		{ m_SelectCreatureAttr = fSelect; }
		const BYTE_ATTRIBUTE	GetActionTarget() const				{ return m_SelectCreatureAttr; }
		...
		BYTE		GetActionStepCnt()	{	return	m_byActionStepCnt ; }
		void		SetActionStepCnt(BYTE byActionStepCnt)	{	m_byActionStepCnt =  byActionStepCnt ; } 
	...
	protected :
		...
		TYPE_ACTIONINFO				m_ComboSkillActionStep[MAX_ACTION_STEP - 1] ;
		...
		WORD						m_CastingTime;				// Casting Sprite 표현 시간 (1/10초 단위)
		...
		int							m_StartFrame[9];				// Effect가 시작하는 Frame
		int							m_ComboSkillStartFrame[3][4];	// ComboSkillEffect가 시작하는 Frame
		...
		int							m_RepeatStartFrame[9];
		int							m_RepeatEndFrame[9];
		WORD						m_RepeatLimit;			// 반복회수 제한

		// 기술의 선택 대상
		BYTE_ATTRIBUTE				m_SelectCreatureAttr;
		bool						m_bUseGrade;		
		bool						m_bAttachSelf;
		
		BYTE						m_MasterySkillStep;
		bool						m_bIgnoreFailDelay; //  스킬 실패해도 딜레이는 남는다.
		BYTE						m_byActionStepCnt; //
