...
MRisingEffectGenerator::Generate( const EFFECTGENERATOR_INFO& egInfo )
{
		...
		egInfo.nActionInfo ==SKILL_DRAGON_FIRE_CRACKER)
	{
		...
		int step_count = sqrt(float(int(sinValue * sinValue)+ int(cosValue)*int(cosValue))) / egInfo.count;		
	...
	{		
		...
		step_count = sqrt( float( int(sinValue * sinValue) + int(cosValue)*int(cosValue) ) ) / egInfo.count;
