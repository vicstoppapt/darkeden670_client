...
//----------------------------------------------------------------------
void		
ITEM_WEARINFO::LoadFromFile(ivfstream& file)
{
...
{ 	
	...
	EffectStatus	= EFFECTSTATUS_NULL;
	EffectSpriteType = MAX_EFFECTSPRITETYPE;
...
CREATURETABLE_INFO::GetActionMax() const
{
	...
	switch (m_CreatureTribe)
	{
		...
		case CREATURETRIBE_NPC :				// NPC
			...
			return ACTION_MAX_VAMPIRE;
		break;		
	case CREATURETRIBE_UNKNOW1 :				// NPC
	case CREATURETRIBE_UNKNOW2 :	
	case CREATURETRIBE_UNKNOW3 :	
		//return ACTION_MAX_NPC;
		return ACTION_MAX_VAMPIRE;
		break;
	case CREATURETRIBE_UNKNOW4 :
	case CREATURETRIBE_UNKNOW5 :
		//return ACTION_MAX_NPC;
		return ACTION_MAX_VAMPIRE;
	}
...
//		file.write((const char*)&SpriteTypes[stcnt-(st+1)], 4);
	...
	file.write((const char*)&EffectStatus, 4);
	//EffectStatus += 135;
	...
	file.write((const char*)&bFade, 1);
	file.write((const char*)&bFadeShadow, 1);

	file.write((const char*)&EffectSpriteType, 4);// 몬스터 몸에 붙는 이팩트 스프라이트
	//EffectSpriteType += 238;//覩굶238 녘膠뫘劤+5
...
//----------------------------------------------------------------------
void			
CREATURETABLE_INFO::LoadFromFile(ivfstream& file)
{
...
//	DeadActionInfo += 122;
	...
	file.read((char*)&bFade, 1);
	file.read((char*)&bFadeShadow, 1);

	file.read((char*)&EffectSpriteType, 4);// 몬스터 몸에 붙는 이팩트 스프라이트
	//EffectSpriteType += 238;//覩굶238 녘膠뫘劤+5
...
CREATURETABLE_INFO::operator = (const CREATURETABLE_INFO& creatureInfo)
{
	...
	EffectStatus	= creatureInfo.EffectStatus;
	Level			= creatureInfo.Level;
	bFade			= creatureInfo.bFade;
	bFadeShadow		= creatureInfo.bFadeShadow;
	EffectSpriteType = creatureInfo.EffectSpriteType;
...
void
CREATURETABLE_INFO::SaveToCSVFile(std::ofstream& file, unsigned int CreatureID)
{
	SaveNumberToCSVFile(file, CreatureID);
	SaveStringToCSVFile(file, Name);
}
...
//----------------------------------------------------------------------
void				
CreatureSpriteTypeMapper::LoadFromFile(ivfstream& file)
{
	int numSpriteTypes;
...
//--------------------------------------------------------------------------
void 
CREATURETABLE_INFO::SaveStringToCSVFile(std::ofstream& file, const MString& value)
{
...
//--------------------------------------------------------------------------
void 
CREATURETABLE_INFO::SaveNumberToCSVFile(std::ofstream& file, int value, int null)
{
	if(null == value)
		file << CSV_NULL_VALUE;
	else
