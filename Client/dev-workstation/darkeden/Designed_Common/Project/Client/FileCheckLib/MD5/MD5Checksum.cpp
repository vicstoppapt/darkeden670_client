...
NOTES:			Provides an interface to the CMD5Checksum class. 'strFilePath' name should 
				...
				exception.
*****************************************************************************************/
std::string CMD5Checksum::GetMD5(LPCTSTR szFilePath)
{
	//open the file as a binary file in readonly mode, denying write access 
	std::ifstream File(szFilePath, std::ios_base::binary);
...
*****************************************************************************************/
std::string CMD5Checksum::GetMD5(std::ifstream& File)
{
	try
	{
		...
		for(;;)
		{
			...
			nLength = static_cast<int>(File.gcount());
			if(nLength <= 0) break;
	...
	{
		assert("CMD5Checksum::GetMD5: CFileException caught" && FALSE);	
		return NULL;
	}
}
