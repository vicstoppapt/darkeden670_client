...
extern MItem*		g_pPacketItemVampireCoat[VAMPIRE_COAT_MAX];
extern MItem*		g_pPacketItemVampireArm[VAMPIRE_ARM_MAX];
...
extern int		GetVampireCreatureType(int shape, bool bMale, int coatType, DWORD wingItemType = 0);
#else
...
#if __CONTENTS(__GEAR_SWAP_CHANGE)	//SetGearInfo	함수 선언
extern void		SetGearInfo(GearInfo* pGearInfo, BloodBibleSignInfo* pBloodBibleInfo = NULL, DWORD dwGearID	= 0, bool bOtherGear = false);
#else
extern void		SetGearInfo(GearInfo* pGearInfo, BloodBibleSignInfo* pBloodBibleInfo = NULL, bool bOtherGear = false);
...
extern void		SetAddonToVampire(MCreatureWear* pCreature, const PCVampireInfo2* pInfo);
extern void		SetAddonToVampire(MCreatureWear* pCreature, const PCVampireInfo3* pInfo);
...
//-----------------------------------------------------------------------------
// Skill to SectorSKILL_SPECTOR_INVERSE
//-----------------------------------------------------------------------------
...
extern void		SkillSPECTORINVERSE(MCreature* pUserCreature, MCreature* pTargetCreature,int skillID, int shape = 0);	// shape 0 : 자동, 1 : 십자모양(+), 2 : 엑스모양(x)
extern void		SkillBlazeWalk(MCreature* pUserCreature, MCreature* pTargetCreature, int skillID, BYTE grade);
extern void     SkillBatStorm1(MCreature* pUserCreature,MCreature* pTargetCreature,int targetX, int targetY,int skillID, int shape = 0);//test bat
#if __CONTENTS(__ONIBLA)
extern void		SkillGhostShadow(MCreature* pUserCreature, MCreature* pTargetCreature, int skillID, int shadowCount, int shape = 0);	// shape 0 : 자동, 1 : 십자모양(+), 2 : 엑스모양(x)
#endif //__ONIBLA
//-----------------------------------------------------------------------------
...
extern void		SetFadeStart(char start, char end, char step, BYTE r = 0, BYTE g = 0, BYTE b = 0,WORD delay = 0, BYTE delayPostion = FADE_DELAY_POSTION_BACK);
