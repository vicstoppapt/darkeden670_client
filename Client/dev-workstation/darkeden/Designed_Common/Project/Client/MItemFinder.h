...
//----------------------------------------------------------------------
// MOustersWingItemFinder  - 아우스터즈 신규이동수단 아이템 Finider
//----------------------------------------------------------------------
//----------------------------------------------------------------------
class MOustersWingItemFinder : public MItemFinder {
	public	:
		MOustersWingItemFinder () {}

		bool	operator () ( const MItem* pItem )
		{
#if __CONTENTS(__FAST_TRANSFORTER||__SECOND_TRANSFORTER)
			return pItem->GetItemClass()==ITEM_CLASS_OUSTERSWING_ITEM
					&& pItem->GetNumber() > 0;
...
//----------------------------------------------------------------------
// MOustersWingItemFinder  - 뱀파이어 신규이동수단 아이템 Finider
//----------------------------------------------------------------------
//----------------------------------------------------------------------
class MVampireWingItemFinder : public MItemFinder {
	public	:
		MVampireWingItemFinder () {}

		bool	operator () ( const MItem* pItem )
		{
#if __CONTENTS(__FAST_TRANSFORTER||__SECOND_TRANSFORTER)
			return pItem->GetItemClass()==ITEM_CLASS_VAMPIREWING_ITEM
					&& pItem->GetNumber() > 0;
#endif //__FAST_TRANSFORTER||__SECOND_TRANSFORTER
			return false;
		}
	...
	public :
		...
		bool	operator () ( const MItem* pItem )
		{
			return pItem->GetItemClass()==ITEM_CLASS_EVENT_GIFT_BOX
					&& pItem->GetItemType() != 1 && pItem->GetItemType() != 27
					&& pItem->IsTrade();
		}
};
...
// MItemClassFinder
//----------------------------------------------------------------------
// 같은 objectid의 item을 찾는다.
//----------------------------------------------------------------------
class MItemObjectFinder : public MItemFinder {
	public :
		MItemObjectFinder(TYPE_OBJECTID objID)		{ m_ItemObject = objID; }
		
		void	SetItemObject(TYPE_OBJECTID objID)	{ m_ItemObject = objID; }

		bool	operator () ( const MItem* pItem )
		{
			return pItem->GetID() == m_ItemObject;
		}

	protected :
		TYPE_OBJECTID	m_ItemObject;	// 찾을려는 itemClass
};
