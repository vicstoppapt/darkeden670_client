...
extern CMessageArray*		g_pWorldChatingMessage;
...
inline		bool			IsAffectFromDarkness()
{
	if( g_pPlayer->IsVampire() ||
		...
		g_pPlayer->HasEffectStatus( EFFECTSTATUS_FLAME_SIGH ) ||
		g_pZone->GetID() == 3001 )
		return false;
...
//	}
	...
	//by viva vampire_ispk_shadow_frame
	m_AdvancementVampireMan171FPK.Release();
	m_AdvancementVampireWoman171FPK.Release();
	m_AdvancementVampireManShadow171FPK.Release();
	m_AdvancementVampireWomanShadow171FPK.Release();

	m_AdvancementSlayerMan171FPK.Release();
	m_AdvancementSlayerWoman171FPK.Release();
	m_AdvancementSlayerManShadow171FPK.Release();
	m_AdvancementSlayerWomanShadow171FPK.Release();

	m_AdvancementOusters171FPK.Release();
	m_AdvancementOustersShadow171FPK.Release();
	m_AdvancementOusters201FPK.Release();//rallser201
	m_AdvancementOustersShadow201FPK.Release();//rallser201

	m_AdvancementSlayerMan201FPK.Release();
	m_AdvancementSlayerWoman201FPK.Release();
	m_AdvancementSlayerManShadow201FPK.Release();
	m_AdvancementSlayerWomanShadow201FPK.Release();
	m_AdvancementSlayerMan231FPK.Release();
	m_AdvancementSlayerWoman231FPK.Release();
	m_AdvancementSlayerManShadow231FPK.Release();
	m_AdvancementSlayerWomanShadow231FPK.Release();

	m_AdvancementVampireMan201FPK.Release();
	m_AdvancementVampireWoman201FPK.Release();
	m_AdvancementVampireManShadow201FPK.Release();
	m_AdvancementVampireWomanShadow201FPK.Release();
	m_AdvancementVampireMan231FPK.Release();
	m_AdvancementVampireWoman231FPK.Release();
	m_AdvancementVampireManShadow231FPK.Release();
	m_AdvancementVampireWomanShadow231FPK.Release();
	m_AdvancementOusters231FPK.Release();//rallser231
	m_AdvancementOustersShadow231FPK.Release();//rallser231
	//end
...
//#if __CONTENTS(__ONIBLA)
	m_CreatureSPK2.Release();			// Creature Sprite들
	m_CreatureSPK3.Release();			// Creature Sprite들
//#endif //__ONIBLA
	m_AddonManSPK.Release();		// Sprite들		
	m_AddonWomanSPK.Release();		// Sprite들		
	...
	//by viva vampire_ispk_shadow
	m_AdvancementVampireMan171SPK.Release();
	m_AdvancementVampireWoman171SPK.Release();
	m_AdvancementVampireMan171SSPK.Release();
	m_AdvancementVampireWoman171SSPK.Release();

	m_AdvancementSlayerMan171SPK.Release();
	m_AdvancementSlayerWoman171SPK.Release();
	m_AdvancementSlayerMan171SSPK.Release();
	m_AdvancementSlayerWoman171SSPK.Release();

	m_AdvancementOusters171SPK.Release();
	m_AdvancementOusters171SSPK.Release();
	
	m_AdvancementOusters201SPK.Release();//rallser201
	m_AdvancementOusters201SSPK.Release();//rallser201

	m_AdvancementSlayerMan201SPK.Release();
	m_AdvancementSlayerWoman201SPK.Release();
	m_AdvancementSlayerMan201SSPK.Release();
	m_AdvancementSlayerWoman201SSPK.Release();
	m_AdvancementSlayerMan231SPK.Release();
	m_AdvancementSlayerWoman231SPK.Release();
	m_AdvancementSlayerMan231SSPK.Release();
	m_AdvancementSlayerWoman231SSPK.Release();

	m_AdvancementVampireMan201SPK.Release();
	m_AdvancementVampireWoman201SPK.Release();
	m_AdvancementVampireMan201SSPK.Release();
	m_AdvancementVampireWoman201SSPK.Release();
	m_AdvancementVampireMan231SPK.Release();
	m_AdvancementVampireWoman231SPK.Release();
	m_AdvancementVampireMan231SSPK.Release();
	m_AdvancementVampireWoman231SSPK.Release();

	m_AdvancementOusters231SPK.Release();//rallser201
	m_AdvancementOusters231SSPK.Release();//rallser201
	//end
	...
	m_AddonManSSPK.Release();			// Sprite들		
	m_AddonWomanSSPK.Release();			// Sprite들		
...
//#if __CONTENTS(__ONIBLA)
	...
	if (m_CreatureSPK3.GetSize()==0)
	{
		m_CreatureSPK3.LoadFromFileRunning(ISPK_CREATURE_3);
	}
//#endif //__ONIBLA
//	UI_DrawProgress(3);
//	DrawTitleLoading();
...
//#if __CONTENTS(__ONIBLA)
	...
	if (m_CreatureSSPK3.GetSize()==0)
	{
		m_CreatureSSPK3.LoadFromFileRunning(SSPK_CREATURE_3);
	}
//#endif //__ONIBLA
//	UI_DrawProgress(4);
//	DrawTitleLoading();
...
//	}
	...
	//by viva vampire_ispk
	m_AdvancementVampireMan171SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_VAMPIRE_MAN_171").c_str() );
	m_AdvancementVampireWoman171SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_171").c_str() );

	m_AdvancementSlayerMan171SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_SLAYER_MAN_171").c_str() );
	m_AdvancementSlayerWoman171SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_SLAYER_WOMAN_171").c_str() );

	m_AdvancementOusters171SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_OUSTERS_171").c_str() );
 //201 begin
    m_AdvancementOusters201SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_OUSTERS_201").c_str() );//rallser201
	
	m_AdvancementVampireMan201SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_VAMPIRE_MAN_201").c_str() );
	m_AdvancementVampireWoman201SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_201").c_str() );

	m_AdvancementSlayerMan201SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_SLAYER_MAN_201").c_str() );
	m_AdvancementSlayerWoman201SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_SLAYER_WOMAN_201").c_str() );

	m_AdvancementOusters231SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_OUSTERS_231").c_str() );//rallser231
	m_AdvancementVampireMan231SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_VAMPIRE_MAN_231").c_str() );
	m_AdvancementVampireWoman231SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_231").c_str() );
	m_AdvancementSlayerMan231SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_SLAYER_MAN_231").c_str() );
	m_AdvancementSlayerWoman231SPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_ISPRITE_ADVANCEMENT_CLASS_SLAYER_WOMAN_231").c_str() );
...
// 	m_AdvancementVampireWomanSPK.LoadFromFilePart(0, m_AdvancementVampireWomanSPK.GetSize());
	...
	//by viva  shadow_spk
	m_AdvancementVampireMan171SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_VAMPIRE_MAN_171").c_str() );
	m_AdvancementVampireWoman171SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_171").c_str() );

	m_AdvancementSlayerMan171SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_SLAYER_MAN_171").c_str() );
	m_AdvancementSlayerWoman171SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_SLAYER_WOMAN_171").c_str() );

	m_AdvancementOusters171SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_OUSTERS_171").c_str() );

	//201 begin
	m_AdvancementOusters201SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_OUSTERS_201").c_str() );//rallser201
	
	m_AdvancementVampireMan201SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_VAMPIRE_MAN_201").c_str() );
	m_AdvancementVampireWoman201SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_201").c_str() );

	m_AdvancementSlayerMan201SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_SLAYER_MAN_201").c_str() );
	m_AdvancementSlayerWoman201SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_SLAYER_WOMAN_201").c_str() );

	m_AdvancementOusters231SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_OUSTERS_231").c_str() );//rallser231
	m_AdvancementVampireMan231SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_VAMPIRE_MAN_231").c_str() );
	m_AdvancementVampireWoman231SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_231").c_str() );
	m_AdvancementSlayerMan231SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_SLAYER_MAN_231").c_str() );
	m_AdvancementSlayerWoman231SSPK.LoadFromFileRunning( g_pFileDef->getProperty("FILE_SSPRITE_ADVANCEMENT_CLASS_SLAYER_WOMAN_231").c_str() );
...
//		itemTileFile2.close();
		m_ItemTileISPK.LoadFromFileRunning(g_pFileDef->getProperty("FILE_ISPRITE_ITEMTILE").c_str());
	//	m_ItemTileISPK.LoadFromFileRunning(ISPK_ITEM);
	}
	...
	*/
	...
	else
	{
		...
		*/
		...
		std::random_shuffle(pixelWidth.begin(), pixelWidth.end());
		...
		std::random_shuffle(pixelHeight.begin(), pixelHeight.end());
...
/*
	...
	const char* strAction[] = {
			...
			"공격",				// ACTION_ATTACK				2
			"마법공격",			// ACTION_MAGIC					3
			"대미지",			// ACTION_DAMAGED				4
			"흡혈당하기",		// ACTION_DRAINED				5
			"죽기",				// ACTION_DIE					6
			"흡혈",				// 
			"정지2",
			"정지3",
...
*/
	...
	*/
	...
	//by viva  vampire_ispk_frame  //-----------------------------vampire-----------------------------------
	ivfstream AdvancementVampireMan171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_MAN_171").c_str(), 
		AdvancementVampireMan171File))
		return false;
	m_AdvancementVampireMan171FPK.LoadFromFile( AdvancementVampireMan171File );
	AdvancementVampireMan171File.close();

	ivfstream AdvancementVampireWonman171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_171").c_str(), 
		AdvancementVampireWonman171File))
		return false;
	m_AdvancementVampireWoman171FPK.LoadFromFile( AdvancementVampireWonman171File );
	AdvancementVampireWonman171File.close();

		ivfstream AdvancementVampireMan201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_MAN_201").c_str(), 
		AdvancementVampireMan201File))
		return false;
	m_AdvancementVampireMan201FPK.LoadFromFile( AdvancementVampireMan201File );
	AdvancementVampireMan201File.close();

	ivfstream AdvancementVampireWonman201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_201").c_str(), 
		AdvancementVampireWonman201File))
		return false;
	m_AdvancementVampireWoman201FPK.LoadFromFile( AdvancementVampireWonman201File );
	AdvancementVampireWonman201File.close();
//-----------------------------------Slayer---------------
	ivfstream AdvancementSlayerMan171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_MAN_171").c_str(), 
		AdvancementSlayerMan171File))
		return false;
	m_AdvancementSlayerMan171FPK.LoadFromFile( AdvancementSlayerMan171File );
	AdvancementSlayerMan171File.close();

	ivfstream AdvancementSlayerWoman171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_WOMAN_171").c_str(), 
		AdvancementSlayerWoman171File))
		return false;
	m_AdvancementSlayerWoman171FPK.LoadFromFile( AdvancementSlayerWoman171File );
	AdvancementSlayerWoman171File.close();//201 begin

	ivfstream AdvancementSlayerMan201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_MAN_201").c_str(), 
		AdvancementSlayerMan201File))
		return false;
	m_AdvancementSlayerMan201FPK.LoadFromFile( AdvancementSlayerMan201File );
	AdvancementSlayerMan201File.close();

	ivfstream AdvancementSlayerWoman201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_WOMAN_201").c_str(), 
		AdvancementSlayerWoman201File))
		return false;
	m_AdvancementSlayerWoman201FPK.LoadFromFile( AdvancementSlayerWoman201File );
	AdvancementSlayerWoman201File.close();
//-------------------------------ousters---------------------
	ivfstream AdvancementOusters171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_OUSTERS_171").c_str(), 
		AdvancementOusters171File))
		return false;
	m_AdvancementOusters171FPK.LoadFromFile( AdvancementOusters171File );
	AdvancementOusters171File.close();
	//end
	ivfstream AdvancementOusters201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_OUSTERS_201").c_str(), 
		AdvancementOusters201File))
		return false;
	m_AdvancementOusters201FPK.LoadFromFile( AdvancementOusters201File );
	AdvancementOusters201File.close();
	...
	//by viva   vampire_shadow_frame
	//---------------------------------------------------------vampire-------------------
	ivfstream AdvancementVampireManShadow171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_MAN_SHADOW_171").c_str(), 
		AdvancementVampireManShadow171File))
		return false;
	m_AdvancementVampireManShadow171FPK.LoadFromFile( AdvancementVampireManShadow171File );
	AdvancementVampireManShadow171File.close();

	ivfstream AdvancementVampireWoman171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_SHADOW_171").c_str(), 
		AdvancementVampireWoman171File))
		return false;
	m_AdvancementVampireWomanShadow171FPK.LoadFromFile( AdvancementVampireWoman171File );
	AdvancementVampireWoman171File.close();//201

	ivfstream AdvancementVampireManShadow201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_MAN_SHADOW_201").c_str(), 
		AdvancementVampireManShadow201File))
		return false;
	m_AdvancementVampireManShadow201FPK.LoadFromFile( AdvancementVampireManShadow201File );
	AdvancementVampireManShadow201File.close();

	ivfstream AdvancementVampireWoman201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_SHADOW_201").c_str(), 
		AdvancementVampireWoman201File))
		return false;
	m_AdvancementVampireWomanShadow201FPK.LoadFromFile( AdvancementVampireWoman201File );
	AdvancementVampireWoman201File.close();
	//-------------------------------------------------------slayer-----------------------
	ivfstream AdvancementSlayerManShadow171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_MAN_SHADOW_171").c_str(), 
		AdvancementSlayerManShadow171File))
		return false;
	m_AdvancementSlayerManShadow171FPK.LoadFromFile( AdvancementSlayerManShadow171File );
	AdvancementSlayerManShadow171File.close();

	ivfstream AdvancementSlayerWomanShadow171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_WOMAN_SHADOW_171").c_str(), 
		AdvancementSlayerWomanShadow171File))
		return false;
	m_AdvancementSlayerWomanShadow171FPK.LoadFromFile( AdvancementSlayerWomanShadow171File );
	AdvancementSlayerWomanShadow171File.close();//201 begin by rallser

	ivfstream AdvancementSlayerManShadow201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_MAN_SHADOW_201").c_str(), 
		AdvancementSlayerManShadow201File))
		return false;
	m_AdvancementSlayerManShadow201FPK.LoadFromFile( AdvancementSlayerManShadow201File );
	AdvancementSlayerManShadow201File.close();

	ivfstream AdvancementSlayerWomanShadow201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_WOMAN_SHADOW_201").c_str(), 
		AdvancementSlayerWomanShadow201File))
		return false;
	m_AdvancementSlayerWomanShadow201FPK.LoadFromFile( AdvancementSlayerWomanShadow201File );
	AdvancementSlayerWomanShadow201File.close();
	//------------------------------------------------------ousters----------------------------
	ivfstream AdvancementOustersShadow171File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_OUSTERS_SHADOW_171").c_str(), 
		AdvancementOustersShadow171File))
		return false;
	m_AdvancementOustersShadow171FPK.LoadFromFile( AdvancementOustersShadow171File );
	AdvancementOustersShadow171File.close();

	//end
	ivfstream AdvancementOustersShadow201File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_OUSTERS_SHADOW_201").c_str(), 
		AdvancementOustersShadow201File))
		return false;
	m_AdvancementOustersShadow201FPK.LoadFromFile( AdvancementOustersShadow201File );
	AdvancementOustersShadow201File.close();//rallser201
	...
	ivfstream AdvancementOusters231File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_OUSTERS_231").c_str(), 
		AdvancementOusters231File))
		return false;
	m_AdvancementOusters231FPK.LoadFromFile( AdvancementOusters231File );
	AdvancementOusters231File.close();

	ivfstream AdvancementOustersShadow231File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_OUSTERS_SHADOW_231").c_str(), 
		AdvancementOustersShadow231File))
		return false;
	m_AdvancementOustersShadow231FPK.LoadFromFile( AdvancementOustersShadow231File );
	AdvancementOustersShadow231File.close();//rallser231

	ivfstream AdvancementSlayerMan231File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_MAN_231").c_str(), 
		AdvancementSlayerMan231File))
		return false;
	m_AdvancementSlayerMan231FPK.LoadFromFile( AdvancementSlayerMan231File );
	AdvancementSlayerMan231File.close();

	ivfstream AdvancementSlayerWoman231File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_WOMAN_231").c_str(), 
		AdvancementSlayerWoman231File))
		return false;
	m_AdvancementSlayerWoman231FPK.LoadFromFile( AdvancementSlayerWoman231File );
	AdvancementSlayerWoman231File.close();

	ivfstream AdvancementSlayerManShadow231File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_MAN_SHADOW_231").c_str(), 
		AdvancementSlayerManShadow231File))
		return false;
	m_AdvancementSlayerManShadow231FPK.LoadFromFile( AdvancementSlayerManShadow231File );
	AdvancementSlayerManShadow231File.close();

	ivfstream AdvancementSlayerWomanShadow231File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_SLAYER_WOMAN_SHADOW_231").c_str(), 
		AdvancementSlayerWomanShadow231File))
		return false;
	m_AdvancementSlayerWomanShadow231FPK.LoadFromFile( AdvancementSlayerWomanShadow231File );
	AdvancementSlayerWomanShadow231File.close();


	ivfstream AdvancementVampireMan231File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_MAN_231").c_str(), 
		AdvancementVampireMan231File))
		return false;
	m_AdvancementVampireMan231FPK.LoadFromFile( AdvancementVampireMan231File );
	AdvancementVampireMan231File.close();

	ivfstream AdvancementVampireWonman231File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_WOMAN_231").c_str(), 
		AdvancementVampireWonman231File))
		return false;
	m_AdvancementVampireWoman231FPK.LoadFromFile( AdvancementVampireWonman231File );
	AdvancementVampireWonman231File.close();

	ivfstream AdvancementVampireManShadow231File;
	if (!FileOpenBinary(g_pFileDef->getProperty("FILE_CFRAME_ADVANCEMENT_CLASS_VAMPIRE_MAN_SHADOW_231").c_str(), 
		AdvancementVampireManShadow231File))
		return false;
	m_AdvancementVampireManShadow231FPK.LoadFromFile( AdvancementVampireManShadow231File );
	AdvancementVampireManShadow231File.close();

/*
	//by viva
	std::ofstream writefile;
	writefile.open("151Action_Ousters.txt", std::ios::binary);
	char strtmp[1024];
	for(int i=0; i< m_AdvancementOustersFPK[1].GetSize(); i++)
	{
		sprintf(strtmp, "%d\r\n", m_AdvancementOustersFPK[1][i][0].GetSize());
...
/*
#ifdef OUTPUT_DEBUG
	// 그래픽팀에서 Slayer작업할때 반복 Frame을 알려주기 위한 TXT를 뽑는다.
	...
	const char* strAction[] = {
			...
			"저격용(SR)",		// ACTION_SLAYER_GUN_SR			
			"소총(AG)",			// ACTION_SLAYER_GUN_AR			
			"샷건(SG)",			// ACTION_SLAYER_GUN_SG			
			"자동소총(SMG)",	// ACTION_SLAYER_GUN_SMG
			
			"검기본",			// ACTION_SLAYER_SWORD			
			"도기본",			// ACTION_SLAYER_BLADE
			"검기술",			// ACTION_SLAYER_SWORD_2		
			"검기술",			// ACTION_SLAYER_BLADE_2		
			
			"오토바이이동",		// ACTION_SLAYER_MOTOR_MOVE
			"오토바이정지",		// ACTION_SLAYER_MOTOR_STAND	
			
			"저격용(SR)SLOW",	// ACTION_SLAYER_GUN_SR_SLOW	
			"저격용(SR)FAST",	// ACTION_SLAYER_GUN_SR_FAST	
			"소총(AG)SLOW",		// ACTION_SLAYER_GUN_AR_SLOW	
			"소총(AG)FAST",		// ACTION_SLAYER_GUN_AR_FAST	
			"샷건(SG)SLOW",		// ACTION_SLAYER_GUN_SG_SLOW	
			"샷건(SG)FAST",		// ACTION_SLAYER_GUN_SG_FAST	
			"자동소총(SMG)SLOW",// ACTION_SLAYER_GUN_SMG_SLOW	
			"자동소총(SMG)FAST",// ACTION_SLAYER_GUN_SMG_FAST
			
			"검기본SLOW",		// ACTION_SLAYER_SWORD_SLOW		
			"검기본FAST",		// ACTION_SLAYER_SWORD_FAST		
			"도기본SLOW",		// ACTION_SLAYER_BLADE_SLOW		
			"도기본FAST",		// ACTION_SLAYER_BLADE_FAST		
			"검기술SLOW",		// ACTION_SLAYER_SWORD_2_SLOW	
			"검기술FAST",		// ACTION_SLAYER_SWORD_2_FAST	
			"도기술SLOW",		// ACTION_SLAYER_BLADE_2_SLOW	
			"도기술FAST",		// ACTION_SLAYER_BLADE_2_FAST

			"전투정지(총)",		// ACTION_SLAYER_BATTLE_STAND_GUN
			"전투정지(검)",		// ACTION_SLAYER_BATTLE_STAND_SWORD
			"전투정지(도)",		// ACTION_SLAYER_BATTLE_STAND_BLADE

			"이동(총)",			// ACTION_SLAYER_MOVE_GUN

			"마법시전",			// ACTION_SLAYER_MAGIC_CASTING
			"검반복",
			"도반복",
	};
...
/*/
	...
	for(i = 0; i < CFPK_TOTAL_SHAPE_NUM; ++i)
	{
		...
		for(int j = 0; j < CFPK_TOTAL_ACTION_NUM; ++j)
		{
			m_OustersShadowFPK[i][j].Init(8); // 방향
		}
	}
	for(i = 0; i < CFPK_TOTAL_SHAPE_NUM; ++i)
	{
...
//	itemdropfile3.close();
	...
	*/
	...
	///*
#if defined(OUTPUT_DEBUG) && defined(_DEBUG)
	//chyaya 2006.09.04 Item수정 작업
	const int maxSlayerItemBroken = (MSlayerGear::MAX_GEAR_SLAYER+2-15) * 3;
	const int maxVampireItemBroken = (MVampireGear::MAX_GEAR_VAMPIRE-15) * 3;
	const int maxOustersItemBroken = (MOustersGear::MAX_GEAR_OUSTERS+1-15) * 3;
	/*
...
//#if __CONTENTS(__ONIBLA)
				...
				{
					m_CreatureSPK3.LoadFromFilePart(first,last);
				}
//#endif //__ONIBLA
				// n번째 creature load	
...
//#if __CONTENTS(__ONIBLA)
					...
					{
						m_CreatureSSPK3.LoadFromFilePart(first,last);
					}
//#endif //__ONIBLA
...
//#if __CONTENTS(__ONIBLA)
			...
			{
				m_CreatureSPK3.ReleasePart(first,last);
			}
//#endif //__ONIBLA
			// 		}
...
//----------------------------------------------------------------------
// 일부의 Tile과 ImageObject를 Thread Load한다.
//----------------------------------------------------------------------
bool		
...
// 	// LargeZone - SmallZone해서 남은 것을 제거한다.
...
// 	}
// 	
// 	// LargeZoneTileID에서 SmallZoneTileID를 제거한다.
// 	LargeZoneTileID -= SmallZoneTileID;
// 	
// 	// LargeZone에만 존재하는 TileID들을 제거한다.
...
//		else
		...
		if(m_delayFrame)
		{
			...
			if(m_delayPostion & FADE_DELAY_POSTION_FRONT)
			{
				...
				{
					if( m_FadeEnd == 0 && 1 == m_FadeValue ) // 질드레 연출땜에 어두워 진담에 잠시 유지..^^;
					{
						if(g_CurrentFrame - TempFadeFrame> 16*m_delayFrame) // 입력값만큼 질드레는 5초간 유지
							m_bFade = false;
					}
					else
					{
...
//	POINT	objectPixelPoint;
	...
	sY2 = sectorPoint.y + 8;		// 거대 몬스터 때문에 더 증가시켰다.
...
//		}
	...
	//NPC 위에 캐릭터가 있을 경우 NPC가 클릭 되지 않았던 부분을 캐릭터가 가려져도 크릭 될 수 있도록 우선 순위를 NPC에게 주었음.	
	for (currentY=sY2; currentY>=sY1; currentY--)	
	{	
		for (currentX=sX1; currentX<=sX2; currentX++)	
		{				
				...
				if (numCreature > 0)
				{
					...
					for (int c=0; c<numCreature; c++, iCreature++)
					{
							...
							IsAbleToActionTarget(pCreature, actionTarget))
						{
							...
							// Creature Table에 Width 정보가 있다면
							if(pCreature->GetWidth() != -1)
							{
								// Width를 너비로 해서 마우스와 충돌체크한다.
							...
							//	&& y > rect.top && y < rect.bottom)
							{
								...
								if(pCreature->IsNPC())	//클릭한 곳에 NPC가 있으면 NPC 정보를 넘겨준다.
									return pCreature;
							}
						}
					}
							...
							IsAbleToActionTarget(pCreature, actionTarget))
						{
							...
							*/
							...
							if ((
								...
								&& !pCreature->IsCutHeight()	// 사라지는 상태면
								&& g_pObjectSelector->CanSelect(pCreature)
								&& bMouseInCreature				// 마우스가 크리처를 가르키고 있는가?
								&& !(pCreature->IsOusters() && pCreature->IsInGroundElemental() && !g_pPlayer->IsOusters()))
							//if (x > rect.left && x < rect.right
							//	&& y > rect.top && y < rect.bottom)
							{
				...
				{	
					...
					*/			
					...
					if (g_pPlayer->IsVampire()&&g_pZone->GetID() != 3001
						...
						|| !g_pPlayer->IsSlayer() && g_pPlayer->ShowInGunFiled(currentX,currentY)
						|| g_pPlayer->HasEffectStatus( EFFECTSTATUS_GHOST )
#ifdef __METROTECH_TEST__
						|| g_bLight
#endif
...
//----------------------------------------------------------------------
// 이 오브젝트가 actionTarget 속성에 비교하였을 때
// Target으로 선택이 가능한 오브젝트인가?
//----------------------------------------------------------------------
bool
MTopView::IsAbleToActionTarget(MObject* pObject, BYTE_ATTRIBUTE actionTarget)
{
	// 일단 모든 오브젝트 가능하면 true
...
// 2008.07.17 zzi - EventMessage 출력
//		EventMessage가 출력된 영역을 static으로 여기서 .. 선언해서 사용하도록 한다.
	static int EventMessageWidth = g_pUserInformation->iResolution_x / 2 + 30;
	static int EventMessageHeight = 20;	// 어차피.. 출력될 문자열의 길이에 의해~~ 조정될 것이다.
	static int EventMessageX = (g_pUserInformation->iResolution_x-EventMessageWidth)/2;
	static int EventMessageY = g_pUserInformation->iResolution_y / 4;		// 고정위치.
	...
	for (c=0; c < g_pEventMessage->GetSize(); c++)
	{
		if ( (*g_pEventMessage)[c][0] != NULL )
		{
			const COLORREF color = RGB(128, 255, 0);
			pEventInfo->text_color = color;
			EventMessageRt.top = EventMessageY - 20*c;
			EventMessageRt.bottom = EventMessageY+EventMessageHeight - 20*c;
	...
	if ( g_pInfomationBord )
		g_pInfomationBord->ShowBord(pEventInfo);

//	GMMessage 출력
	PrintInfo* pGMInfo = g_ClientPrintInfo[FONTID_LARGE_CHAT];
	for( c = 0; c< g_pGMMessage->GetSize(); c++ )
	{
...
//	strY = 40;
	...
	char message[300];
	for (c=0; c<g_pWorldChatingMessage->GetSize(); c++)
	{
		if ((*g_pWorldChatingMessage)[c][0] != NULL)
		{
			
			strcpy(message,(*g_pWorldChatingMessage)[c]);
			...
			pPrintInfo->text_color = 0;
			g_Print(11, strY+1, message, pPrintInfo);

			pPrintInfo->text_color = color;
			g_Print(10, strY, message, pPrintInfo);
...
//-
	...
	static DWORD lastWorldChatingTime = g_CurrentTime;
	if (g_CurrentTime - lastWorldChatingTime >= (g_pClientConfig->DELAY_SYSTEMMESSAGE + 5000) )
	{
		g_pWorldChatingMessage->Add("\0");
		lastWorldChatingTime = g_CurrentTime;
	}
	...
	{
	case RACE_SLAYER:
		strX = g_pUserInformation->iResolution_x-245;
		strY = g_pUserInformation->iResolution_y-300;
		break;
...
MTopView::DrawZone(int firstPointX,int firstPointY)
{
	...
	// 이름과 정보를 표시하는가?
	//-------------------------------------------------
	BOOL bDrawCreatureMyName	= FALSE;	// 내 이름
	//BOOL bDrawCreatureName	= FALSE;	// 남 이름
	...
	*/
	...
	while (bDrawBackGround && iImageObjectOutput0 != m_mapImageObject.end())
	{
		...
		if (pImageObject->GetViewpoint() == 0)
		{
			...
			if ( pImageObject->GetSpriteID() < 65535 )						
	...
	if (m_bTileSearchForCreature)
	{
		for (y=sY1; y<=sY2; y++)
		{
			...
			{	
				...
				int GunFiledCount = g_pPlayer->GetGunFiledCount();
				if (GunFiledCount < 0
					|| max(abs(g_pPlayer->GetX()-x), abs(g_pPlayer->GetY()-y)) <= GunFiledCount+2)
				{			
					const MSector& sector = m_pZone->GetSector(x,y);
						...
						if (numCreature > 0)
						{
							...
							for (int c=0; c<numCreature; c++, iCreature++)
							{
								...
								if (iCreature->first >= MSector::POSITION_GROUNDCREATURE)
								{
									...
									if (
										(pCreature==g_pPlayer
										|| g_pPlayer->ShowInGunFiled(pCreature->GetX(), pCreature->GetY())) &&
										!pCreature->IsFakeCreature()										
										)
									{											
										point.x = pCreature->GetPixelX() - m_FirstZonePixel.x;
		...
		{				
			...
			{	
				...
				{				
					...
					if ((pItem != NULL
						&& g_pPlayer->ShowInDarkness(x, y)) ||(pItem != NULL && g_pPlayer->ShowInGunFiled(x,y)))
					{		
						//----------------------------------------
						// 시체인 경우
				...
				{	
					...
					{				
						...
						if ((pItem != NULL
							...
							(pItem != NULL && g_pPlayer->ShowInGunFiled(x,y)))
						{		
							//----------------------------------------
							// 시체인 경우
							// Effect를 출력하고 나서 출력한다.
...
//							DrawNameCreatureVec.push_back(std::make_pair(pCreature, 0));
	...
	// m_mapImageObject에는 Viewpoint가 key값이 되어서 Sort되어 있다.
	//------------------------------------------------------
	while (bDrawBackGround && iImageObjectOutput != m_mapImageObject.end() && m_pZone->GetID()!=7000 && m_pZone->GetID()!=7001)
	{
		MImageObject* const pImageObject = (MImageObject* const)((*iImageObjectOutput).second);
...
/*		if ( pImageObject->GetSpriteID() != 2814 && pImageObject->GetSpriteID() != 2815 && pImageObject->GetSpriteID() != 2819 && pImageObject->GetSpriteID() != 2820
			 && pImageObject->GetSpriteID() != 2822 && pImageObject->GetSpriteID() != 2822
			 && pImageObject->GetSpriteID() != 2832 && pImageObject->GetSpriteID() != 2833 && pImageObject->GetSpriteID() != 2834 && pImageObject->GetSpriteID() != 2835)	*/			
			DrawImageObject(&point, pImageObject);
...
//			}
		...
		*/
		...
		if((g_pZone->GetID() != 1006 || !g_pPlayer->IsInSafeSector()) &&
			!g_pZone->IsSurvivalZone() &&	// 서바이벌 경기장은 무조건 밝다. Very Hard -_-;;
			g_pZone->GetID() != 1311 &&			// 아우스터즈 마을을 무조건 밝다.
			!g_pPlayer->HasEffectStatus( EFFECTSTATUS_GHOST ))
		{
			//----------------------------------------------------------------
			//
...
MTopView::DrawTileSurface()
{
	...
	//std::ofstream file("Error.log", std::ios::out | std::ios::app);
	//file << "*** 댄轎 X=" << g_pPlayer->GetX() << ", Y=" << g_pPlayer->GetY() << ", SX=" << g_pPlayer->GetSX() << ", SY=" << g_pPlayer->GetSY()<< std::endl;
	//file.close();
...
MTopView::DrawPartyHP(POINT* pPoint, MCreature* pCreature)
{
	...
	// HP계산
	//-----------------------------------------------------
	HP_t currentHP		= pCreature->GetHP();
	HP_t maxHP			= pCreature->GetMAX_HP();
...
void	MTopView::SetTitleEffectPos(POINT* pPoint, int iIndex)	//이펙트 위치 변경
{
	std::vector<S_TITLE_UI_EFFECT>::iterator it	= m_TitleUI_EFFECT_Info.begin();
	for(int i = 0; i < m_TitleUI_EFFECT_Info.size(); i++, it++)
	{
...
//튕긴다는 이야기에 질린다.. 어떤 경우에도 튕길 만한 인덱스라면 패스하자. 이래도 튕기면 누군가 침범하는겨...
	...
	if (spriteID != SPRITEID_NULL)
	{
		...
		point.y += Frame.GetCY();
//DRAW_ALPHASPRITEPAL(&point, spriteID, m_EffectAlphaSPK, m_EffectAlphaPPK[_FrameID])//, m_EffectAlphaSPKI, m_EffectAlphaSPKFile)
//위 코드로 실행 되던 녀석 튕김 현상을 잡아 보기 위해서 매크로를 아래와 같이 가져와 사용...
		CAlphaSpritePal* pSprite = &m_EffectAlphaSPK[spriteID];
		m_pSurface->BltAlphaSpritePal(&point, pSprite, m_EffectAlphaPPK[_FrameID]);
	}
	...
	if (spriteID != SPRITEID_NULL)
	{
		point.x += Frame_Reverse.GetCX();
		point.y += Frame_Reverse.GetCY();
		CAlphaSpritePal* pSprite = &m_EffectAlphaSPK[spriteID];
		m_pSurface->BltAlphaSpritePal(&point, pSprite, m_EffectAlphaPPK[_FrameID]);
	}
}
...
//					}
			...
			if(!(pCreature->GetCreatureType() >= 371 && pCreature->GetCreatureType() <= 376 || pCreature->GetCreatureType() >= 560 && pCreature->GetCreatureType() <= 563 || pCreature->GetCreatureType() >= 526 && pCreature->GetCreatureType() <= 549 || pCreature->GetCreatureType() == 672 || pCreature->GetCreatureType() == 673 ))
			{
				if(pCreature->GetCreatureType() == 482 ||
				   ...
				   pCreature->GetCreatureType() == 819)	// 트리는 select안되게
				{
					RECT rect;
					rect.left	= orPoint.x;
					rect.top	= orPoint.y;
	...
	else
	{
		...
		*/
		...
		#endif
			int ID =  Frame.GetSpriteID()  ;
			//if (ID == 1599)
			//{
			//	int a = 100;
			//}
		 //if(ID==1597 || ID==1598 || ID==1599)
		 //{
			// ID = 1581;	 //1597 1598 1599 3몸暠튬鞫刻굼웰
		 //}
		CIndexSprite* pSprite = &m_ItemTileISPK[ ID ];		//bug췄甘：侶몸ID角膠틔딜쭝瞳뒈충�龜豚쓸켐셕� 1599侶몸暠튬鞫刻앎굼웰，뎃角痰묏야옵鹿꿴였1599돨暠튬鞫刻攣횅，와빵똥侶쟁pSrite轟랬셕炬굼웰
			...
			//---------------------------------------------------------
			{
				...
				if (m_SelectItemID == pItem->GetID())
				{
					...
					if (
							...
							|| pCreature->GetCreatureType() == 819	// 터프한 눈사람
...
/*
	...
	*/
			   ...
			   g_pOperatorOption->bShowImageObjectInfo)
			{
				...
				//메모리 충돌 남 언롹 롹 주석 처리 함 by diesirace
				//m_pSurface->Unlock();
				m_pSurface->GDI_Text(pPoint->x, pPoint->y, str, 0xFFFF);
				//m_pSurface->Lock();
			}
...
{	
	...
	for (int i=0; i<size; i++)	
	{
			...
			)
		{
			...
			// 또한 뱀파이어 고스트 일 경우에 마스터 이팩트를 제거한다.
...
//				pCreature->IsHallu() ||
			...
			if(pCreature->HasEffectStatus(EFFECTSTATUS_INSTALL_MK1))
			{
				if(frameID != EFFECTSPRITETYPE_unknow_2349 &&
					TempSecreenEffect != EFFECTSPRITETYPE_unknow_2349)
				{
					iEffect++;
					continue;
				}
...
//				}
//			}
			//20100716 by svi 瞳긴�箴늬�苟꼇鞫刻區唐교밟
			bool b_tmpIsRareSpriteType = (
					frameID == (*g_pEffectStatusTable)[EFFECTSTATUS_MASTER_EFFECT_ANGEL_FLUTTER].EffectSpriteType ||
					frameID == (*g_pEffectStatusTable)[EFFECTSTATUS_MASTER_EFFECT_SHADOW_OF_BLOODY_DARKNESS].EffectSpriteType ||
					frameID == (*g_pEffectStatusTable)[EFFECTSTATUS_MASTER_EFFECT_ARIEL_BREATHING].EffectSpriteType 
				);
			bool b_tmpIsFastMoving = (
					...
					pCreature->GetCreatureType() == CREATURETYPE_NEDE ||
					pCreature->GetCreatureType() == CREATURETYPE_KLTL ||
					pCreature->GetCreatureType() == CREATURETYPE_SHAPE_OF_DEMON ||
					//pCreature->GetCreatureType() == CREATURETYPE_INSTALL_MK1 ||
					pCreature->GetMoveDevice() == MCreature::MOVE_DEVICE_RIDE ||
					pCreature->GetMoveDevice() == MCreature::MOVE_DEVICE_SUMMON_SYLPH ||
					pCreature->GetMoveDevice() == MCreature::MOVE_DEVICE_WING_SYLPH
				);
			if(b_tmpIsRareSpriteType && b_tmpIsFastMoving)
			{
				iEffect++;
				continue;
			}
			...
			switch ((*iEffect)->GetBltType())
			{
				...
				case BLT_EFFECT :
				{
					...
					if (aest!=FRAMEID_NULL)
					{
							...
							aest == ACTIONEFFECTSPRITETYPE_EXTREME_FEMALE )
						{
							if( GET_ACTION_EFFECTSPRITETYPE_FRAMEID( aest, action ) == EFFECTSPRITETYPE_NULL ||
								...
								pCreature->GetCreatureType() == CREATURETYPE_VAMPIRE_GHOST ||
								pCreature->GetCreatureType() == CREATURETYPE_WOLF ||
								pCreature->GetCreatureType() == CREATURETYPE_WER_WOLF||//긴의빈땡鱗
					...
					bool    bLarStoke		= est >= EFFECTSPRITETYPE_unknow_2298 && est <= EFFECTSPRITETYPE_unknow_2303;
#if __CONTENTS(__SECOND_TRANSFORTER)
					bool	bUnicorn		= (pCreature->GetAction() ==ACTION_OUSTERS_UNICORN_STAND);
#endif //__SECOND_TRANSFORTER
										
					if( bRediance || bLarSlash   || bLarStoke
#if __CONTENTS(__SECOND_TRANSFORTER)
						|| bUnicorn
#endif //__SECOND_TRANSFORTER
						)
					{						
						...
						//else if( bLarStoke && (pCreature->GetAction() == ACTION_SLAYER_SWORD ||  pCreature->GetAction() == ACTION_SLAYER_SWORD_FAST || pCreature->GetAction() == ACTION_SLAYER_SWORD_SLOW) ||
						//	pCreature->GetAction() == ACTION_SLAYER_SWORD_2 || pCreature->GetAction() == ACTION_SLAYER_SWORD_2_SLOW || pCreature->GetAction() == ACTION_SLAYER_SWORD_2_FAST )
						//{
						//	frame = max(0,min(pCreature->GetActionCount(),pEffect->GetMaxFrame()-1) );
						//}
...
////															m_EffectAlphaSPKFile)
						...
						|| type==2 && bFrameBackground)
					{
						...
						if (sprite!=SPRITEID_NULL)
						{
							...
							AFFECT_ORBIT_EFFECT_POSITION( pEffect, pointTemp )
							if(frameID == 2311 || frameID == 2858)
							{
								CAlphaSpritePal* pSprite = &m_EffectAlphaSPK[sprite];
								m_pSurface->BltAlphaSpritePalAlpha(&pointTemp,pSprite,8,m_EffectAlphaPPK[frameID])   ;
							}
							else
							{
							DRAW_ALPHASPRITEPAL(&pointTemp, 
...
//	return;
	...
	*/
	...
	case RACE_SLAYER:
	{
		...
		pGear = g_pSlayerGear;
		//chyaya 2006.09.04 Cue Of Adam 관련 수정 작업 begin
		toSomewhatBroken = MSlayerGear::MAX_GEAR_SLAYER + 2-15;
		//toSomewhatBroken = MSlayerGear::MAX_GEAR_SLAYER + 2-13;
		//toSomewhatBroken = MSlayerGear::MAX_GEAR_SLAYER + 2-12;
		//chyaya 2006.09.04 Cue Of Adam 관련 수정 작업 end
	...
	case RACE_VAMPIRE:
	{
		...
		//chyaya 2006.09.04 Cue Of Adam 관련 수정 작업 begin
		toSomewhatBroken = MVampireGear::MAX_GEAR_VAMPIRE-15;
	...
	case RACE_OUSTERS:
		{
			...
			//chyaya 2006.09.04 Cue Of Adam 관련 수정 작업 begin
			toSomewhatBroken = MOustersGear::MAX_GEAR_OUSTERS + 1-15;
			...
			{			
				...
				if (g_pPlayer->IsSlayer())
				{
					...
					{				
						...
						if (pItem->IsGunItem())
						{
							//chyaya 2006.09.04 Item수정 작업 begin
							frameID = MSlayerGear::MAX_GEAR_SLAYER + 1 - 15;
							//frameID = MSlayerGear::MAX_GEAR_SLAYER + 1 - 13;
							//frameID = MSlayerGear::MAX_GEAR_SLAYER + 1 - 12;
							//chyaya 2006.09.04 Item수정 작업 end
						}
						// 십자가?
				...
				else if (g_pPlayer->IsOusters() )
				{
					if( i == MOustersGear::GEAR_OUSTERS_RIGHTHAND )
					{
						if( pItem->GetItemClass() == ITEM_CLASS_OUSTERS_WRISTLET )
							//chyaya 2006.09.04 Item수정 작업 begin
							frameID = MOustersGear::MAX_GEAR_OUSTERS -15;
...
{	
	...
	if( creatureType >= 726 &&
		creatureType <= 729)
		return FALSE;
	if(creatureType == 1704 || creatureType == 1714)
		return FALSE;//뒈掉裂쳔꼇鞫刻츰俚
	...
	//-----------------------------------------------------
 	else if (!pCreature->IsPlayerOnly())
 	{
		...
		else
		{
			...
			if(levelGap < -30)
			{
				color = RGB(217, 217, 217);	// 회색 - 쉬움
				monsterlevlecolor = false;
			}
			else if(levelGap < -11)
			{
				color = RGB(6, 216, 86);	// 연두색 - 약간 쉬움
				monsterlevlecolor = false;
			}
			else if(levelGap < 10)
			{
				color = RGB(239, 228, 3);	// 노랑색 - 보통
				monsterlevlecolor = true;
			}
			else if(levelGap < 29)
			{
				color = RGB(255, 168, 41);	// 주황색 - 약간 어려움
				monsterlevlecolor = true;
			}
			else
			{
				color = RGB(228, 28, 204);	// 보라색 - 어려움
				monsterlevlecolor = true;
			}
		}
...
//		if(g_pObjectSelector->CanAttack( pCreature ) )
	...
	if (pName!=NULL)
	{
		...
		if(bDrawName)
		{
				...
				*/
				...
				if (guildID > 0)
				{
					...
					else
					{
						...
						//	RECT rcRect;
						//	rcRect.left = pointTemp.x;
						//	if ( rcRect.left < 0 )
						//		rcRect.left = 0;

						//	rcRect.right = pointTemp.x + 22;
						//	if ( rcRect.right < 0 )
						//		rcRect.right = 0;

						//	rcRect.top = pointTemp.y;
						//	if ( rcRect.top < 0 )
						//		rcRect.top = 0;

						//	rcRect.bottom = pointTemp.y + 20;
						//	if ( rcRect.bottom < 0 )
						//		rcRect.bottom = 0;

						//	m_pSurface->BltColorAlpha(&rcRect, 0x0000, 0);				

						//	m_pSurface->Unlock();
						//const int FontHeight = g_pClientConfig->FONT_HEIGHT;
		    //            const int FontHeight2 = FontHeight << 1;
						//const COLORREF levelNameColor = RGB(200, 250, 200);
						////const COLORREF levelNameColor = RGB(0, 0, 0);
			   //         int yPoint_40 = yPoint - FontHeight2 - 3;
						////m_pSurface->Lock();
						//static char szCreatreLevel[MAX_PATH];
						//sprintf(szCreatreLevel, "%d", pCreature->GetLevel());

						//int x = pointTemp.x;
						//int y = pointTemp.y + 5;
						//
						//int nLen = strlen(szCreatreLevel);
						//switch( nLen )
						//{
						//case 1:
						//	x += 7;
						...
						//DRAWTEXT_NODE* pMonsterLevelNode = new DRAWTEXT_NODE (
						//			x,
						//			y,//2+1,
						//			szCreatreLevel,
						//			levelNameColor,
						...
						//AddText( pMonsterLevelNode );
							CSprite* pSprite;
							if(monsterlevlecolor == false)
							{
								pSprite = g_pGuildMarkManager->SetMonsterLevelMarkGreen();
							}
							else if(monsterlevlecolor == true)
							{
								pSprite = g_pGuildMarkManager->SetMonsterLevelMarkRed();
							}
							...
							{			
								...
								POINT pointTemp = { rectLeft-25, yPoint_20 };
								...
								char sz_tempHP[MAX_PATH];
								char sz_HpNum[MAX_PATH];

								wsprintf(sz_tempHP,"%d",pCreature->GetLevel());
								wsprintf(sz_HpNum,"HP:%d",currentHP);

								int nFontWidth = g_GetStringWidth(sz_tempHP,gpC_base->m_chatting_pi.hfont);
								int Fwidth = (25-nFontWidth)/2;

								int nHpFontWidth = g_GetStringWidth(sz_HpNum,gpC_base->m_user_id_pi.hfont);
								int HpFwidth = (MAX_HP_BAR+25-nHpFontWidth)/2;

								g_PrintColorStr(x+Fwidth, y+4, sz_tempHP, gpC_base->m_chatting_pi, color);
								g_PrintColorStr(x+HpFwidth, y-20, sz_HpNum, gpC_base->m_user_id_pi, RGB_GREEN);
			...
			// HP계산
			//-----------------------------------------------------
			DWORD currentHP		= max(0,DWORD(pCreature->GetHP()));
			DWORD maxHP			= pCreature->GetMAX_HP();
...
//#endif
			...
			{
				if(yPoint-42<0) return FALSE; // 걍 찍지 말자..ㅎㅎ
...
void//m_bDrawRequest를 사용 하기 전에 반드시 false로 만들어 놓을것 만약에라도 그림이 그려지지 않는 경우를 생각해서...
...
//----------------------------------------------------------------------
// DrawCreatureName에서 이름,정보를 출력하지만
// 텍스트는 모아서 출력되기 때문에
// 그보다 위에 출력되어야 할 것만 따로 모아 텍스트 위에 찍는다.
// 지금은 출신마크 밖에 없음 -_-);;
//----------------------------------------------------------------------
void
MTopView::DrawCreatureNameOverText(POINT* pPoint, MCreature* pCreature)
{
		...
		)
	{
		...
		if(!pCreature->IsPlayer())
		{
			...
			pointTemp.x		= rectLeft + ORIGIN_MARK_X_FIX;						// 왼쪽
			pointTemp.y		= yPoint_20 + ORIGIN_MARK_Y_FIX;
			m_pSurface->BltSpriteColorSet(&pointTemp, pSprite, 80);
			
			pointTemp.x		= rectRight-ORIGIN_MARK_WIDTH-ORIGIN_MARK_X_FIX;	// 오른쪽
			pointTemp.y		= yPoint_20 + ORIGIN_MARK_Y_FIX;
			m_pSurface->BltSpriteColorSet(&pointTemp, pSprite, 80);
...
//		if(pCreature->GetClassType() == MCreature::CLASS_PLAYER)	// 플레이어인 경우에만
			...
			g_pPlayer->GetRace() == pCreature->GetRace() )
		{
			...
			// 2005.12.23 chyaya
			// 출신마크 출력
			// 기존 몰다비아, 왈라키아, 히스티리아 출신의 캐릭터만 출력한다.
			//
...
// DrawCreatureName
//----------------------------------------------------------------------
// DrawNameCreatureInfo를 파라미터로 사용하는 DrawCreatureName()
//----------------------------------------------------------------------
...
// DrawCreatureNameOverText
//----------------------------------------------------------------------
// DrawNameCreatureInfo를 파라미터로 사용하는 DrawCreatureNameOverText()
//----------------------------------------------------------------------
...
MTopView::DrawUndergroundCreature(POINT *pPoint, MCreature *pCreature)
{
	...
	// 크리처 이름을 출력하자 
	//---------------------------------------- 	
	if(pCreature->IsShowNameCreature())	// 대지 정령의 뿔
	{
...
{	
	...
	if (pCreatureName!=NULL)
	{
		...
		// HP계산
		//-----------------------------------------------------
		HP_t currentHP		= max(0,int(g_pPlayer->GetHP()));
		HP_t maxHP			= g_pPlayer->GetMAX_HP();
...
void MTopView::VampireComeFromPortalNonTurn(MCreature* pCreature)
{
	...
	else if(creatureType >= 1083 && creatureType <= 1085)		// 녘膠梁뻥땡鱗
	{
		summonActionResult = SKILL_CLIENT_CATS_SUMMON;
	}
	else if(creatureType >= 1129 && creatureType <= 1132)		// 마시마로
	{
		summonActionResult = SKILL_CLIENT_RUFFIAN_SUMMON;
	}
	else if(creatureType == 1183)		// 직책뭍
	{
		summonActionResult = SKILL_CLIENT_BEARBOX_SUMMON;
	}
	else if(creatureType == 1359 || creatureType == 1560 || creatureType == 1562 || creatureType == 1563 || creatureType == 1564 || creatureType == 1881 || creatureType == 1897 || creatureType == 1898 || creatureType == 2016)		// 직책뭍
	{
		summonActionResult = SKILL_CLIENT_CATS_SUMMON;
	}
#endif //__PET_CATS
...
#endif //__PET_MASHIMARO
#if __CONTENTS(__PET_CATS)
	else if(creatureType >= 1083 && creatureType <= 1085)		// 마시마로
	{
		summonActionResult = SKILL_CLIENT_CATS_SUMMON;
	}
		else if(creatureType >= 1129 && creatureType <= 1132)		// 마시마로
...
//#if __CONTENTS(__ONIBLA)
	else if (body>286 && body<409)
	{
					return m_CreatureSPK2[spriteIdx];
	}
    else
	{
				return m_CreatureSPK3[spriteIdx];
	}
//#endif //__ONIBLA
}
...
//#if __CONTENTS(__ONIBLA)
	...
	{
		return m_CreatureSSPK3[spriteIdx];
	}
//#endif //__ONIBLA
}