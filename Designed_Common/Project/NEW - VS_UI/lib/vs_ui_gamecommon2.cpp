	...
	{		
		...
		if( !m_ItemList.empty() )
		{
			if(pCurrentFocusItem != NULL )
			{
				...
				else if(pCurrentFocusItem->GetItemOptionExtColorSet() >= 495)
...
C_VS_UI_REMOVE_OPTION::IsCanRemoveOption_Puritas(const MItem* pItem, const MItem* pTItem)
{
	...
	if( pItem == NULL || pTItem == NULL ||
		...
		pItem->GetItemClass() == ITEM_CLASS_MIXING_ITEM && (pItem->GetItemType() != 18 && pItem->GetItemType() != 25) &&pTItem->GetItemClass() != ITEM_CLASS_PET_ITEM && pTItem->GetItemOptionListCount() != 2 || // 일반적으로 2옵션중 하나를 삭제 한다.
		pItem->GetItemClass() == ITEM_CLASS_MIXING_ITEM && (pItem->GetItemType() == 18 || pItem->GetItemType() == 25) &&pTItem->GetItemClass() != ITEM_CLASS_PET_ITEM && pTItem->GetItemOptionListCount() != 1||
		!(
		pItem->GetItemClass() == ITEM_CLASS_MIXING_ITEM && 
		(pItem->GetItemType() >= 9 && pItem->GetItemType() <= 18 || pItem->GetItemType() >= 22 && pItem->GetItemType() <= 25)
		)
		)
	{
...
assert(pItem);
	...
	{
		m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(close_button_offset_x, close_button_offset_y + 30 , gpC_global_resource->m_pC_assemble_box_button_spk->GetWidth(C_GLOBAL_RESOURCE::AB_BUTTON_X), gpC_global_resource->m_pC_assemble_box_button_spk->GetHeight(C_GLOBAL_RESOURCE::AB_BUTTON_X), CLOSE_ID, this, C_GLOBAL_RESOURCE::AB_BUTTON_X));
		m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(alpha_button_offset_x, alpha_button_offset_y + 30 , gpC_global_resource->m_pC_assemble_box_button_spk->GetWidth(C_GLOBAL_RESOURCE::AB_BUTTON_ALPHA), gpC_global_resource->m_pC_assemble_box_button_spk->GetHeight(C_GLOBAL_RESOURCE::AB_BUTTON_ALPHA), ALPHA_ID, this, C_GLOBAL_RESOURCE::AB_BUTTON_ALPHA));	
	}
}
		...
		while (m_MultiPackItem->IsNotEnd())
		{
			...
			if(p_item)
			{
				...
				if (p_selected_item && (p_selected_item->GetID() == p_item->GetID()))
				{
					...
					else if(const_cast<MItem *>(p_item)->GetItemOptionColorSet() >= 495)
