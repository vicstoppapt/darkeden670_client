...
C_VS_UI_BOOKCASE::C_VS_UI_BOOKCASE()
{
	...
	w_x = g_pUserInformation->iResolution_x/2 - m_pC_bookcase_spk->GetWidth(BOOKCASE_WINDOW)/2;
	w_y = 50;
...
C_VS_UI_BOOK::C_VS_UI_BOOK(const char *szFilename, int row, int col):C_VS_UI_DESC()
{
	...
	w_x = g_pUserInformation->iResolution_x/2 - m_pC_book_spk->GetWidth(BOOK_WINDOW)/2;
	w_y = 30;
...
C_VS_UI_COMPUTER::C_VS_UI_COMPUTER():C_VS_UI_DESC()
{
	...
	w_x = g_pUserInformation->iResolution_x/2 - m_pC_monitor_spk->GetWidth(MONITOR_WINDOW)/2;
	w_y = 30;
...
//	ZeroMemory(szLine, dSTRING_LEN);
	...
	if( pImageFileNameElement != NULL )
	{
		...
		//if (access(spkname.c_str(), 0))	_Error(FILE_OPEN);
		temp->LoadFromFile( spkname.c_str() );
