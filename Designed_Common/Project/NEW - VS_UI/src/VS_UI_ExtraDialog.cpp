...
//		digit_count = 0;
	...
	int digit_count = 0;
	int number = max_val;
	for (; number > 0; number /= 10, digit_count++);
...
*/
	...
	m_sz_question_msg[7][11]=(*g_pGameStringTable)[UI_STRING_MESSAGE_TUNING_CONFIRM1].GetString();
	m_sz_question_msg[7][12]=(*g_pGameStringTable)[UI_STRING_MESSAGE_TUNING_CONFIRM2].GetString();
	m_sz_question_msg[7][13]=(*g_pGameStringTable)[UI_STRING_MESSAGE_TUNING_CONFIRM3].GetString();
	m_sz_question_msg[7][14]=(*g_pGameStringTable)[UI_STRING_MESSAGE_TUNING_CONFIRM4].GetString();
#endif //__TUNING_ITEM

#if __CONTENTS(__PET_MIXINGFORGE)
	m_sz_question_msg[7][15]=(*g_pGameStringTable)[UI_STRING_MESSAGE_REMOVE_PET_MIXINGOPTION].GetString();
	...
	m_sz_question_msg[21][0]=(*g_pGameStringTable)[UI_STRING_MESSAGE_USE_ITEM_DEL].GetString();
	m_sz_question_msg[21][1]=(*g_pGameStringTable)[UI_STRING_MESSAGE_USE_ITEM].GetString();


	m_sz_question_msg[22][0]=(*g_pGameStringTable)[UI_STRING_MESSAGE_PKSERVER_AGREEMENT].GetString();
	m_sz_question_msg[22][1]="";

	m_sz_question_msg[23][0]=(*g_pGameStringTable)[UI_STRING_MESSAGE_DELEGATION_OF_POWER].GetString();
	m_sz_question_msg[23][1]="";

	m_sz_question_msg[24][0]=(*g_pGameStringTable)[UI_STRING_MESSAGE_ASK_DIALOG_LOWINITSTATUS].GetString();
	m_sz_question_msg[24][1]="";
	
	m_sz_question_msg[25][0]=(*g_pGameStringTable)[UI_STRING_MESSAGE_ASK_DIALOG_USE_SKILL_BOOK].GetString();
	m_sz_question_msg[25][1]="";

	m_sz_question_msg[26][0]=(*g_pGameStringTable)[UI_STRING_MESSAGE_ENCHANT_CONFIRM].GetString();
	m_sz_question_msg[26][1]=(*g_pGameStringTable)[UI_STRING_MESSAGE_ENCHANT_CONFIRM].GetString();
	m_sz_question_msg[26][2]=(*g_pGameStringTable)[UI_STRING_MESSAGE_ENCHANT_CONFIRM].GetString();
#if __CONTENTS(__REPAIRSYSTEM_RENEWAL)
	m_sz_question_msg[28][0]=(*g_pGameStringTable)[UI_STRING_MESSAGE_ALL_PRICE].GetString();
	m_sz_question_msg[28][1]=(*g_pGameStringTable)[UI_STRING_MESSAGE_REPAIR_CONFIRM].GetString();
							...
							C_VS_UI_DIALOG(_x, _y, center_x, center_y, exec_fp, dd_button)
{
	...
	if(type == ASK_ENCHANT&&(value==10||value==11))
	{
		m_button_yPos = 18;
	}
#endif //__TUNING_ITEM
...
//		break;
		...
		case ASK_PARTY_REQUEST :
		{
			...
			m_sz_question_msg_temp[0] = new char [strlen(m_sz_question_msg[type][0])+strlen(pName)+strlen(PartyName)];
...
#if __CONTENTS(__PET_MIXINGFORGE)
			case 14:
				m_sz_question_msg_temp[0] = new char [strlen(m_sz_question_msg[type][15])+1];
				sprintf(m_sz_question_msg_temp[0], m_sz_question_msg[type][15]);
				SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);	
				break;
#endif //__PET_MIXINGFORGE
			}
		...
		case ASK_USE_DYE_POTION : 
			...
			{
				m_sz_question_msg[18][0] =	(*g_pGameStringTable)[UI_STRING_MESSAGE_USE_ITEM].GetString();
				SetMessage(m_sz_question_msg[type], 1, SMO_NOFIT);	
			}		
			break;

		case ASK_USE_EFFECT_ITEM : 
			if(value == 1)
			{
				m_sz_question_msg_temp[0] = new char [strlen(m_sz_question_msg[type][1])+1];
				sprintf(m_sz_question_msg_temp[0], m_sz_question_msg[type][1]); 
				SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
				break ; 
			}
			else
		...
		case ASK_DELEGATION_OF_POWER:
			{
				const char* pName		= (const char*)m_pTemporayValue;

				m_sz_question_msg_temp[0] = new char [strlen(m_sz_question_msg[type][0])+strlen(pName)+ strlen(pName) + 10];
				sprintf(m_sz_question_msg_temp[0], m_sz_question_msg[type][0],
		...
		case ASK_DYEPOTION_INITPOTION:
			m_sz_question_msg_temp[0] = new char [strlen(m_sz_question_msg[type][0])+20];
			sprintf(m_sz_question_msg_temp[0], m_sz_question_msg[type][0],(char*) pValue , value, value);
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);
			break;

		case ASK_USE_SKILL_BOOK:
			m_sz_question_msg_temp[0] = new char [strlen( m_sz_question_msg[type][0])+1];
			sprintf(m_sz_question_msg_temp[0], m_sz_question_msg[type][0]);
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT );
			break;

		case ASK_THIRD_ENCHANT:
			m_sz_question_msg_temp[0] = new char [strlen( m_sz_question_msg[type][value])+1];
			sprintf(m_sz_question_msg_temp[0], m_sz_question_msg[type][value]);
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT );
			break;
#if __CONTENTS(__REPAIRSYSTEM_RENEWAL)
		case ASK_REPAIRITEMALL:
			//수리비는 소지금 한도금액인 20억을 넘을수 없겠지? 10의 자리를 사용 한다. 따라서 8<x<16이기 땜시 포함하는 최소값 16으로 세팅
			char temp[16];
			itoa(value, temp, 10);
			m_sz_question_msg_temp[0] = new char [strlen( m_sz_question_msg[type][0])+16+1];
			sprintf(m_sz_question_msg_temp[0], m_sz_question_msg[type][0], temp);
...
C_VS_UI_NPC_DIALOG::C_VS_UI_NPC_DIALOG(void (*exec_fp)(C_VS_UI_DIALOG *, id_t), int sprite_num, const char* pname, WORD dd_button):
							C_VS_UI_DIALOG(-1, -1, 349, 388, exec_fp, dd_button)//C_VS_UI_DIALOG(-1, -1, 6, 3, exec_fp, dd_button)
{
	InitNpcDlg(sprite_num, pname);//속潼NPC눗왯
}
...
void C_VS_UI_NPC_DIALOG::InitNpcDlg(int sprite_num, const char* pname , bool IsShowItem)
{
	...
	//		m_iSprite = // g_npc_num[i];	// + GUARD
		...
		if(m_iSprite != -1)
		{
			...
			//assert(re);
			re = m_p_NPCFace.LoadFromFileData( 0, m_iSprite, SPK_NPCFACE, SPKI_NPCFACE );
			assert(re);
...
//			assert(re);
	...
	m_bIf_End_InventoryPopUp = false;
	m_Picture_Alpha = 10;
...
#if __CONTENTS(__DYNAMIC_UI)
void C_VS_UI_NPC_DIALOG::Run(id_t id)
{
	switch(id)
	{
		...
		default:
			C_VS_UI_DIALOG::Run(id);
	}
}

BOOL C_VS_UI_NPC_DIALOG::IsDynamicEnd()
{
	return (m_tx >= (g_pUserOption->Resolution1024?1024:800)+30);
}
#endif //__DYNAMIC_UI
...
//	{
//		gC_vs_ui.UnProgressFlg();
//		gC_vs_ui.ClosePCTalkDlg();
//
//		// Finish()시에 윈도우가 재편성되면서 MessageDlg가 떠 있을경우 반짝이는 문제 때문에
//		if(gC_vs_ui.Is_ActiveMessageDlg()==false)
//		{
//			Finish();
//
//			if(m_bIf_End_InventoryPopUp)
//			{
...
//
//	if(m_p_NPCFace.GetSize()>0)
//	{
//		m_client_rect.x += (m_p_NPCFace[0].GetWidth()+20);
//		m_client_rect.w -= (m_p_NPCFace[0].GetWidth()+20);
//	}
//
//	m_menu_rect.x = m_client_rect.x;
//	m_menu_rect.w = m_client_rect.w;
//
//	C_VS_UI_DIALOG::SetTx(m_tx); // 부모클레스에서 구현되는 타이핑 효과때문에
//#endif //__DYNAMIC_UI



	C_VS_UI_DIALOG::ShowNpcNew();
	...
	if(m_iSprite != -1)
	{
		...
		int _x = x + 40, _y = y + 40;
#else
		int _x = m_client_rect.x - m_p_NPCFace[0].GetWidth()-10-35, _y = m_client_rect.y-29;
#endif //__DYNAMIC_UI
		...
		if(m_id < (*g_pCreatureTable).GetSize())
		{
			...
			if(str && strcmp(str, ""))g_PrintColorStr(_x + m_p_NPCFace[0].GetWidth()+15, _y+4, str, gpC_base->m_info_pi, RGB_YELLOW);
		}
	}
}
...
#if __CONTENTS(__TUNING_ITEM)
			if((*g_pItemTable)[p_item->GetItemClass()][p_item->GetItemType()].DropItemNameTag)		// ItemNameTag?
			{
				title_color  = (*g_pItemTable)[p_item->GetItemClass()][p_item->GetItemType()].DropItemNameTag;
			} else
#endif //__TUNING_ITEM
...
//			m_rep_string.push_back(temp_string);
			...
			if (p_item->GetMaxDurability() != -1&&!p_item->IsUniqueItem()&& p_item->GetItemClass() != ITEM_CLASS_VAMPIRE_AMULET
				...
				&&  p_item->GetItemClass() != ITEM_CLASS_OUSTERS_HARMONIC_PENDENT && !p_item->IsQuestItem())
			{
...
//			}
			...
			if(void_ptr2!=NULL)
			{
				...
				if(p_AddItem->GetItemClass() == ITEM_CLASS_ETHEREAL_CHAIN && (p_AddItem->GetItemType()==0 || (p_AddItem->GetItemType()>=3 && p_AddItem->GetItemType()<=12)))
				{
...
#if __CONTENTS(__TUNING_ITEM)
			if( p_item->IsTuningItem() && p_item != NULL )
			{
				...
				if(m_pack_file.IsSet())
				{
					...
					m_pack_file.Open(filename.c_str());

					const int maxsize = 256;
					char szLine[maxsize];
					ZeroMemory(szLine, maxsize);

					m_pack_file.GetString(szLine, maxsize);

					std::string tmemstr = szLine;
...
void	C_VS_UI_SUMMER_COME_BACK::Show()
{
	...
	else if(m_type == COME_BACK2006_LOGIN_AFTER)
		str = (*g_pGameStringTable)[UI_STRING_MESSAGE_SUMMER_COMEBACK2006_LOGIN_AFTER].GetString();
	else if(m_type == COME_BACK2006_SETTLEMENT_AFTER)
		str = (*g_pGameStringTable)[UI_STRING_MESSAGE_SUMMER_COMEBACK2006_SETTLEMENT_AFTER].GetString();
