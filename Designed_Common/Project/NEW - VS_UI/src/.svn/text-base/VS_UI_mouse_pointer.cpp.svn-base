...
//	else frame = 0;
	...
	if(IsMoveZoneEffect())
	{
		...
		if(GetMoveZoneEffectCount()>0)
		{
			m_MoveZoneInfo.m_nMoveEffectCount--;
			eff_pt.x = m_MoveZoneInfo.m_EffectPoint.x-27;
			eff_pt.y = m_MoveZoneInfo.m_EffectPoint.y-15;

			DrawUIEffect(&eff_pt,(int)343,0,3,true);
		}

		eff_pt.x = m_mouse_x-26;
		eff_pt.y = m_mouse_y+75;

		DrawUIEffect(&eff_pt,(int)339,0,3,true);

		gpC_base->m_p_DDSurface_back->Unlock();

		static int color_count = 255;
		static bool flg;

		if(g_CurrentFrame%2==0)
		{
			if(color_count>=250)
				flg = true;
		...
		g_FL2_GetDC();
		g_PrintColorStr(m_mouse_x+23, m_mouse_y-8, "Move Point", gpC_base->m_item_pi, RGB(color_count,color_count,color_count));
		g_FL2_ReleaseDC();
	}
#endif //__MOVE_ZONE
	...
	if (m_bl_pickup_item)
	{
		...
		if(m_p_current_pickup_item->IsTuningItem())
			CIndexSprite::SetUsingColorSet(m_p_current_pickup_item->GetTuningItemColorSet() ,0 );
		else
#endif //__TUNING_ITEM
		...
		else if(m_p_current_pickup_item->GetItemOptionColorSet() >= OPTIONEXT_ITEM_COLOR)
			CIndexSprite::SetUsingColorSet(m_p_current_pickup_item->GetItemOptionExtColorSet(), 0);
...
#if __CONTENTS(__TUNING_ITEM)
			if(m_p_current_pickup_item->IsTuningItem())
				gpC_item->BltLockedColorSet(m_mouse_x-m_half_x, m_mouse_y-m_half_y,
													frame_id, m_p_current_pickup_item->GetTuningItemColorSet());
			else
#endif //__TUNING_ITEM
...
//		int mouse_y = m_mouse_y + g_mouse_point_fix[m_cursor==CURSOR_SLAYER_PORTAL?CURSOR_VAMPIRE_PORTAL:m_cursor].y;
		...
		else
		{
				...
				m_string.size() != 0)// || m_cursor == CURSOR_VAMPIRE_PORTAL)
			{
				...
				else if(rect.bottom > g_pUserInformation->iResolution_y){ rect.top -= rect.bottom - g_pUserInformation->iResolution_y; rect.bottom = g_pUserInformation->iResolution_y -1; }
				...
				else if(rect.right >= g_pUserInformation->iResolution_x){ rect.left -= rect.right - g_pUserInformation->iResolution_x; rect.right = g_pUserInformation->iResolution_x -1; }
...
- Process.
-----------------------------------------------------------------------------*/
void C_VS_UI_MOUSE_POINTER::Process()
{
	// 개발자용 "맵 이동하는 기능" - 2008.11.7 ij-ch
	if(IsMoveZoneEffect()&&GetMoveZoneEffectCount()==0)
	{
		static char strMove[256];
		sprintf(strMove,"%d %d %d",m_MoveZoneInfo.m_ZoneID,m_MoveZoneInfo.m_ZonePoint.x,m_MoveZoneInfo.m_ZonePoint.y);
		gpC_base->SendMessage(UI_MOVE_ZONE, 0L, 0L, strMove);
