...
#if __CONTENTS(__OPTION_NAME_PRINT)
...
{	
	...
	case 16:
		m_pC_spk.Open(SPK_PET_INFO_TABBYCAT);	//카라 미노타우루스 태비 캣
		break;
	case 17:
		m_pC_spk.Open(SPK_PET_INFO_TABBYCAT);	//카라 프타 태비 캣
		break;
	case 18:
		m_pC_spk.Open(SPK_PET_INFO_WEALTHYCAT);	//카라 로크 웰시 캣
		break;
	case 19:
		m_pC_spk.Open(SPK_PET_INFO_WEALTHYCAT);	//카라 프타 웰시 캣
		break;
	case 20:
		m_pC_spk.Open(SPK_PET_INFO_SIAMINGCAT);	//카라 지니어스 샤밍 캣
		break;
	case 21:
		m_pC_spk.Open(SPK_PET_INFO_SIAMINGCAT);	//카라 프타 샤밍 캣
		break;
	case 22:
		m_pC_spk.Open(SPK_PET_INFO_TABBYCAT);	//델라 데스트로이 태비 캣
		break;
	case 23:
		m_pC_spk.Open(SPK_PET_INFO_TABBYCAT);	//델라 바루나 태비 캣
		break;
	case 24:
		m_pC_spk.Open(SPK_PET_INFO_WEALTHYCAT);	//델라 네크로맨시 웰시 캣
		break;
	case 25:
		m_pC_spk.Open(SPK_PET_INFO_WEALTHYCAT);	//델라 바루나 웰시 캣
		break;
	case 26:
		m_pC_spk.Open(SPK_PET_INFO_SIAMINGCAT);	//델라 드라큘라 샤밍 캣
		break;
	case 27:
		m_pC_spk.Open(SPK_PET_INFO_SIAMINGCAT);	//델라 바루나 샤밍 캣
		break;
#endif //__PET_CATS_UPGRADED
#if __CONTENTS(__UPGRADE_MONSTER_PET)
	case 28: case 29: case 34: case 35:	// 인큐버스
		m_PetInfo.ITEM_TYPE = 11;
		m_pC_spk.Open(SPK_PET_INFO_INCUBUS);
		break;
	case 30: case 31: case 36: case 37:	// 서큐버스
		m_PetInfo.ITEM_TYPE = 10;
		m_pC_spk.Open(SPK_PET_INFO_SUCCUBUS);
		break;
	case 32: case 33: case 38: case 39:	// 가고일
...
//				jpg.Release();
		...
		else 
		{
			...
			if(linecount < count)			//  문자열이 count 보다 작은경우
			{
				...
				count -= linecount;			// count 는 다음문자열까지의 값을 가져와야 함으로 현제까지의 문자열길이를 뺴준�?
				pCur = NULL;		
				count = limitlinecnt;
			}
			else
...
{
	char ignore_char[] = {'\"', '\'', '/'
#if __CONTENTS(__PERSNALSHOP_NAMING)
		,' '
#endif
	};
