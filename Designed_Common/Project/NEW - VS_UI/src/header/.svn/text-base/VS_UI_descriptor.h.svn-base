...
struct DESCRIBED_UNIT
{
	...
	void		(*fp_rect_calculator)(void (*fp_show)(Rect, void *, long, long), int, int, void *, long, long, int);
...
private:
	typedef std::multimap<int, DESC> DescMap;
	DescMap	m_DescMap;
public:
	DescriptorManager();
	~DescriptorManager();

	void	Set(id_t id, int x, int y, void * void_ptr, long left=0, long right=0, bool add = false, int z_order = 0);
	const DESC*	GetDesc(void* void_ptr) const;
	//void	Unset();
	...
	void	RectCalculationFinished(void (*fp_show)(Rect, void *, long, long), Rect rect, void * void_ptr, long left, long right, int z_order);
};
