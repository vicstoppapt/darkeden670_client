...
public:
	...
	struct _GQuestInfo{
		...
		WORD				wQeustPos_X;
		WORD				wQeustPos_Y;
		std::string			strQuestCompensation;	//퀘스트 보상 내용
#endif
	};
...
#if __CONTENTS(__QUEST_RENEWAL)
	std::string					m_strQuestType[QUESTTYPEMAX];
	S_QUESTICON_INFO			m_QuestInfo;
...
#if __CONTENTS(__QUEST_RENEWAL)
	bool						InitQuestInfo(const XMLTree *pElement, _GQuestInfo GQuestInfo);
	bool						GetQuestType(const XMLTree *pElement, _GQuestInfo GQuestInfo);
#endif
...
public:
	enum INFO_SPK
	{
		...
		QUEST_LIST_TUTORIAL,
		QUEST_LIST_LEVEL,
		QUEST_LIST_PROMOTION,
		QUEST_LIST_ADVENTURE,
		QUEST_LIST_NPC,
		QUEST_LIST_PET,
#endif
		
#if __CONTENTS(__QUEST_RENEWAL2)
		BUTTON_QUEST_COMPENSATION_VIEW,
		BUTTON_QUEST_COMPENSATION_VIEW_HILIGHT,
...
//		TAB2_ID,
	...
	char						*m_pchQeustCompesationList[10];
#endif // __QUEST_RENEWAL2
...
public:
	...
	void	QeustCompensationStringRelease();
#endif // __QUEST_RENEWAL2
...
#if __CONTENTS(__QUEST_RENEWAL)
	void	SetIconIndex(SHORT	shIndex);
	void	SetQuestPersonString(std::string str, BYTE btStatus);
#endif
};
...
protected:
	enum INVENTORY_SPK_INDEX
	{
		...
		INVENTORY_WINDOW_10X6,
		INVENTORY_WINDOW_10X6_ALPHA,
	};
