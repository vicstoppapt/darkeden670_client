...
C_VS_UI_EXCHANGE::C_VS_UI_EXCHANGE()
{
	...
	case RACE_SLAYER: 
		...
		ok_button_y = 433;
		close_button_y = 458;
		m_my_grid_rect.Set(w_x + 24, w_y + 247, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_X*C_VS_UI_INVENTORY::GRID_X, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_Y*C_VS_UI_INVENTORY::GRID_Y);
		m_your_grid_rect.Set(w_x + 24, w_y + 23, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_X*C_VS_UI_INVENTORY::GRID_X, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_Y*C_VS_UI_INVENTORY::GRID_Y);
	...
	case RACE_VAMPIRE:
		...
		ok_button_y = 427;
		close_button_y = 452;
		m_my_grid_rect.Set(w_x + 17, w_y + 244, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_X*C_VS_UI_INVENTORY::GRID_X, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_Y*C_VS_UI_INVENTORY::GRID_Y);
		m_your_grid_rect.Set(w_x + 17, w_y + 19, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_X*C_VS_UI_INVENTORY::GRID_X, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_Y*C_VS_UI_INVENTORY::GRID_Y);
	...
	case RACE_OUSTERS:
		...
		m_my_grid_rect.Set(w_x + 36, w_y + 258, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_X*C_VS_UI_INVENTORY::GRID_X, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_Y*C_VS_UI_INVENTORY::GRID_Y);
		m_your_grid_rect.Set(w_x + 36, w_y + 34, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_X*C_VS_UI_INVENTORY::GRID_X, C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_Y*C_VS_UI_INVENTORY::GRID_Y);
...
bool C_VS_UI_EXCHANGE::Check(int grid_start_x, int grid_start_y)
{
	...
	if(g_pTradeManager->IsAcceptMyTrade())	// 2008.09.25 shootkj 거래 버튼이 클릭된 상태에서는 더이상 아이템을 올릴 수 없다.
	{
//		return false;
	}
...
void	C_VS_UI_EXCHANGE::ShowButtonDescription(C_VS_UI_EVENT_BUTTON * p_button)
{
	...
	g_descriptor_manager.Set(DID_INFO, p_button->x, p_button->y,
		(void *)m_help_string[p_button->GetID()],0,0);
}
...
//			m_bl_inventory_money_button_focused = false;
...

			{
				...
				if(_x > m_inventory_money_button_point.x+30 + ptOffset.x	&&
				   _x < m_inventory_money_button_point.x+184 + ptOffset.x	&&
				   _y > m_inventory_money_button_point.y+45 + ptOffset.y	&&
				   _y < m_inventory_money_button_point.y+67 + ptOffset.y)
				{
					g_descriptor_manager.Set(DID_INFO,
						m_inventory_money_button_point.x+30 + ptOffset.x,
						m_inventory_money_button_point.y+45 + ptOffset.y,
						(void *)m_help_string[0],0,0);
				}

				if(_x > m_money_button_point.x+30 + ptOffset.x	&&
				   _x < m_money_button_point.x+184 + ptOffset.x	&&
				   _y > m_money_button_point.y+45 + ptOffset.y	&&
				   _y < m_money_button_point.y+67 + ptOffset.y)
				{
					
					g_descriptor_manager.Set(DID_INFO,
						m_money_button_point.x+30 + ptOffset.x,
						m_money_button_point.y+45 + ptOffset.y, (void *)m_help_string[1],0,0);
				}
			}
...
//					}
			...
			if (p_selected_item && (p_selected_item->GetID() == p_item->GetID()))
			{
				...
				if(p_item->IsTuningItem())
					CIndexSprite::SetUsingColorSet(const_cast<MItem *>(p_item)->GetTuningItemColorSet(), 0);
				else
#endif //__TUNING_ITEM
				...
				if(p_item->GetItemClass() == ITEM_CLASS_OUSTERS_WRISTLET && g_eRaceInterface == RACE_OUSTERS)
				{
						...
						)
					{
						...
						gpC_global_resource->m_pC_info_spk->BltLockedOutline(
							GetFocusedItemGridX(p_item), GetFocusedItemGridY(p_item),
							RGB_WHITE, C_GLOBAL_RESOURCE::OUSTERS_ELEMENTAL_MARK_FIRE+eType);
					}
				}
			}
			else
				...
				if(p_item->GetItemClass() == ITEM_CLASS_OUSTERS_WRISTLET && g_eRaceInterface == RACE_OUSTERS)
				{
						...
						)
					{
						...
						gpC_global_resource->m_pC_info_spk->BltLocked(
							GetFocusedOtherItemGridX(p_item), GetFocusedOtherItemGridY(p_item),
							C_GLOBAL_RESOURCE::OUSTERS_ELEMENTAL_MARK_FIRE+eType);
					}
				}
			}
			else
			{
				...
				if(p_item->GetItemClass() == ITEM_CLASS_OUSTERS_WRISTLET && g_eRaceInterface == RACE_OUSTERS)
				{
						...
						)
					{
						// 2004, 3, 9 sobeit - x,y 변경
						//gpC_global_resource->m_pC_info_spk->BltLockedColor(item_x-(p_item->GetGridWidth()*C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_X)/2, item_y-(p_item->GetGridHeight()*C_VS_UI_INVENTORY::GRID_UNIT_PIXEL_Y)/2, C_GLOBAL_RESOURCE::OUSTERS_ELEMENTAL_MARK_FIRE+eType, 0);
						gpC_global_resource->m_pC_info_spk->BltLockedColor(
							GetFocusedOtherItemGridX(p_item), GetFocusedOtherItemGridY(p_item),
							C_GLOBAL_RESOURCE::OUSTERS_ELEMENTAL_MARK_FIRE+eType, 0);
					}
				}
			}
