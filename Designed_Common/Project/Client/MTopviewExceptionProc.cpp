...
void NewGoreGland( MCreature* pCreature, int& action, int& frame, int& direction )
{
	action = ACTION_STAND;
//	frame = 0;
	direction = 2;
...
{
	if(action == ACTION_DIE && (pCreature->GetCreatureType() == 765 || pCreature->GetCreatureType() == 891 || pCreature->GetCreatureType() == 2015))
		direction = 2;
...
void FUBENDOOR( MCreature* pCreature, int& action, int& frame, int& direction )
{
	if(pCreature->IsDead())
	{
		action = ACTION_DIE;
...
void	MTopView::InitMapingExceptionProc()
{
	...
	RegistExceptionProcFunction( 1358 , NewGoreGland );
	RegistExceptionProcFunction( 764 , Ctype_764_765_Proc );
	RegistExceptionProcFunction( 765 , Ctype_764_765_Proc );
	RegistExceptionProcFunction( 891 , Ctype_764_765_Proc );
	RegistExceptionProcFunction( 2015 , Ctype_764_765_Proc );
	...
	RegistExceptionProcFunction( 1704 , FUBENDOOR );
	RegistExceptionProcFunction( 1714 , FUBENDOOR );
	RegistExceptionProcFunction( 1613 , FUBENDOOR );
	// 2004, 12, 11, sobeit add end
	...
	// 미클리스
	RegistExceptionProcFunction( 808 , MakeDirectionToSouth );

	// 오니블라
	for(i = 0; i < 3; ++i)
		RegistExceptionProcFunction( 810 + i , MakeDirectionToSouth );
}
