...
class SKILLINFO_NODE
{
	...
	//	void			SetMP(int ymp)					{ m_YMP = ymp; }
	//	void			SetMP(int zmp)					{ m_ZMP = zmp; }
	...
	//	int					GetMP() const				{ return m_YMP; }
		//int					GetMP() const				{ return m_ZMP; }
		...
		DWORD				GetCurrentDelayTime() const	{ return m_CurrentDelayTime; }

		bool				IsAvailableTime() const;		// 지금 사용 가능한가?
		DWORD				GetAvailableTimeLeft() const;	// 남은 사용 시간

		// 같이 딜레이 걸리는 스킬들 추가
		void				AddDelayGroup(ACTIONINFO id)	{ m_listDelayGroup.push_back(id);	}
		const SKILLID_LIST& GetDelayGroupList() const		{ return m_listDelayGroup;			}
		...
		//------------------------------------------------------
		void		LoadFromFileServerSkillInfo(ivfstream& file);


		//------------------------------------------------------
		// 기술 설명
		//------------------------------------------------------
		void		SetDescription(LPCTSTR desc)	{ m_Description = desc;				}
		LPCTSTR		GetDescription() const			{ return m_Description.GetString();	}

		//------------------------------------------------------
		// 스킬 트리에서 숨기자
		//------------------------------------------------------
		void		SetHideInSkillTree(bool h)		{ m_bHideInSkillTree = h;		}
		bool		IsHideInSkillTree() const		{ return m_bHideInSkillTree;	}
	...
	protected :
		...
		DWORD			m_CurrentDelayTime;	// 현재 딜레이
		...
		// 같이 딜레이 걸리는 스킬들
		//------------------------------------------------------
		SKILLID_LIST	m_listDelayGroup;
		
		//------------------------------------------------------
		// 기술 설명
		//------------------------------------------------------
		MString			m_Description;			// 기술의 간단한 설명

		//------------------------------------------------------
		// 스킬 트리에서 숨기자
		//------------------------------------------------------
		bool			m_bHideInSkillTree;
...
class SKILLID_NODE {
	...
	public :
		SKILLID_NODE()
		{
			SkillID = (ACTIONINFO)ACTIONINFO_NULL;
			Flag	= 0;
		}
...
class MSkillDomain {
	...
	public :
		...
		// Load SERVER_SKILLINFO
		//------------------------------------------------------
		void		LoadFromFileServerDomainInfo(ivfstream& file);
...
public :
	...
	void			SetAvailableTime(TYPE_ACTIONINFO id, int delay = 0);
	void			SetNextAvailableTime(TYPE_ACTIONINFO id);
};
