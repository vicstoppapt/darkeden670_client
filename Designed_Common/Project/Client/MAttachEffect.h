...
class MAttachEffect : public MMovingEffect {
	public :
		MAttachEffect(TYPE_EFFECTSPRITETYPE type, DWORD last,
			DWORD linkCount=MAX_LINKCOUNT, bool bBasisCreature=true);
		~MAttachEffect();
		...
		bool					IsBasisCreature() const			{ return m_bBasisCreature;	}
		void					SetBasisCreature(bool bc)		{ m_bBasisCreature = bc;	}

		//int					GetBasisCreatureFrame() const	{ return m_nBasisCreatureFrame;	}
		//void					SetBasisCreatureFrame(int bcf)	{ m_nBasisCreatureFrame = bcf;	}
	...
	protected :
		...
		bool						m_bBasisCreature;		// 크리처 프레임을 기준으로 애니메이션 되는가?
		//int							m_nBasisCreatureFrame;	// 기준 크리처 프레임 저장
