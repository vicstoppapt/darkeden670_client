...
enum 
{
	...
	NPC_RESPONSE_SHOW_DONATION_WEDDING_COMPLETE_DIALOG, // 기부성공창을 띄운다.
	NPC_RESPONSE_SHOW_ALEADY_DONATED_DIALOG,			// 이미 축의금을 내셨습니다.

	NPC_RESPONSE_NOT_CAHNGE_COLOR,                      // 색상이 중복되어 바꿀수 없다.
    NPC_RESPONSE_NOT_ENOUGH_MONEY_CAHNGE_COLOR,         // 돈이 충분하지 않아 색상을 바꿀수 없다.
    NPC_RESPONSE_CHANGE_COLOR_OK,                       // 염색이 완료 되었습니다.
	
	NPC_NOT_ENOUGH_EXCHANGE_OF_MONEY,                   // 돈이 충분하지 않아 수표로 바꿀수 없다
    NPC_ENOUGH_PLAYER_MONEY,                            // 플레이어의 소지금이 최대 소지금액을 넘었다
    NPC_NOT_ENOUGH_CHECK_MONEY,                         // 수표가 없다!
    NPC_EXCHANGE_OF_MONEY_OK,                           // 정상적으로 거래가 완료 되었습니다
	NPC_NOT_ENOUGH_SOCCER_BALL,
	NPC_NOT_ENOUGH_EXCHANGE_SEALING_STONE,

	NPC_RESPONSE_NOT_ENOUGH_LEVEL,                      // 레벨이 충분 하지 않다 
    NPC_RESPONSE_NOT_ENOUGH_GUILD_MEMBER,               // 길드원이 부족하다 
    NPC_RESPONSE_NOT_ENOUGH_GUILD_LEVEL,                // 길드원의 평균 레벨이 부족하다 

	NPC_RESPONSE_NOT_ENTER_SURVIVAL_ZONE,				// 서바이벌 존에 입장 불가능하다. (입장 불가능 시간, 제한 인원)

	NPC_RESPONSE_GIVE_GAMBLE_ITEM_OK,					// 겜블에서 아이템을 받았을 때
	NPC_RESPONSE_GAMBLE_INIT_OK,						// 겜블 누적점수 초기화 할 때

	NPC_NOT_ENOUGH_EVENT_ITEM,							// 교환에 필요한 이벤트 아이템이 부족할 때
	NPC_NOT_ENOUGH_ITEM,								// 교환에 필요한 아이템이 부족할 때

	NPC_RESPONSE_DENY_ATTEND,                           // 신규 가입 회원만 이벤트에 참여 가능 합니다.
    NPC_RESPONSE_TODAY_NOT_ATTEND,                      // 이미 출석 하셨습니다. 내일 다시 참여해 주세요.
	NPC_RESPONSE_CHECK_COUNT_ATTEND,					// [캐릭터]님은 [출석 체크 회수]번 출석하셨습니다.

#if __CONTENTS(__BLITZ_COUPON_EVENT)
	NPC_RESPONSE_BLITZ_COUPON,							// 블리츠서버 쿠폰 갯수 보기
	NPC_RESPONSE_REWORD_ITEM_OK,						// 아이템 보상시 아이템명까지 보여주는 다이얼로그띄울때
...
enum CommonMessage
{
	...
	YOU_CAN_GET_EVENT_200507_COMEBACK_ITEM,				// 8	컴백 사용자 아이템을 받을 수 있습니다. 2005.7
	YOU_CAN_GET_EVENT_200507_COMEBACK_PREMIUM_ITEM,		// 9	컴백 사용자 결제 아이템을 받을 수 있습니다. 2005.7
	YOU_CAN_GET_EVENT_200507_COMEBACK_RECOMMEND_ITEM,	// 10	컴백 사용자 추천 아이템을 받을 수 있습니다. 2005.7
	
	YOU_CAN_GET_EVENT_COMEBACK_200604_ITEM,				// 11   컴백 사용자 아이템을 받을 수 있습니다. 2006.4
	YOU_CAN_GET_EVENT_COMEBACK_200604_PREMIUM_ITEM,     // 12   컴백 사용자 결제 아이템을 받을 수 있습니다. 2006.4
