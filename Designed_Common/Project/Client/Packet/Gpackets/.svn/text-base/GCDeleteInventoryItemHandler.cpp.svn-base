	 ...
	 throw ( ProtocolException , Error )
{
	...
	else
	{
		...
		pItem = ((MItemManager*)g_pInventory)->GetItemToModifyAll( pPacket->getObjectID() , pSubInventory );
		
		if (pItem != NULL)
		{
			if( pSubInventory == NULL )
				g_pInventory->RemoveItem( pPacket->getObjectID() );
			else
				((MSubInventory*)pSubInventory)->RemoveItem( pPacket->getObjectID() );
		}
		else
		{
