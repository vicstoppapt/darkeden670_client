	 ...
	 throw ( Error )
{
	...
	if( skillID == MAGIC_UN_TRANSFORM)
	{
		...
		if(g_pPlayer->IsOusters())
		{
			...
			if(g_pPlayer->HasEffectStatus( EFFECTSTATUS_OusterWings_New_1))//침쥣劤麟폄
			{
				g_pPlayer->RemoveAttachEffect(EFFECTSPRITETYPE_unknow_2327);
				g_pPlayer->RemoveAttachEffect(EFFECTSPRITETYPE_unknow_2326);
			}
			if(g_pPlayer->HasEffectStatus( EFFECTSTATUS_OusterWings_New_2))//침쥣劤麟폄
			{
				g_pPlayer->RemoveAttachEffect(EFFECTSPRITETYPE_unknow_2698);
				g_pPlayer->RemoveAttachEffect(EFFECTSPRITETYPE_unknow_2697);
			}
		...
		{
			return;
			//resultActionInfo = RESULT_.SKILL_INSTALL_TURRET;
		}
		else if(g_pPlayer->HasEffectStatus(EFFECTSTATUS_INSTALL_MK1))
		{
			return;
