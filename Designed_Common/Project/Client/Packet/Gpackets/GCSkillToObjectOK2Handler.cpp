	 ...
	 throw ( Error )
{
	...
	else
	{
		...
		{
			Add_FakeCreature_Wild_Wolf_Strike(pCreature,g_pPlayer);
		}
...
//				SkillIllendue( pCreature, g_pPlayer, SKILL_CLIENT_BAT_BREAKER);
   //            break;
			case SKILL_SPECTOR_INVERSE :
				// 2004, 12, 15, sobeit modify start - 힐직이 인챈 라이트볼 쓰는게 버그.
				//SkillIllendue( pCreature, g_pPlayer, SKILL_LIGHT_BALL );
				SkillSPECTORINVERSE( pCreature, g_pPlayer, SKILL_DUMMY_ACTION_MAGIC);
				// 2004, 12, 15, sobeit modify end
				break;
			...
			case SKILL_ADV2_SUDDEN_BLAST:	
				//pCreature->AddEffectStatus(EFFECTSTATUS_SUDDEN_BLAST, 20);
				pCreature->CreateAttachEffect(EFFECTSPRITETYPE_unknow_2342,10,0);
				break;
			case SKILL_ACCELERATE_CHARGE_1:
				 pCreature->CreateAttachEffect(EFFECTSPRITETYPE_unknow_2879,10,0);
				 break;
			case SKILL_ACCELERATE_CHARGE_2:
				pCreature->CreateAttachEffect(EFFECTSPRITETYPE_unknow_2892,10,0);
				break;
