...
class MGridItemManager : public MItemManager {
	public :
		...
		// UpdateStatus
		//------------------------------------------------------
		void			UpdateStatus(BYTE x, BYTE y) const;

	protected:
		//------------------------------------------------------
		// Add
		//------------------------------------------------------
		virtual bool	AddItem(MItem* pItem);					// 적절한 grid에 추가한다.
		virtual bool	AddItem(MItem* pItem, BYTE x, BYTE y);	// grid(x,y)에 pItem을 추가한다.
