...
MInternetFile::Init(const char* pURL, const char* pLocalName)
{
	...
	{	
		...
		// 파일 크기 얻기
		char	szBuffer[32] = { 0 };
		DWORD	bufferLength = sizeof(szBuffer);

		if(HttpQueryInfo(m_hFile, HTTP_QUERY_CONTENT_LENGTH, szBuffer, &bufferLength, NULL))
		{
			m_nFileSize = atoi(szBuffer);
		}
		else	
		{
			if (m_hFile!=NULL)
