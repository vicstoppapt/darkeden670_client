...
: MLinearEffect(bltType)
{
	...
	m_bArrive		= false;	// 타겟에 도달하였는가?
	m_nArriveDelay	= 0;		// 타겟에 도달하였을 때 잠시 멈추는 시간
}
...
{	
	if (g_CurrentFrame < m_EndFrame || m_nArriveDelay > 0)
	{
		if (!TraceCreature())
			return false;
		...
		else
		{
			...
			// m_nArriveDelay만큼 대기
			//------------------------------------------
			if(--m_nArriveDelay < 0) return false;
		}
