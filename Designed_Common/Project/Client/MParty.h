...
class MParty {
	public :
		...
		typedef PARTY_VECTOR::iterator			PARTY_ITER;
		typedef PARTY_VECTOR::const_iterator	PARTY_CONST_ITER;
	...
	public :
		...
		bool		IsOmissionMember(const char* pName) const;

	private:	
		bool		IsOmissionMember(PARTY_INFO* pInfo) const;	// 특수한 경우에 제명된 놈 ex)할루시네이션

	protected :
		//int						m_Size;			// 파티원 수	이건 왜 있는걸까? 삭제! by chyaya
