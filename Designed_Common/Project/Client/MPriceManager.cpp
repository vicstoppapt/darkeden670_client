...
//			bMysterious = false;
	...
	{		
		...
		else if ( pItem->GetItemClass() == ITEM_CLASS_SLAYER_PORTAL_ITEM || pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_PORTAL_ITEM )
		{
			...
			#ifdef __GAME_CLIENT__
				domainLevel = max(domainLevel , g_pPlayer->GetLEVEL())  ; 
			#endif
			
			if(pItem->GetItemClass()==ITEM_CLASS_SLAYER_PORTAL_ITEM)
			{
				if(pItem->GetItemType() < 3)
					ChargePrice = pow((float)domainLevel , 1.25f)  * 100 ;   // 아이템 가격 계산 (Sjheon 2005.06.02) 
				else if(pItem->GetItemType() == 3 || pItem->GetItemType()  == 4)
					ChargePrice = pow((float)domainLevel , 1.50f)  * 100 ;   // 아이템 가격 계산 (Sjheon 2005.06.02) 
			}
			else
				ChargePrice = pow((float)domainLevel , 1.3f)  * 100 ;   // 아이템 가격 계산 (Sjheon 2005.06.02)
		}
		...
		if ( pItem->GetItemClass() != ITEM_CLASS_SLAYER_PORTAL_ITEM && pItem->GetItemClass() != ITEM_CLASS_VAMPIRE_PORTAL_ITEM )
		{
			int finalPrice = 0 ; 
			// 가격 계산 
			int itemPrice = pItem->GetPrice();
			finalPrice = itemPrice + curCharge * ChargePrice;
...
int MPriceManager::GetMysteriousPrice(MItem *pItem) const
{
	...
	else
	{
		...
		if ( pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_WEAPON )
			multiplier = CLevel * 6000;
		else
			multiplier = CLevel * 3000;
	...
	if(pItem->GetItemClass() == ITEM_CLASS_BLADE  || pItem->GetItemClass() == ITEM_CLASS_SWORD  || 
	   pItem->GetItemClass() == ITEM_CLASS_AR     || pItem->GetItemClass() == ITEM_CLASS_SMG    || 
	   pItem->GetItemClass() == ITEM_CLASS_SG     || pItem->GetItemClass() == ITEM_CLASS_SR     || 
	   pItem->GetItemClass() == ITEM_CLASS_CROSS  || pItem->GetItemClass() == ITEM_CLASS_MACE   ||
	   pItem->GetItemClass() == ITEM_CLASS_COAT   || pItem->GetItemClass() == ITEM_CLASS_TROUSER   ||
	   pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_WEAPON   || pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_COAT   ||
	   pItem->GetItemClass() == ITEM_CLASS_OUSTERS_CHAKRAM   || pItem->GetItemClass() == ITEM_CLASS_OUSTERS_WRISTLET ||

	   pItem->GetItemClass() == ITEM_CLASS_OUSTERS_COAT   || pItem->GetItemClass() == ITEM_CLASS_OUSTERS_BOOTS ||
	   pItem->GetItemClass() == ITEM_CLASS_OUSTERS_ARMSBAND)
	{
		final_price = 680000 ; 
	}
	 
	
	if(pItem->GetItemClass() == ITEM_CLASS_HELM  || pItem->GetItemClass() == ITEM_CLASS_GLOVE		|| 
	   pItem->GetItemClass() == ITEM_CLASS_BELT     || pItem->GetItemClass() == ITEM_CLASS_SHOES    ||
	   pItem->GetItemClass() == ITEM_CLASS_SHIELD)
	{
		final_price = 360000 ; 
	}
		
	if(pItem->GetItemClass() == ITEM_CLASS_NECKLACE  || pItem->GetItemClass() == ITEM_CLASS_BRACELET				 || 
	   pItem->GetItemClass() == ITEM_CLASS_RING      || pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_NECKLACE		 || 
	   pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_BRACELET  || pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_RING     || 
	   pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_EARRING   || pItem->GetItemClass() == ITEM_CLASS_OUSTERS_CIRCLET  || 
	   pItem->GetItemClass() == ITEM_CLASS_OUSTERS_RING      || pItem->GetItemClass() == ITEM_CLASS_VAMPIRE_AMULET   ||
	   pItem->GetItemClass() == ITEM_CLASS_OUSTERS_PENDENT)
	{
		final_price = 130000 ; 
	}
...
#if __CONTENTS(__REPAIRSYSTEM_RENEWAL)
void
MPriceManager::RepairPriceListInsert(DWORD itemObjectID, MPriceManager::REPAIRPIRCEPAIR repairPircePair)
{
	m_RepairPriceList[itemObjectID] = repairPircePair;
}


bool MPriceManager::IsRepairPriceListHave(DWORD itemObjectID)
{
//	MPriceManager::REPAIRPRICELIST::iterator repairPiracePos = GetRepairPriceListBegin();
//	MPriceManager::REPAIRPRICELIST::iterator repairPiracePosEnd = GetRepairPriceListEnd();
//
//	int repairItemID = itemObjectID;
//	
//	while(repairPiracePos != repairPiracePosEnd)
//	{
//		if(repairPiracePos->first == repairItemID)
//		{
//			return true;
//		}
//		repairPiracePos++;
...
// 	return false;
	...
	MPriceManager::REPAIRPRICELIST::iterator iter;

	iter = m_RepairPriceList.find(itemObjectID);
	if ( iter != GetRepairPriceListEnd() )
		return true;
 	return false;
}
MPriceManager::REPAIRPIRCEPAIR MPriceManager::GetRepairPricePair(DWORD itemObjectID)
{
	MPriceManager::REPAIRPRICELIST::iterator iter = m_RepairPriceList.find(itemObjectID);
	if(iter != GetRepairPriceListEnd())
	{
		return iter->second;
	}
	...
	{
		MPriceManager::REPAIRPIRCEPAIR zero(0, 0);
		return zero;
	}
}
void
MPriceManager::RepairPriceListerase(DWORD itemObjectID)
{
	MPriceManager::REPAIRPRICELIST::iterator iter;

	iter = m_RepairPriceList.find(itemObjectID);
	if (iter != GetRepairPriceListEnd())
		m_RepairPriceList.erase(iter);
