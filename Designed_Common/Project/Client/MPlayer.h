...
extern MSlayerGear*		g_pSlayerGearOther;
extern MVampireGear*	g_pVampireGearOther;
extern MOustersGear*	g_pOustersGearOther;
...
// 테스트 코드
		...
		char arrchRootingTest[255];
		char arrchSummon[255];
	...
	public :
		...
		const char*	GetName() const	{
			if(!g_pUserInformation) return NULL;
			return g_pUserInformation->CharacterID.GetString(); }
		...
		// 상태 값 바꾸기
		//----------------------------------------------------------
		void	AffectItemOption(const MItem* pItem);
...
//		void	UnSetLightSight();	
		...
		void	SetLockMode()			{ m_bLockMode = true; SetTraceID(OBJECTID_NULL); m_LockTimer = GetTickCount(); }
		void	UnSetLockMode()			{ m_bLockMode = false; }
		int		FindEnemy();

		bool    IsBlazeWalk2Mode () const { return m_bBlazeWalk2Mode;}
		void    SetBlazeWalk2LockMode()			{ m_bBlazeWalk2Mode = true;}
		void	UnBlazeWalk2SetLockMode()			{ m_bBlazeWalk2Mode = false; }
		//----------------------------------------------------------
		// player가 Darkness안에 있는가?
		// DarknessCount는 몇타일이 보이는가?이다.
		//----------------------------------------------------------
		void	CheckInDarkness();
		void	CheckInGunFiled();
		...
		void	SetPrevDyeColorSet(TYPE_ITEMTYPE PrevDyeColorSet) { m_PrevDyeColorSet = PrevDyeColorSet ; }
		int		GetPrevDyeColorSet() { return m_PrevDyeColorSet; }
		//by chaya
		void	SetEnforceResurrect(bool enforceResurrect)		{ m_bEnforceResurrect = enforceResurrect;	}
		bool	GetEnforceResurrect()							{ return m_bEnforceResurrect;				}
		//end
		//20070409 by diesirace
		...
		void	ActionToSendPacket(bool bImmediately = false);	// packet을 보낸다..
		bool	ActionInTraceDistance();	// 행동 가능 거리에 있을 때

#if __CONTENTS(__GEAR_SWAP_CHANGE)
		MItem*	GetGearItemIndexFromAddonInfo(INT iIndex)	//장착창의 인덱스에 해당하는 아이템을 넘겨 준다.
		{
			return		NewItemFromAddonInfo(iIndex);
		}
#endif
		void	RecordCharName(TYPE_OBJECTID id);

#if __CONTENTS(__FRIEND_ADDITION)
		void	RequestModeExec();
...
//		TYPE_SECTORPOSITION		m_TimeLightSightY;		
		...
		TYPE_ITEMTYPE			m_PrevDyeColorSet;		// NPCScrip 에서 임시로 저장하는 ColorSet
		
		bool					m_bEnforceResurrect;		// 일반 부활 버튼과 이터니티 스킬 사용 버튼을 숨긴다. (기타 엘릭서 같은건 사용가능)
		bool					m_bInvincible;

		TYPE_SECTORPOSITION		m_SkillCastingX;
		TYPE_SECTORPOSITION		m_SkillCastingY;
};
