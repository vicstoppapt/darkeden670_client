...
{
	int zone_id, x, y, portal_x, portal_y,extend_portal;
};
...
// 테스트 코드
		...
		CREATURE_MAP GetCretureMap() { return	m_mapCreature;}
		...
		bool		LoadFromFileSectorSound(ivfstream& file);
		...
		bool		CanAddEffect(MEffect* pEffect);						// Zone에 해당 Effect를 추가할 수 있는가?
		...
		bool		RemoveTileEffectByActionInfo(TYPE_SECTORPOSITION sX, TYPE_SECTORPOSITION sY, TYPE_ACTIONINFO actionInfoIdx, int serverID=0);
		...
		bool		RemoveGroundEffectByActionInfo(TYPE_SECTORPOSITION sX, TYPE_SECTORPOSITION sY, TYPE_ACTIONINFO actionInfoIdx);
		...
		// Tile SpriteID 얻기
		//--------------------------------------------------------------
		void						GetNearSpriteSet(CSpriteSetManager& TileSSM, CSpriteSetManager& ImageObjectSSM,
													 TYPE_SECTORPOSITION sX, TYPE_SECTORPOSITION sY) const;

		void						GetNearOrderAllSpriteSet(CSpriteSetManager& TileSSM, CSpriteSetManager& ImageObjectSSM,
															 TYPE_SECTORPOSITION sX, TYPE_SECTORPOSITION sY,
															 int NearIgnoreCX = 0, int NearIgnoreCY = 0) const;
		...
		// other creatures : New, Add, Get, Remove
		MFakeCreature*	NewFakeCreature(int creatureType, int x, int y, int dir);
		MFakeCreature*	NewFakeCreature(MCreature* pCreature, int x, int y);
		...
		// HelicopterManager
		//--------------------------------------------------------------
		bool			AddHelicopter(TYPE_OBJECTID creatureID, int x0, int y0, int ExtendType = 0);
