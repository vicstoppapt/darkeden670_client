...
{
	//#ifdef	OUTPUT_DEBUG
		//	DEBUG_ADD("MasterVolume : start init");
	//#endif
	...
	{
		//#ifdef	OUTPUT_DEBUG
				//DEBUG_ADD("MasterVolume : MasterOutputVolume: FAILURE: Could not open WaveOut Mapper");
		//#endif
		//TRACE(".MasterOutputVolume: FAILURE: Could not open WaveOut Mapper. mmResult=%d\n", mmResult );
		return false;
	} else {
		//#ifdef	OUTPUT_DEBUG
				//DEBUG_ADD("mixer getid");
		//#endif
		...
		{
			//#ifdef	OUTPUT_DEBUG
				//	DEBUG_ADD("WaveOut Mapper in mixer is not available");
			//#endif
	...
	//#ifdef	OUTPUT_DEBUG
			//DEBUG_ADD("register classex");
	//#endif
	::RegisterClassEx(&wcx);
	//#ifdef	OUTPUT_DEBUG
			//DEBUG_ADD("create window");
	//#endif
	...
	if ( !m_hWnd )
	{
		...
		//#ifdef	OUTPUT_DEBUG
				//DEBUG_ADD("Could not create internal window");
		//#endif
		return false;
	}
	//#ifdef	OUTPUT_DEBUG
		//		DEBUG_ADD("show window");
		//#endif
	::ShowWindow(m_hWnd, SW_HIDE);
	//#ifdef	OUTPUT_DEBUG
				//DEBUG_ADD("mixer open");
		//#endif
	mmResult = mixerOpen( (LPHMIXER)&m_dwMixerHandle, m_uMixerID, (DWORD)m_hWnd, 0L, CALLBACK_WINDOW );
	if ( mmResult != MMSYSERR_NOERROR )
	{
		//#ifdef	OUTPUT_DEBUG
				//DEBUG_ADD("could not open mixer");
		//#endif
		//TRACE(".MasterOutputVolume: FAILURE: Could not open Mixer. mmResult=%d\n", mmResult );
			//#ifdef	OUTPUT_DEBUG
				//DEBUG_ADD("destroy window");
		//#endif
		::DestroyWindow( m_hWnd );
		return false;
	}
	//#ifdef	OUTPUT_DEBUG
		//		DEBUG_ADD("MasterVolume : Init OK");
		//#endif
	return true;
}
//////////////
