...
//----------------------------------------------------------------------
// dirName : 현재 directory 밑에 생성할려는 directory Path
//----------------------------------------------------------------------
// Directory Path를 입력하면 하위 Directory까지 자동으로 생성한다.
//----------------------------------------------------------------------
bool UUFCreateDirectoryRecursive(const char* dirName)
{
	...
	if (p) 
	{
		...
		*p = '\0';
		...
		if (!UUFCreateDirectoryRecursive(szParent))
		{
			free(szParent);
			return false;
		}
...
// dirName : 지울려는 directory 이름
//----------------------------------------------------------------------
// 해당 Directory와 하위 Directory 중 빈 Directory를 삭제한다.
//----------------------------------------------------------------------
	...
	while(bResult)
	{
		if(wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
		{
			if(wfd.cFileName[0] != '.')
			{
				...
				UUFDeleteEmptyDirectoryRecursive(newPath);
			}
		}
...
// dirName : 파일 개수를 샐 위치
//----------------------------------------------------------------------
// 해당 Directory와 하위 directory의 파일 개수를 샌다.
//----------------------------------------------------------------------
void UUFGetFileCount(const char* dirName, int* pFileCount)
{
	...
	while(bResult)
	{
		if(wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
		{
			...
			{
				wsprintf(newPath, "%s%s%s", drive, dir, wfd.cFileName);
				UUFGetFileCount(newPath, pFileCount);
			}
		}
		else
		{
...
{				
	if (UUFHasPermission( FilenameTarget ) && UUFHasPermission( FilenameSource))				
	{
		...
		// 디렉토리를 생성한다.
		char szBuf[MAX_PATH];
		_splitpath(FilenameTarget, NULL, szBuf, NULL, NULL);
		UUFCreateDirectoryRecursive(szBuf);
...
{	
		...
		))		
	{
		...
		if (bAppendIndex)
		{
			...
			// 추가할 수 있게 한다.
			ivfstream addIndexFile(FilenameIndexAdd, std::ios::binary);
