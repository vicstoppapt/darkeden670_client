...
//by viva  check GB2312
/*
			  런竟櫓匡：   
  멕貫：129~160   ，됴貫：64~254（<>127）   
  멕貫：170~254   ，됴貫：64~160（<>127）   
  숌竟櫓匡：   
  멕貫：176~247   ，됴貫：161~254（<>127）   
  景喝俚륜（관윅櫓匡깃듐）：   
  멕貫：161~167   ，됴貫：161~254   
  멕貫：168~169   ，됴貫：64~254（<>127） 
  */
...
//			char strspec[] = "꿥꿦";
			...
			if(ch == strblack[0] && *str == strblack[1])
				return 0;
			if( (unsigned char)ch>=176&&(unsigned char)ch<=247 && (unsigned char)*str>=161&&(unsigned char)*str<=254 ) //숌竟櫓匡
			{
			}
			else
			{
//				if(ch == strspec[0] && *str == strspec[1] ||
//					ch == strspec[2] && *str == strspec[3])
//				{
//				}
//				else
					return 0;
...
//			else if( (unsigned char)ch>=129&&(unsigned char)ch<=160 && (unsigned char)*str>=64&&(unsigned char)*str<=254 )
//			{
//			}
//			else if( (unsigned char)ch>=170&&(unsigned char)ch<=254 && (unsigned char)*str>=64&&(unsigned char)*str<=160 )
//			{
//			}
//			else if( (unsigned char)ch>=161&&(unsigned char)ch<=167 && (unsigned char)*str>=161&&(unsigned char)*str<=254 )
//			{가臨’ 
//			}
//			else if( (unsigned char)ch>=168&&(unsigned char)ch<=169 && (unsigned char)*str>=64&&(unsigned char)*str<=254 )
//			{
//			}
...
/*
			...
			*/
			...
			if( !( strTemp[0] >= 0xB0 && strTemp[0] < 0xC9	&&
				   strTemp[1] >= 0xA1 && strTemp[1] <= 0xEF ) ) {
				return 0;
			}
			*/
...
/*
			...
			MultiByteToWideChar(
				...
				strKor,					// string to map
				2,						// number of bytes in string
				strWC,					// wide-character buffer
				2						// size of buffer
				);
//by viva
//			if( !( strWC[0] >= 0xAC00 && strWC[0] < 0xD7A3 ) ) {
//				return 0;
//			}
//			if( !( strWC[0] >= 0xA1A1 && strWC[0] < 0xFEFE ) ) {
//				return 0;
//			}
			//
			//////////////////////////////////////////////////////////////////////////
