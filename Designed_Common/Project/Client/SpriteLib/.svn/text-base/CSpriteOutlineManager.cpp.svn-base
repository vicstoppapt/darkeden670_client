...
CSpriteOutlineManager::InitInfo(int width, int height)
{
	...
	// 메모리 잡기..
	m_ppPixelInfo = new WORD* [m_Height];
	m_ppColorInfo = new WORD* [m_Height];

	for (int i=0; i<m_Height; i++)
	{
		m_ppPixelInfo[i] = new WORD [m_Width];
...
CSpriteOutlineManager::Blt(WORD* pSurface, WORD Pitch, WORD color)
{
	...
	for (i=0; i<=rectBottom; i++)
	{
		...
		j = 0;
		WORD* pPixelInfo = m_ppPixelInfo[i];
		while (j <= rectRight)
		{		
			info = *pPixelInfo;
		...
		if (count > 0)
		{
			...
			do {		
				...
				for(k = 0; k < colorCount; ++k)
					pPixelInfoTemp[k] = PIXELTYPE_COLOR;
	...
	for (i=0; i<height; i++)
	{
		...
		if (count > 0)
		{
			...
			do {				
				...
				for(k = 0; k < indexCount+colorCount; ++k)
					pPixelInfoTemp[k] = PIXELTYPE_COLOR;
