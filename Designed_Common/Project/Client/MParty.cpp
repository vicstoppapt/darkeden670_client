...
/*
int
MParty::GetSize() const
{
	PARTY_CONST_ITER		pos		= m_pInfo.begin();
	const PARTY_CONST_ITER	enditer = m_pInfo.end();

	int cnt = 0;

	for(; pos != enditer; ++pos)
	{
		if(!IsOmissionMember(pos))
			++cnt;
	}
...
MParty::GetMemberInfo(int n) const
{
	...
	{
		PARTY_INFO* pInfo = m_pInfo[n];

		if(!IsOmissionMember(pInfo))
			return pInfo;
	}
...
MParty::GetMemberInfo(const char* pName) const
{
	...
	while (iInfo != m_pInfo.end())
	{
		...
		if (pInfo!=NULL && pInfo->Name==pName && !IsOmissionMember(pInfo))
		{
			return pInfo;
		}
...
MParty::GetMemberInfoByIP(const char* pIP) const
{
	...
	while (iInfo != m_pInfo.end())
	{
		...
		if (pInfo!=NULL && pInfo->IP==pIP && !IsOmissionMember(pInfo))
		{
			return pInfo;
		}
...
MParty::HasMember(const char* pName) const
{
	...
	PARTY_CONST_ITER iInfo = m_pInfo.begin();
...
//----------------------------------------------------------------------
bool
MParty::IsOmissionMember(const char* pName) const
{
	PARTY_INFO* pInfo = GetMemberInfo(pName);
	return IsOmissionMember(pInfo);
}
...
//----------------------------------------------------------------------
bool
MParty::IsOmissionMember(PARTY_INFO* pInfo) const
{
#ifdef __GAME_CLIENT__
	//MCreature*	pCreature = g_pZone->GetCreature(pInfo->ID);
	...
	if(g_pPlayer != NULL)
	{
		//return pCreature->IsHallu();	// 할루시네이션 걸리면 제명
		return g_pPlayer->HasEffectStatus(EFFECTSTATUS_HALLUCINATION);
	}
#endif
