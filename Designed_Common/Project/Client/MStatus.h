...
//		DWORD	GetArmageddonHP() const			{ return m_Status[MODIFY_ARMAGEDDON_HP]; }
		...
		DWORD	GetAttackBloodBurstPoint()	const	{ return m_Status[ MODIFY_ATTACK_BLOOD_BURST_POINT ]; }
		DWORD	GetDefenseBloodBurstPoint()	const	{ return m_Status[ MODIFY_DEFENSE_BLOOD_BURST_POINT ]; }
		DWORD	GetPartyBloodBurstPoint()	const	{ return m_Status[ MODIFY_PARTY_BLOOD_BURST_POINT ]; }

		DWORD	GetSlayerAdvancedStr()		const	{ return m_Status[MODIFY_ADVANCED_SLAYERSTR];}
		DWORD	GetSlayerAdvancedDex()		const	{ return m_Status[MODIFY_ADVANCED_SLAYERDEX];}
		DWORD	GetSlayerAdvancedInt()		const	{ return m_Status[MODIFY_ADVANCED_SLAYERINT];}

#if __CONTENTS(__CONTRIBUTE_SYSTEM)		
		DWORD	GetContributePoint()		const	{ return m_Status[MODIFY_CONTRIBUTE_POINT];}
#endif //__CONTRIBUTE_SYSTEM

		DWORD	GetLuck()		const	{ return m_Status[MODIFY_LUCK];}
		DWORD	GetHpSteal()		const	{ return m_Status[MODIFY_HPSTEAL];}
		DWORD	GetMpSteal()		const	{ return m_Status[MODIFY_MPSTEAL];}
		DWORD	GetAllRes()		const	{ return m_Status[MODIFY_ALLRES];}
		DWORD	GetCri()		const	{ return m_Status[MODIFY_CRI];}
