...
{	
	ivfstream file(filename, std::ios_base::binary);
...
void MHelpMessageManager::LoadFromFile(ivfstream &file)
{
	MHelpMessage message;
	MString		 m_sender;
	char sztemp[dSTRING_LEN];
...
//		const MString& strSender = MHelpMessageManager::Instance().getSender(i);
		const MString& strSender = getSender(i);
		file << strSender << "\r\n";
	}
...
//		const MHelpMessage& message = MHelpMessageManager::Instance().getMessage(i);
		const MHelpMessage& message = getMessage(i);
		file << "[===KeyWord===]" << "\r\n" ;
