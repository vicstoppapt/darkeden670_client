...
char								CDirectDraw::m_AddGammaStep[3] = {0, 0, 0};
...
bool CDirectDraw::InitWindowMode(WORD wWidth, WORD wHeight)
{
	...
	if (m_pDD->SetCooperativeLevel(m_hWnd, DDSCL_EXCLUSIVE | DDSCL_NORMAL) != DD_OK)
	{
...
void CDirectDraw::SetGammaRamp(WORD step)
{
	...
	if (m_pDDGammaControl!=NULL)
	{
		...
		for(int i = 0; i < 3; i++)
		{
			short addGammaStep = max(-100, min(100, m_AddGammaStep[i]));
...
void	CDirectDraw::SetAddGammaRamp(char rStep, char gStep, char bStep)
{
