...
MGuildMarkManager::MGuildMarkManager(const char* pGuildMarkSPKFilename, 
									...
									const char* pMonsterLevelMarkSPKFilename,
									const char* pOriginMarkSPKFilename )
{
...
//	}
	...
	m_GuildMarks.LoadFromFileRunning(pGuildMarkSPKFilename);
...
//	m_GradeMarks.Init(GRADE_MARK_MAX<<2, CDirectDraw::Is565());
	...
	m_OriginMarks.LoadFromFileRunning(pOriginMarkSPKFilename);
	m_LevelMarks.LoadFromFileRunning(pLevelMarkSPKFilename);
	m_MonsterLevelRed.LoadFromFileRunning(pMonsterLevelMarkSPKFilename);
}
...
{
	// m_GuildMarks를 참조할 뿐이므로 delete할 필요없다.
	/*
	GUILDMARK_MAP::iterator iMark = m_GuildMarksMap.begin();

	while (iMark != m_GuildMarksMap.end())
	{
...
MGuildMarkManager::AddGuildMark(WORD guildID, CSprite* pSprite, CSprite* pSpriteSmall)
{
	...
	GUILDMARK_MAP::iterator iMark = m_GuildMarksMap.find(guildID);

	// 이미 있다면 기존에꺼 delete해야 한다.
	if (iMark != m_GuildMarksMap.end())
	{
		CSprite* pSprite = iMark->second.pSprite;
		CSprite* pSpriteSmall = iMark->second.pSpriteSmall;
	...
	m_GuildMarksMap[guildID] = GUILD_SPRITES( pSprite, pSpriteSmall );
...
MGuildMarkManager::GetGuildMark(WORD guildID)
{
	...
	if (iMark != m_GuildMarksMap.end())
	{
		return m_GuildMarksMap[guildID].pSprite;
	}
...
MGuildMarkManager::GetGuildMarkSmall(WORD guildID)
{
	...
	if (iMark != m_GuildMarksMap.end())
	{
		return m_GuildMarksMap[guildID].pSpriteSmall;
	}
...
{
	GUILDMARK_MAP::const_iterator iMark = m_GuildMarksMap.find(guildID);
...
MGuildMarkManager::LoadGuildMark(WORD guildID)
{
	...
	if (iMapper != g_pGuildInfoMapper->end())
	{
		...
		if(spriteID != GUILDMARK_NULL)
		{
			CSprite &sprite			= m_GuildMarks.Get(spriteID);
			CSprite &spriteSmall	= m_GuildMarks.Get(spriteID + 1);

			m_GuildMarksMap[guildID] = GUILD_SPRITES(&sprite, &spriteSmall);
	...
	if (iMapper!=g_pGuildInfoMapper->end())
	{
		...
		ivfstream spkFile(m_GuildMarkSPKFilename.GetString(), std::ios::binary);
		ivfstream spkiFile(spkiFilename, std::ios::binary);

		if (spkFile.is_open() && spkiFile.is_open())
		{	
			//---------------------------------------------------------
			// Sprite의 개수를 알아낸다.
			//---------------------------------------------------------
	...
	*/
	...
	m_GuildMarksMap[guildID] = GUILD_SPRITES( NULL, NULL );
...
MGuildMarkManager::SaveGuildMark(WORD guildID, CSprite* pSprite, CSprite* pSpriteSmall)
{
		...
		&& pSpriteSmall!=NULL)
	{
		...
		ivfstream spkInputFile(m_GuildMarkSPKFilename.GetString(), std::ios::binary);
...
// 		if (access(pSPKFilenameApp, 0)
// 			|| access(pSPKIFilenameApp, 0))	// 추가할게 없는 경우
// 		{
// 			return false;
// 		}

		ivfstream spkInputFileApp(pSPKFilenameApp, std::ios::binary);
		ivfstream spkiInputFileApp(pSPKIFilenameApp, std::ios::binary);

		if(!spkInputFileApp.is_open() || !spkiInputFileApp.is_open())
			return false;

		//---------------------------------------------------------
		// Sprite의 개수를 알아낸다.
		//---------------------------------------------------------
		ivfstream spkInputFile(pSPKFilenameOrg, std::ios::binary);
		...
		ivfstream spkiFileApp(pSPKIFilenameApp, std::ios::binary);
		...
		ivfstream mapperFileOrg(pIDMapperFilenameOrg, std::ios::binary);
		...
		ivfstream mapperFileApp(pIDMapperFilenameApp, std::ios::binary);
...
//----------------------------------------------------------------------
// Get GetOriginMark	- 2005.12.23 chyaya
//----------------------------------------------------------------------
CSprite*
MGuildMarkManager::GetOriginMark(WORD origin)
{
	if(origin >= ORIGIN_MAX)
		return NULL;
	
	return &m_OriginMarks[origin];
}
...
CSprite*
MGuildMarkManager::SetMonsterLevelMarkGreen()
{
	return &m_MonsterLevelRed[0];
}
CSprite*
MGuildMarkManager::SetMonsterLevelMarkRed()
{
	return &m_MonsterLevelRed[1];
}
