...
{	
	...
	// 초기화면의 음악
	//--------------------------------------------------------------
	MUSIC_THEME				= MUSIC_OVERTURE; //MUSIC_TREASURE;
...
//	URL_HOMEPAGE_BILING				= "http://member.t2cn.com/billing";
	URL_HOMEPAGE					= "http://www.crazytuan.net/";				// 홈페이지 main
	URL_HOMEPAGE_NEW_USER			= "http://www.crazytuan.net/reg.php";		// 새 사용자 등록 URL
	URL_HOMEPAGE_NEW_USER_NETMARBLE	= "http://www.crazytuan.net/";
	URL_HOMEPAGE_BILING				= "http://www.crazytuan.net/mail.php";
#if __CONTENTS(__ID_PASSWORD)
	URL_HOMEPAGE_NEW_USER = "http://www.crazytuan.net/reg.php";
#endif	//__ID_PASSWORD
...
//이거 정리좀 하자.. 더럽다.. ㅜ.ㅜ
	...
	REPEAT_TIME				= 3*60*1000;				// 자동공격 제한시간 10분
	LOCK_TIME				= 3*60*1000 ; // 10*60*1000;				// Caps Lock자동공격 제한시간 10분
	
	// 유니크 아이템 칼라셋
	UniqueItemColorSet		= 405;
	QuestItemColorSet		= 345;
#if __CONTENTS(__LEVEL_WAR_RENEWAL)
	LevelWarItemColorSet	= 200;
	...
	DELAY_PLAYER_RESURRECT_TIMEOUT_SIEGEWAR		= 10 * 1000;
	DELAY_PLAYER_RESURRECT_TIMEOUT_SURVIVALZONE	= 0;
	
	// 공격 준비 동작 지속 시간 (msec)
	BATTLE_STAND_KEEP_TIME	= 10000;
}
...
ClientConfig::SaveToFile(const char* filename)
{
	...
	URL_HOMEPAGE_NEW_USER_NETMARBLE.SaveToFile( file );		// 새 사용자 등록 URL
...
#if __CONTENTS(__LEVEL_WAR_RENEWAL)
	file.write((const char*)&LevelWarItemColorSet, 4);
#endif //__LEVEL_WAR_RENEWAL
	...
	file.write((const char*)&DELAY_PLAYER_RESURRECT_TIMEOUT_SIEGEWAR, 4);

	// 공격 준비 동작 지속 시간 
	file.write((const char*)&BATTLE_STAND_KEEP_TIME, 4);
}
...
ClientConfig::LoadFromFile(const char* filename)
{
	...
	URL_HOMEPAGE_NEW_USER_NETMARBLE.LoadFromFile( file );		// 새 사용자 등록 URL
...
#if __CONTENTS(__LEVEL_WAR_RENEWAL)
	READ_CHECK_EOF( LevelWarItemColorSet, temp, 4);
#endif //__LEVEL_WAR_RENEWAL
	...
	READ_CHECK_EOF( DELAY_PLAYER_RESURRECT_TIMEOUT_SIEGEWAR, temp, 4 );

	// 공격 준비 동작 지속 시간 
	READ_CHECK_EOF( BATTLE_STAND_KEEP_TIME, temp, 4 );
