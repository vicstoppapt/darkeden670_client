		...
		if(itr->second.eventDelay != -1)
		{
			if((GetTickCount() - itr->second.eventStartTickCount) > itr->second.eventDelay)
			{
				...
				if(delete_id == EVENTID_GDR_PRESENT)
					SetFadeStart(31, -1, 1, 0,0,0, 5, FADE_DELAY_POSTION_BACK);
				else
				if(delete_id == EVENTID_FADE_OUT)
					SetFadeStart(0, 31, 1, 0, 0, 0, 3, FADE_DELAY_POSTION_FRONT);
...
bool MEventManager::AssertEventBackground(EVENTBACKGROUND_ID id)
{
	static std::string strFilename[EVENTBACKGROUNDID_MAX] = 
	{
		...
		"Data\\Image\\EventBackgroundCosmos",
		"Data\\Image\\EventBackgroundOustersSlayer",
		"Data\\Image\\EventBackgroundOustersVampire",
		"Data\\Image\\EventBackgroundQuest2",
		"Data\\Image\\EventBackgroundCloud",
	};
...
#	ifdef __GAME_CLIENT__
	...
	{
		for(int i = 0; i < EVENTBACKGROUNDID_MAX; ++i)
			strFilename[i] += "_1024.jps";
	}
	else
	{
		for(int i = 0; i < EVENTBACKGROUNDID_MAX; ++i)
