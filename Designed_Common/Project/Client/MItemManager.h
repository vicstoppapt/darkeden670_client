...
class MItemManager {
	public :
		...
		typedef	std::list<MItem*>					ITEM_LIST;

	public :
		MItemManager();
		virtual ~MItemManager();
		...
		// Add / Get / Remove
		//------------------------------------------------------
		virtual bool	AddItem(MItem* pItem);				// Item 추가
	...
	//protected :
...
#if __CONTENTS(__GEAR_SWAP_CHANGE)	//CheckAffectStatusAllGear, CheckAffectStatusPickupItem
		void			CheckAffectStatusAllGear();
		void			CheckAffectStatusPickupItem();	//들고 있는 아이템
#endif //CheckAffectStatusAllGear
		...
		int				FindItemList( MItemFinder& itemFinder, ITEM_LIST& findList) ;

		MItem*			FindItemOrderByIndex( MItemFinder& itemFinder ) const;
		MItem*			FindItemAllOrderByIndex( MItemFinder& itemFinder, MItem*& pSubInventoryItem ) const;
